package p2;

import e9.i0;
import e9.k0;
import java.util.List;
import java.util.Map;
public final class m extends q {
    public final int d;
    public final long e;
    public final boolean f40461f;
    public final boolean f40462g;
    public final long h;
    public final boolean f40463i;
    public final int f40464j;
    public final long f40465k;
    public final int f40466l;
    public final long f40467m;
    public final long f40468n;
    public final boolean f40469o;
    public final boolean f40470p;
    public final b2.o f40471q;
    public final i0 f40472r;
    public final i0 f40473s;
    public final k0 f40474t;
    public final long f40475u;
    public final l v;
    public final i0 f40476w;

    public m(int i10, String str, List list, long j3, boolean z10, long j10, boolean z11, int i11, long j11, int i12, long j12, long j13, boolean z12, boolean z13, boolean z14, b2.o oVar, List list2, List list3, l lVar, Map map, List list4) {
        super(str, list, z12);
        boolean z15;
        this.d = i10;
        this.h = j10;
        this.f40462g = z10;
        this.f40463i = z11;
        this.f40464j = i11;
        this.f40465k = j11;
        this.f40466l = i12;
        this.f40467m = j12;
        this.f40468n = j13;
        this.f40469o = z13;
        this.f40470p = z14;
        this.f40471q = oVar;
        this.f40472r = i0.v(list2);
        this.f40473s = i0.v(list3);
        this.f40474t = k0.a(map);
        this.f40476w = i0.v(list4);
        if (!list3.isEmpty()) {
            h hVar = (h) e9.q.l(list3);
            this.f40475u = hVar.e + hVar.f40453c;
        } else if (!list2.isEmpty()) {
            j jVar = (j) e9.q.l(list2);
            this.f40475u = jVar.e + jVar.f40453c;
        } else {
            this.f40475u = 0L;
        }
        long j14 = -9223372036854775807L;
        if (j3 != -9223372036854775807L) {
            if (j3 >= 0) {
                j14 = Math.min(this.f40475u, j3);
            } else {
                j14 = Math.max(0L, this.f40475u + j3);
            }
        }
        this.e = j14;
        if (j3 >= 0) {
            z15 = true;
        } else {
            z15 = false;
        }
        this.f40461f = z15;
        this.v = lVar;
    }

    @Override
    public final Object a(List list) {
        return this;
    }
}
