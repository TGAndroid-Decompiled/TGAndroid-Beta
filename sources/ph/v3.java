package ph;
public final class v3 implements o1.f {
    public final int f42475a;
    public final c5 f42476b;
    public final boolean f42477c;

    public v3(c5 c5Var, boolean z4, int i10) {
        this.f42475a = i10;
        this.f42476b = c5Var;
        this.f42477c = z4;
    }

    @Override
    public final void a(o1.h hVar, boolean z4, float f10, float f11) {
        switch (this.f42475a) {
            case 0:
                c5 c5Var = this.f42476b;
                h4 h4Var = c5Var.f41362t1;
                if (hVar == c5Var.f41374z1) {
                    c5Var.f41374z1 = null;
                    if (!this.f42477c) {
                        h4Var.setVisibility(8);
                        cg.f1.e(c5Var.C1).g();
                        h4Var.getAdapter().l();
                        return;
                    }
                    return;
                }
                return;
            default:
                c5 c5Var2 = this.f42476b;
                dg.q2 q2Var = c5Var2.f41343j1;
                if (hVar == c5Var2.f41360s1) {
                    c5Var2.f41360s1 = null;
                    if (!this.f42477c) {
                        q2Var.setVisibility(8);
                    }
                    q2Var.setMaskProvider(null);
                    return;
                }
                return;
        }
    }
}
