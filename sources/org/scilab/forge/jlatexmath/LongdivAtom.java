...
public class LongdivAtom extends VRowAtom {
    public LongdivAtom(long j10, long j11) {
        setHalign(1);
        setVtop(true);
        String[] strArrMakeResults = makeResults(j10, j11);
        RuleAtom ruleAtom = new RuleAtom(1, 0.0f, 1, 2.6f, 1, 0.5f);
        for (int i10 = 0; i10 < strArrMakeResults.length; i10++) {
            Atom atom = new TeXFormula(strArrMakeResults[i10]).root;
            if (i10 % 2 == 0) {
    ...
    private String[] makeResults(long j10, long j11) {
        ...
        String string = Long.toString(j12);
        int length = string.length();
        for (int i10 = 0; i10 < length; i10++) {
            long jCharAt = ((long) (string.charAt(i10) - '0')) * ((long) Math.pow(10.0d, (length - i10) - 1)) * j10;
            j11 -= jCharAt;
