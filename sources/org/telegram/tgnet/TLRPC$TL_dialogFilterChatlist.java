...
public class TLRPC$TL_dialogFilterChatlist extends TLRPC$DialogFilter {
    ...
    public void serializeToStream(AbstractSerializedData abstractSerializedData) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ((TLRPC$InputPeer) this.pinned_peers.get(i2)).serializeToStream(abstractSerializedData);
        }
        abstractSerializedData.writeInt32(481674261);
        int size2 = this.include_peers.size();
        abstractSerializedData.writeInt32(size2);
        for (int i3 = 0; i3 < size2; i3++) {
            ((TLRPC$InputPeer) this.include_peers.get(i3)).serializeToStream(abstractSerializedData);
        }
    }
}
