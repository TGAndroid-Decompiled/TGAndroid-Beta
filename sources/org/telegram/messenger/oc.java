package org.telegram.messenger;

import org.telegram.tgnet.TLObject;
public final class oc implements Runnable {
    public final int f17172a = 1;
    public final MessagesController f17173b;
    public final long f17174c;
    public final TLObject d;

    public oc(MessagesController messagesController, long j3, TLObject tLObject) {
        this.f17173b = messagesController;
        this.f17174c = j3;
        this.d = tLObject;
    }

    @Override
    public final void run() {
        switch (this.f17172a) {
            case 0:
                this.f17173b.lambda$deleteUserPhoto$114(this.d, this.f17174c);
                return;
            default:
                this.f17173b.lambda$loadPeerSettings$79(this.f17174c, this.d);
                return;
        }
    }

    public oc(MessagesController messagesController, TLObject tLObject, long j3) {
        this.f17173b = messagesController;
        this.d = tLObject;
        this.f17174c = j3;
    }
}
