    ...
    public static void lambda$startJob$0() {
        ...
        try {
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m34d("starting keep-alive job");
            }
            synchronized (sync) {
                startingJob = true;
            }
    ...
    public static void finishJobInternal() {
        synchronized (sync) {
            if (countDownLatch != null) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m34d("finish keep-alive job");
                }
                countDownLatch.countDown();
            }
            if (startingJob) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m34d("finish queued keep-alive job");
                }
                startingJob = false;
            }
        }
    ...
    protected void onHandleWork(Intent intent) {
        synchronized (sync) {
            if (startingJob) {
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m34d("started keep-alive job");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m34d("ended keep-alive job");
                }
            }
        }
    }
