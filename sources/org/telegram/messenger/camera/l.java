package org.telegram.messenger.camera;

import org.telegram.messenger.camera.CameraView;
public final class l implements Runnable {
    public final int f16074a = 1;
    public final CameraView f16075b;
    public final int f16076c;
    public final CameraView.CameraGLThread d;

    public l(CameraView cameraView, int i10, CameraView.CameraGLThread cameraGLThread) {
        this.f16075b = cameraView;
        this.f16076c = i10;
        this.d = cameraGLThread;
    }

    @Override
    public final void run() {
        switch (this.f16074a) {
            case 0:
                CameraView.CameraGLThread cameraGLThread = this.d;
                CameraView.i(this.f16075b, this.f16076c, cameraGLThread);
                return;
            default:
                CameraView.j(this.f16075b, this.f16076c, this.d);
                return;
        }
    }

    public l(CameraView cameraView, CameraView.CameraGLThread cameraGLThread, int i10) {
        this.f16075b = cameraView;
        this.d = cameraGLThread;
        this.f16076c = i10;
    }
}
