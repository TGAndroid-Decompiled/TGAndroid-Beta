package org.telegram.messenger;

import org.telegram.messenger.Utilities;
import org.telegram.tgnet.TLMethod;
import org.telegram.tgnet.TLRPC;
public final class o9 implements Runnable {
    public final int f16931a;
    public final long f16932b;
    public final int f16933c;
    public final long d;
    public final Object e;
    public final Object f16934f;

    public o9(int i10, long j3, long j10, org.telegram.ui.ActionBar.g1 g1Var, org.telegram.ui.ActionBar.g1 g1Var2) {
        this.f16931a = 3;
        this.f16933c = i10;
        this.f16932b = j3;
        this.d = j10;
        this.e = g1Var;
        this.f16934f = g1Var2;
    }

    @Override
    public final void run() {
        int i10;
        switch (this.f16931a) {
            case 0:
                int i11 = this.f16933c;
                ((MediaDataController) this.e).lambda$loadBotInfo$199(this.f16932b, this.d, (Utilities.Callback) this.f16934f, i11);
                return;
            case 1:
                int i12 = this.f16933c;
                long j3 = this.d;
                ((MessagesController) this.e).lambda$loadFullChat$67(this.f16932b, (TLRPC.TL_messages_chatFull) this.f16934f, i12, j3);
                return;
            case 2:
                int i13 = this.f16933c;
                ((MessagesStorage) this.e).lambda$loadPendingTasks$20(this.f16932b, this.d, (TLMethod) this.f16934f, i13);
                return;
            default:
                org.telegram.ui.ActionBar.g1 g1Var = (org.telegram.ui.ActionBar.g1) this.e;
                org.telegram.ui.ActionBar.g1 g1Var2 = (org.telegram.ui.ActionBar.g1) this.f16934f;
                int i14 = this.f16933c;
                MessagesController messagesController = MessagesController.getInstance(i14);
                long j10 = this.f16932b;
                long j11 = this.d;
                if (messagesController.isDialogMuted(j10, j11)) {
                    g1Var.g(LocaleController.getString(R.string.UnmuteNotifications), R.drawable.msg_unmute, null);
                    i10 = org.telegram.ui.ActionBar.j6.w0(null, org.telegram.ui.ActionBar.j6.f19228x6, false);
                    g1Var2.setVisibility(8);
                } else {
                    g1Var.g(LocaleController.getString(R.string.MuteNotifications), R.drawable.msg_mute, null);
                    int w02 = org.telegram.ui.ActionBar.j6.w0(null, org.telegram.ui.ActionBar.j6.f19101q7, false);
                    g1Var2.setVisibility(0);
                    if (MessagesController.getInstance(i14).isDialogNotificationsSoundEnabled(j10, j11)) {
                        g1Var2.g(LocaleController.getString(R.string.SoundOff), R.drawable.msg_tone_off, null);
                    } else {
                        g1Var2.g(LocaleController.getString(R.string.SoundOn), R.drawable.msg_tone_on, null);
                    }
                    i10 = w02;
                }
                g1Var.c(i10, i10);
                g1Var.setSelectorColor(org.telegram.ui.ActionBar.j6.l1(0.1f, i10));
                return;
        }
    }

    public o9(BaseController baseController, long j3, long j10, Object obj, int i10, int i11) {
        this.f16931a = i11;
        this.e = baseController;
        this.f16932b = j3;
        this.d = j10;
        this.f16934f = obj;
        this.f16933c = i10;
    }

    public o9(MessagesController messagesController, long j3, TLRPC.TL_messages_chatFull tL_messages_chatFull, int i10, long j10) {
        this.f16931a = 1;
        this.e = messagesController;
        this.f16932b = j3;
        this.f16934f = tL_messages_chatFull;
        this.f16933c = i10;
        this.d = j10;
    }
}
