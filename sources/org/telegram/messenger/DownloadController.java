...
public class DownloadController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void lambda$startDownloadFile$5(final MessageObject messageObject) {
        ...
        while (true) {
            ...
            if (i >= this.recentDownloadingFiles.size()) {
                z2 = false;
                break;
            } else if (this.recentDownloadingFiles.get(i).getDocument().f850id == messageObject.getDocument().f850id) {
                z2 = true;
                break;
            } else {
                i++;
        ...
        if (!z2) {
            for (int i2 = 0; i2 < this.downloadingFiles.size(); i2++) {
                if (this.downloadingFiles.get(i2).getDocument().f850id == messageObject.getDocument().f850id) {
                    break;
                }
            }
        }
    ...
    public void lambda$startDownloadFile$4(MessageObject messageObject) {
        try {
            ...
            executeFast.bindLong(3, messageObject.getDocument().f850id);
    ...
    public void lambda$onDownloadComplete$7(final MessageObject messageObject) {
        ...
        while (true) {
            ...
            if (i >= this.downloadingFiles.size()) {
                z2 = false;
                break;
            } else if (this.downloadingFiles.get(i).getDocument().f850id == messageObject.getDocument().f850id) {
        ...
        if (z2) {
            ...
            while (true) {
                if (i2 >= this.recentDownloadingFiles.size()) {
                    z = false;
                    break;
                } else if (this.recentDownloadingFiles.get(i2).getDocument().f850id == messageObject.getDocument().f850id) {
                    break;
                } else {
                    i2++;
                }
    ...
    public void lambda$onDownloadComplete$6(MessageObject messageObject) {
        try {
            getMessagesStorage().getDatabase().executeFast(String.format(Locale.ENGLISH, "UPDATE downloading_documents SET state = 1, date = %d WHERE hash = %d AND id = %d", Long.valueOf(System.currentTimeMillis()), Integer.valueOf(messageObject.getDocument().dc_id), Long.valueOf(messageObject.getDocument().f850id))).stepThis().dispose();
    ...
    public void lambda$onDownloadFail$8(MessageObject messageObject, int i) {
        ...
        while (true) {
            if (i2 >= this.downloadingFiles.size()) {
                z = false;
                break;
            } else if (this.downloadingFiles.get(i2).getDocument().f850id == messageObject.getDocument().f850id) {
        ...
        if (z) {
            getNotificationCenter().postNotificationName(NotificationCenter.onDownloadingFilesChanged, new Object[0]);
            if (i == 0) {
                NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.showBulletin, 1, LocaleController.formatString("MessageNotFound", C0890R.string.MessageNotFound, new Object[0]));
            }
        }
    }
    ...
    public void lambda$onDownloadFail$9(MessageObject messageObject) {
        try {
            ...
            executeFast.bindLong(2, messageObject.getDocument().f850id);
            executeFast.step();
            executeFast.dispose();
        } catch (Exception e) {
            FileLog.m30e(e);
    ...
    public void lambda$deleteRecentFiles$13(ArrayList arrayList) {
        try {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                executeFast.bindLong(2, ((MessageObject) arrayList.get(i)).getDocument().f850id);
                executeFast.step();
                try {
