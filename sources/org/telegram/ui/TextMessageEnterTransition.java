...
public class TextMessageEnterTransition implements MessageEnterTransitionContainer.Transition {
    ...
    public TextMessageEnterTransition(final ChatMessageCell chatMessageCell, final ChatActivity chatActivity, RecyclerListView recyclerListView, final MessageEnterTransitionContainer messageEnterTransitionContainer, Theme.ResourcesProvider resourcesProvider) {
        ...
        if (editText.length() != charSequenceReplaceEmoji.length() || z2) {
            ...
            if (iArr[0] > 0) {
                ...
                height = chatActivityEnterView.getEditField().getLayout().getLineBottom(chatActivityEnterView.getEditField().getLayout().getLineForOffset(charSequenceTrim.length() + iArr[0])) - lineTop;
            } else {
                lineTop = 0;
            }
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            StaticLayout.Builder hyphenationFrequency = StaticLayout.Builder.obtain(charSequenceReplaceEmoji, 0, charSequenceReplaceEmoji.length(), textPaint2, width).setBreakStrategy(1).setHyphenationFrequency(0);
        ...
        if (!this.crossfade && i2 > 0 && i > 0) {
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                StaticLayout.Builder hyphenationFrequency2 = StaticLayout.Builder.obtain(spannableString, 0, spannableString.length(), textPaint2, width).setBreakStrategy(1).setHyphenationFrequency(0);
                ...
                this.layout = new StaticLayout(spannableString, textPaint3, width, alignment3, 1.0f, 0.0f, false);
                this.rtlLayout = new StaticLayout(spannableString2, textPaint3, width, alignment3, 1.0f, 0.0f, false);
            }
        }
        ...
        valueAnimatorOfFloat.addUpdateListener(new TextMessageEnterTransition$$ExternalSyntheticLambda1(0, chatActivityEnterView, this, messageEnterTransitionContainer));
    ...
    public void onDraw(Canvas canvas) {
        ...
        float x2 = (this.listView.getX() + this.messageView.getX()) - this.container.getX();
        float top = ((this.listView.getTop() + (this.messageView.getPaddingTop() + this.messageView.getTop())) - this.container.getY()) - (this.listViewTargetBottomPadding - this.listView.getPaddingBottom());
        ...
        float f21 = f20 > 0.4f ? 1.0f : f20 / 0.4f;
        float interpolation2 = CubicBezierInterpolator.EASE_OUT.getInterpolation(CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(f20));
        float f22 = this.textX + x2;
        float f23 = this.textY + top;
        float f24 = 1.0f - interpolation2;
        int measuredHeight = (int) ((y * interpolation2) + (this.container.getMeasuredHeight() * f24));
        ...
        canvas2.save();
        canvas2.clipRect(0.0f, ((this.chatActivity.getChatListViewPadding() + this.listView.getY()) - this.container.getY()) - AndroidUtilities.dp(3.0f), this.container.getMeasuredWidth(), this.container.getMeasuredHeight());
        canvas2.save();
        float backgroundDrawableLeft = ((f3 - (f2 - this.toXOffset)) * f4) + this.messageView.getBackgroundDrawableLeft() + x2;
        float backgroundDrawableTop = this.messageView.getBackgroundDrawableTop() + top;
        float f25 = 1.0f - interpolation;
        float y3 = ((this.drawableFromTop - this.container.getY()) * f25) + (backgroundDrawableTop * interpolation);
        float backgroundDrawableBottom = ((backgroundDrawableTop + (this.messageView.getBackgroundDrawableBottom() - this.messageView.getBackgroundDrawableTop())) * interpolation) + ((this.drawableFromBottom - this.container.getY()) * f25);
        int iDp = (int) ((AndroidUtilities.dp(4.0f) * f4) + this.messageView.getBackgroundDrawableRight() + x2);
        ...
        if (currentBackgroundDrawable != null) {
            ...
            if (f21 != 1.0f && (drawable = this.fromMessageDrawable) != null) {
                drawable.setBounds((int) backgroundDrawableLeft, (int) y3, iDp, (int) backgroundDrawableBottom);
                this.fromMessageDrawable.draw(canvas2);
            }
            if (shadowDrawable != null) {
                shadowDrawable.setAlpha((int) (f * 255.0f));
                shadowDrawable.setBounds((int) backgroundDrawableLeft, (int) y3, iDp, (int) backgroundDrawableBottom);
                shadowDrawable.draw(canvas2);
                shadowDrawable.setAlpha(255);
            }
            currentBackgroundDrawable.setAlpha((int) (f21 * 255.0f));
            currentBackgroundDrawable.setBounds((int) backgroundDrawableLeft, (int) y3, iDp, (int) backgroundDrawableBottom);
        ...
        if (currentBackgroundDrawable != null) {
            if (this.currentMessageObject.isOutOwner()) {
                canvas2.clipRect(AndroidUtilities.dp(4.0f) + backgroundDrawableLeft, AndroidUtilities.dp(4.0f) + y3, iDp - AndroidUtilities.dp(10.0f), backgroundDrawableBottom - AndroidUtilities.dp(4.0f));
            } else {
                canvas2.clipRect(AndroidUtilities.dp(4.0f) + backgroundDrawableLeft, AndroidUtilities.dp(4.0f) + y3, iDp - AndroidUtilities.dp(4.0f), backgroundDrawableBottom - AndroidUtilities.dp(4.0f));
            }
        }
        float x3 = (this.listView.getX() + this.messageView.getLeft()) - this.container.getX();
        float fM = DiffUtil.m(y2, f23, f25, top);
        canvas2.translate(x3, fM);
        float f26 = f21;
        this.messageView.drawTime(canvas2, f26, false);
        this.messageView.drawNamesLayout(canvas2, f26);
        this.messageView.drawCommentButton(canvas2, f26);
        this.messageView.drawCaptionLayout(canvas2, false, f26);
        this.messageView.drawReactionsLayout(canvas2, f26, null);
        this.messageView.drawCommentLayout(canvas2, f26);
        this.messageView.drawLinkPreview(canvas2, f26);
        canvas2.restore();
        if (this.hasReply) {
            this.chatActivity.getReplyNameTextView().setAlpha(0.0f);
            this.chatActivity.getReplyObjectTextView().setAlpha(0.0f);
            f8 = f;
            AndroidUtilities.lerp(AndroidUtilities.dp(35.0f), this.messageView.replyHeight, f8);
            int iDp2 = AndroidUtilities.dp(10.0f);
            float x4 = this.replyFromStartX - this.container.getX();
            float y4 = this.replyFromStartY - this.container.getY();
            ChatMessageCell chatMessageCell2 = this.messageView;
            float f27 = x2 + chatMessageCell2.replyStartX;
            float f28 = top + chatMessageCell2.replyStartY;
            ...
            if (this.currentMessageObject.shouldDrawWithoutBackground()) {
                ...
                if (this.currentMessageObject.isReplyToStory()) {
                    ...
                    themedColor2 = ColorUtils.blendARGB(f18, themedColor4, Theme.adaptHue(themedColor4, themedColor));
                }
                ...
                int themedColor5 = getThemedColor(Theme.key_chat_outReplyMessageText);
                MessageObject messageObject4 = this.currentMessageObject;
                f6 = f25;
                if (messageObject4.forceAvatar) {
                    themedColor3 = themedColor5;
                    f19 = 0.0f;
                } else if (messageObject4.hasValidReplyMessageObject()) {
                ...
                i = iDp;
                themedColor2 = ColorUtils.blendARGB(f19, themedColor3, Theme.adaptHue(themedColor3, themedColor));
            }
            interpolation = interpolation;
            Theme.chat_replyTextPaint.setColor(ColorUtils.blendARGB(interpolation, this.replayObjectFromColor, themedColor2));
            Theme.chat_replyNamePaint.setColor(ColorUtils.blendARGB(interpolation, this.replayFromColor, themedColor));
            if (this.messageView.needReplyImage) {
                x4 -= AndroidUtilities.dp(44.0f);
            }
            float f29 = x4;
            float fLerp = AndroidUtilities.lerp(f29, f27, f8);
            float fLerp2 = AndroidUtilities.lerp((AndroidUtilities.dp(12.0f) * interpolation) + y4, f28, interpolation);
            ...
            rectF.set(f29, y4, this.replyFromStartWidth + f29, AndroidUtilities.dp(35.0f) + y4);
            ...
            this.messageReplySelectorRect.offset(x2, fM);
            AndroidUtilities.lerp(rectF, this.messageReplySelectorRect, f8, this.replySelectorRect);
            ChatMessageCell chatMessageCell4 = this.messageView;
            chatMessageCell4.replyLine.drawBackground(canvas, this.replySelectorRect, f26, chatMessageCell4.isReplyQuote, chatMessageCell4.getMessageObject().shouldDrawWithoutBackground());
            canvas2 = canvas;
            f7 = f26;
            this.messageView.replyLine.drawLine(canvas2, this.replySelectorRect, f7);
            if (this.messageView.needReplyImage) {
                canvas2.save();
                float fLerp3 = AndroidUtilities.lerp(AndroidUtilities.dp(35.0f), Math.min(this.replySelectorRect.height() - AndroidUtilities.dp(10.0f), Theme.chat_replyTextPaint.getTextSize() + Theme.chat_replyNamePaint.getTextSize() + AndroidUtilities.dp(this.messageView.isReplyQuote ? 3.0f : 7.0f)), f8);
                ImageReceiver imageReceiver = this.messageView.replyImageReceiver;
                float fLerp4 = AndroidUtilities.lerp(fLerp, this.replySelectorRect.left + AndroidUtilities.dp(8.0f), f8);
                float f30 = this.replySelectorRect.top;
                ChatMessageCell chatMessageCell5 = this.messageView;
                imageReceiver.setImageCoords(fLerp4, AndroidUtilities.lerp(fLerp2, f30 + AndroidUtilities.dp(((!chatMessageCell5.isReplyQuote || (staticLayout = chatMessageCell5.replyTextLayout) == null || staticLayout.getLineCount() > 1) ? 0 : 2) + 5), f8), fLerp3, fLerp3);
            ...
            canvas2.save();
            float f31 = iDp2 * f8;
            canvas2.translate(f31, 0.0f);
            MessageObject messageObject6 = this.currentMessageObject;
            float f32 = -((messageObject6 == null || !messageObject6.shouldDrawWithoutBackground()) ? AndroidUtilities.dp(1.0f) : AndroidUtilities.dp(6.0f));
            MessageObject messageObject7 = this.currentMessageObject;
            float fDp2 = (messageObject7 == null || !messageObject7.shouldDrawWithoutBackground()) ? AndroidUtilities.dp(3.0f) : AndroidUtilities.dp(1.0f);
            float f33 = this.messageView.replyTextOffset;
            float f34 = (f27 - f33) + f32;
            float f35 = (f27 - this.replyNameDx) + f32;
            AndroidUtilities.lerp(f29 - f33, f34, f8);
            float fLerp5 = AndroidUtilities.lerp(f29, f35, f8) + (this.messageView.needReplyImage ? AndroidUtilities.dp(3.0f) + f17 : 0.0f);
            if (this.messageView.replyNameLayout != null) {
                canvas2.save();
                canvas2.translate(fLerp5, (fDp2 * f8) + fLerp2);
                ...
                canvas2.saveLayerAlpha(0.0f, 0.0f, replyNameTextView.getWidth(), replyNameTextView.getHeight(), (int) (f4 * f5), 31);
            ...
            if (chatMessageCell6.isReplyQuote && chatMessageCell6.replyQuoteDrawable != null) {
                ...
                chatMessageCell9.replyQuoteDrawable.setBounds((int) (((this.replySelectorRect.right - f31) - AndroidUtilities.dp((!chatMessageCell9.drawPinnedTop ? 1 : 0) + 2)) - this.messageView.replyQuoteDrawable.getIntrinsicWidth()), (int) (this.replySelectorRect.top + AndroidUtilities.dp((!this.messageView.drawPinnedTop ? 1 : 0) + 2)), (int) ((this.replySelectorRect.right - f31) - AndroidUtilities.dp((!this.messageView.drawPinnedTop ? 1 : 0) + 2)), (int) (this.replySelectorRect.top + AndroidUtilities.dp((!this.messageView.drawPinnedTop ? 1 : 0) + 2) + this.messageView.replyQuoteDrawable.getIntrinsicHeight()));
                this.messageView.replyQuoteDrawable.setAlpha((int) (f8 * f5));
                this.messageView.replyQuoteDrawable.draw(canvas2);
            }
            if (this.messageView.replyTextLayout != null) {
                canvas2.save();
                float fLerp6 = AndroidUtilities.lerp(AndroidUtilities.dp(19.0f), Theme.chat_replyNamePaint.getTextSize() + AndroidUtilities.dp(4.0f) + fDp2, f8) + fLerp2;
                ChatMessageCell chatMessageCell10 = this.messageView;
                float fDp3 = (chatMessageCell10.isReplyQuote && chatMessageCell10.needReplyImage) ? f34 - AndroidUtilities.dp(2.0f) : f34;
                ...
                if (chatMessageCell11.needReplyImage && (!chatMessageCell11.isReplyQuote || chatMessageCell11.replyTextRTL)) {
                    fDp3 += f17 + AndroidUtilities.dp(3.0f);
                }
                ...
                if (chatMessageCell12.isReplyTaskOrPollOption && chatMessageCell12.replyTaskCheckbox != null) {
                    float fLerp7 = AndroidUtilities.lerp(f29 - chatMessageCell12.replyTextOffset, fDp3, f8);
                    this.messageView.replyTaskCheckbox.setBounds((int) fLerp7, AndroidUtilities.dp(2.0f) + ((int) fLerp6), AndroidUtilities.dp(12.0f), AndroidUtilities.dp(12.0f));
                ...
                if (chatMessageCell13.replyTextRTL && chatMessageCell13.replyTextOffset > 0) {
                    fDp3 = ((this.replySelectorRect.right - AndroidUtilities.dp(8.0f)) - this.messageView.replyTextLayout.getWidth()) - f31;
                }
                canvas2.translate(AndroidUtilities.lerp(r7 - this.messageView.replyTextOffset, fDp3, f8), fLerp6);
                canvas2.save();
                ...
                for (SpoilerEffect spoilerEffect : this.messageView.replySpoilers) {
                    boolean z2 = spoilerEffect.shouldInvalidateColor;
        ...
        if (this.messageView.getMessageObject() == null || this.messageView.getMessageObject().type != 19) {
            canvas2.clipRect(backgroundDrawableLeft + AndroidUtilities.dp(4.0f), y3 + AndroidUtilities.dp(4.0f), i - AndroidUtilities.dp(4.0f), backgroundDrawableBottom - AndroidUtilities.dp(4.0f));
        }
        float f36 = (this.scaleFrom * f4) + f8;
        float f37 = this.drawBitmaps ? (this.scaleY * f4) + f8 : 1.0f;
        canvas2.save();
        float f38 = f3 * f4;
        float f39 = f2;
        float f40 = y2 * f6;
        canvas2.translate(DiffUtil.m(f39, this.toXOffset, f8, f38), ((this.textLayoutBlock.textYOffset(this.messageView.getMessageObject().textLayoutBlocks, this.messageView.transitionParams) + f23) * interpolation) + f40);
        float f41 = f37 * f36;
        canvas2.scale(f36, f41, 0.0f, 0.0f);
        if (this.drawBitmaps) {
            if (this.crossfade) {
                f16 = f7;
                this.bitmapPaint.setAlpha((int) ((1.0f - f16) * f5));
            } else {
                f16 = f7;
            }
            ...
            if (z3 && this.changeColor) {
                ...
                this.layout.getPaint().setColor(ColorUtils.blendARGB(f42, this.fromColor, this.toColor));
                float f43 = 1.0f - f42;
                f9 = f41;
                f10 = f42;
                canvas2.saveLayerAlpha(0.0f, 0.0f, this.layout.getWidth(), this.layout.getHeight(), (int) (f43 * f5), 31);
                this.layout.draw(canvas2);
                f12 = f40;
                f13 = f38;
                f11 = f23;
                f14 = f39;
                AnimatedEmojiSpan.drawAnimatedEmojis(canvas2, this.layout, this.animatedEmojiStack, 0.0f, null, 0.0f, 0.0f, 0.0f, f43);
                ...
                if (z3) {
                    float f44 = 1.0f - f10;
                    canvas2 = canvas;
                    canvas2.saveLayerAlpha(0.0f, 0.0f, this.layout.getWidth(), this.layout.getHeight(), (int) (f44 * f5), 31);
                    this.layout.draw(canvas2);
        ...
        if (this.rtlLayout != null) {
            canvas2.save();
            canvas2.translate(DiffUtil.m(f14, this.toXOffsetRtl, f8, f13), ((this.textLayoutBlock.textYOffset(this.messageView.getMessageObject().textLayoutBlocks, this.messageView.transitionParams) + f11) * interpolation) + f12);
            f15 = f9;
            canvas2.scale(f36, f15, 0.0f, 0.0f);
            if (this.drawBitmaps) {
                if (this.crossfade) {
                    this.bitmapPaint.setAlpha((int) ((1.0f - f10) * f5));
                }
                ...
                if (z4 && this.changeColor) {
                    ...
                    this.rtlLayout.getPaint().setColor(ColorUtils.setAlphaComponent(ColorUtils.blendARGB(f10, this.fromColor, this.toColor), (int) ((1.0f - f10) * Color.alpha(color4))));
                    this.rtlLayout.draw(canvas2);
                    this.rtlLayout.getPaint().setColor(color4);
                } else if (z4) {
                    int alpha2 = this.rtlLayout.getPaint().getAlpha();
                    this.rtlLayout.getPaint().setAlpha((int) ((1.0f - f10) * alpha2));
                    this.rtlLayout.draw(canvas2);
        ...
        if (this.crossfade) {
            canvas2.save();
            canvas2.translate(DiffUtil.m(f3, f14, f4, (this.listView.getX() + this.messageView.getLeft()) - this.container.getX()), fM);
            canvas2.scale(f36, f15, this.messageView.getTextX(), this.messageView.getTextY());
            canvas2.translate(0.0f, -this.crossfadeTextOffset);
            if (this.crossfadeTextBitmap != null) {
                this.bitmapPaint.setAlpha((int) (f10 * f5));
                ...
                float f45 = f10;
                chatMessageCell15.drawMessageText(canvas, chatMessageCell15.getMessageObject().textLayoutBlocks, true, f45, true);
                canvas2 = canvas;
                this.messageView.drawAnimatedEmojis(canvas2, f45);
        ...
        if (z) {
            ...
            canvas2.drawRect(0.0f, f46, this.container.getMeasuredWidth(), this.container.getMeasuredHeight(), this.gradientPaint);
            canvas2.restore();
        }
        ...
        canvas2.saveLayerAlpha(0.0f, 0.0f, sendButton.getWidth(), sendButton.getHeight(), (int) ((1.0f - f48) * f5));
