package org.telegram.ui;

import android.animation.Animator;
import android.animation.AnimatorListenerAdapter;
public final class tt0 extends AnimatorListenerAdapter {
    public final int f37040a;
    public final ut0 f37041b;

    public tt0(ut0 ut0Var, int i10) {
        this.f37041b = ut0Var;
        this.f37040a = i10;
    }

    @Override
    public final void onAnimationEnd(Animator animator) {
        if (this.f37041b.f37341b.f30100j8) {
            PhotoViewer photoViewer = this.f37041b.f37341b;
            if (photoViewer.f30162r1) {
                photoViewer.B3();
            }
        }
        if (this.f37040a == 3) {
            PhotoViewer photoViewer2 = this.f37041b.f37341b;
            photoViewer2.G2(photoViewer2.P4, false, true, true);
        }
    }

    @Override
    public final void onAnimationStart(Animator animator) {
        int i10;
        PhotoViewer photoViewer = this.f37041b.f37341b;
        photoViewer.P0.setVisibility(0);
        if (photoViewer.E3()) {
            photoViewer.f30126n0.setVisibility(0);
        } else {
            photoViewer.S0.setVisibility(0);
        }
        photoViewer.F.setVisibility(0);
        if (photoViewer.f30086i2) {
            ru0 ru0Var = photoViewer.Q1;
            if (ru0Var.getTag() != null) {
                i10 = 0;
            } else {
                i10 = 4;
            }
            ru0Var.setVisibility(i10);
        }
        if (!photoViewer.f30042d2 && !photoViewer.f30051e2) {
            int i11 = photoViewer.f30033c2;
            if ((i11 == 0 || i11 == 4 || ((i11 == 2 || i11 == 5) && photoViewer.f30073g7.size() > 1)) && !photoViewer.f30062f4) {
                photoViewer.N0.setVisibility(0);
                photoViewer.O0.setVisibility(0);
                photoViewer.s3();
            }
        }
    }
}
