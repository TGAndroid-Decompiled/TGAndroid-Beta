package org.telegram.ui;

import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.LiteMode;
import org.telegram.messenger.Utilities;
public final class bh implements Utilities.Callback0Return {
    public final int f32377a;
    public final zn f32378b;

    public bh(zn znVar, int i10) {
        this.f32377a = i10;
        this.f32378b = znVar;
    }

    @Override
    public final Object run() {
        boolean z10;
        switch (this.f32377a) {
            case 0:
                if (LiteMode.isEnabled(65536) && org.telegram.ui.Components.m11.c()) {
                    zn znVar = this.f32378b;
                    org.telegram.ui.Components.m11 m11Var = znVar.f40462v0;
                    if (m11Var == null || m11Var.e) {
                        if (znVar.getParentActivity() != null && org.telegram.ui.Components.m11.c() && znVar.f40488x0 != null && znVar.X0 != null) {
                            org.telegram.ui.Components.m11 m11Var2 = znVar.f40462v0;
                            if (m11Var2 != null) {
                                AndroidUtilities.removeFromParent(m11Var2);
                            }
                            org.telegram.ui.Components.m11 m11Var3 = new org.telegram.ui.Components.m11(znVar.getParentActivity(), new org.telegram.ui.ActionBar.h6(24, znVar, r2));
                            znVar.f40462v0 = m11Var3;
                            org.telegram.ui.Components.m11[] m11VarArr = {m11Var3};
                            rm rmVar = znVar.X0;
                            rmVar.addView(m11Var3, rmVar.indexOfChild(znVar.f40488x0) + 1, w7.y5.c(-1.0f, -1));
                        }
                    }
                    return znVar.f40462v0;
                }
                return null;
            default:
                this.f32378b.getClass();
                if (org.telegram.ui.Components.m11.c() && LiteMode.isEnabled(65536)) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                return Boolean.valueOf(z10);
        }
    }
}
