...
public class SliderView extends View {
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        if (this.currentType == 0) {
            ...
            double dM = f3 - f4 != 0.0f ? DiffUtil.m(f3, f4, this.value, f4) : 0.0f;
            float f5 = this.wave1Alpha.set(dM > 0.25d);
            canvas.save();
            canvas.translate((1.0f - f5) * (-AndroidUtilities.dpf2(0.33f)), 0.0f);
            this.speakerWave1Paint.setAlpha((int) (f5 * 255.0f));
            canvas.drawPath(this.speakerWave1Path, this.speakerWave1Paint);
            canvas.restore();
            float f6 = this.wave2Alpha.set(dM > 0.5d);
            canvas.save();
            canvas.translate((1.0f - f6) * (-AndroidUtilities.dpf2(0.66f)), 0.0f);
