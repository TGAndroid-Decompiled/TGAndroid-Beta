package org.telegram.ui.Components;

import org.telegram.messenger.LanguageDetector;
import org.telegram.messenger.Utilities;
public final class u31 implements LanguageDetector.StringCallback, LanguageDetector.ExceptionCallback {
    public final String f28528a;
    public final String f28529b;
    public final Utilities.Callback2 f28530c;

    public u31(String str, String str2, Utilities.Callback2 callback2) {
        this.f28528a = str;
        this.f28529b = str2;
        this.f28530c = callback2;
    }

    @Override
    public void run(String str) {
        j41.x(this.f28528a, str, this.f28529b, this.f28530c);
    }

    @Override
    public void run(Exception exc) {
        j41.x(this.f28528a, "en", this.f28529b, this.f28530c);
    }
}
