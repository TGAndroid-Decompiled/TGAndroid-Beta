...
public class ReplyMessageLine {
    ...
    private void checkPatternBitmap(int i, int i2, int i3, float f, float f2, float f3) {
        int iMultAlpha = Theme.multAlpha(f3, i);
        int iCompositeColors = ColorUtils.compositeColors(Theme.multAlpha(f * f3, i2), iMultAlpha);
        int iCompositeColors2 = this.hasColor3 ? ColorUtils.compositeColors(Theme.multAlpha(f3 * f2, i3), iMultAlpha) : 0;
        int iRound = Math.round(AndroidUtilities.dpf2(6.33f));
        int iMax = Math.max(1, AndroidUtilities.dp(3.0f));
        int iDp = this.hasColor3 ? AndroidUtilities.dp(18.99f) : AndroidUtilities.dp(12.66f);
    ...
    private void incrementLoadingT() {
        ...
        this.loadingT = (Math.min(30L, jCurrentTimeMillis - this.lastLoadingTTime) * f) + this.loadingT;
        this.loadingTranslationT = (Math.min(30L, jCurrentTimeMillis - this.lastLoadingTTime) * f) + this.loadingTranslationT;
        ...
        if (i == 4 && (message4 = messageObject.messageOwner) != null && MessageObject.getMedia(message4) != null && (MessageObject.getMedia(messageObject.messageOwner) instanceof TLRPC.TL_messageMediaContact)) {
            ...
            if (colorId4 < 0) {
                if (DialogObject.isEncryptedDialog(messageObject.replyMessageObject.getDialogId())) {
                    ...
                    if (user5 != null) {
                        colorId4 = UserObject.getColorId(user5);
            ...
            resolveColor(messageObject.replyMessageObject, colorId4, resourcesProvider);
            this.backgroundColor = Theme.multAlpha(0.1f, this.color1);
            this.nameColor = this.color1;
        }
    ...
    public void setSimpleColor(int i, boolean z) {
        ...
        this.color2 = i;
        this.color1 = i;
        this.backgroundColor = Theme.multAlpha(z ? 0.12f : 0.1f, i);
        this.emojiColor = i;
    }
    ...
    public void drawLine(Canvas canvas, RectF rectF, float f) {
        ...
        this.color1Paint.setColor(Theme.multAlpha(f, i2));
        ...
        if (f2 > 0.0f && !this.hasColor2) {
            ...
            this.rectF.set(rectF.left, (AndroidUtilities.lerp(0.0f, 1.0f - CubicBezierInterpolator.EASE_IN.getInterpolation(MathUtils.clamp(((Math.max(fPow, 0.5f) + 1.5f) % 3.5f) * 0.5f, 0.0f, 1.0f)), f2) * rectF.height()) + rectF.top, rectF.left + AndroidUtilities.dp(6.0f), (AndroidUtilities.lerp(1.0f, 1.0f - CubicBezierInterpolator.EASE_OUT.getInterpolation(MathUtils.clamp((((fPow + 1.5f) % 3.5f) - 1.5f) * 0.5f, 0.0f, 1.0f)), f2) * rectF.height()) + rectF.top);
        ...
        float fDp = ((((this.switchStateT.set(this.switchedCount * 425) + this.loadingTranslationT) + (this.reversedOut ? 100 : 0)) / 1000.0f) * AndroidUtilities.dp(30.0f)) % (fHeight - i);
        checkPatternBitmap(i2, this.color2Animated.set(this.color2), this.color3Animated.set(this.color3), f3, f4, f);
    ...
    public void drawBackground(Canvas canvas, RectF rectF, float f, boolean z, boolean z2) {
        ...
        if (!z2) {
            this.backgroundPaint.setColor(Theme.multAlpha(f, this.backgroundColorAnimated.set(this.backgroundColor)));
            if (FBool.radiiAreSame(this.radii)) {
