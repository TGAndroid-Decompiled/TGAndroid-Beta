...
public class AudioVisualizerDrawable {
    ...
    public void setWaveform(boolean z, boolean z2, float[] fArr) {
        if (LiteMode.isEnabled(32)) {
            ...
            float fM = z3 ? this.ANIMATION_DURATION * 2.0f : this.ANIMATION_DURATION;
            if (this.lastAmplitudeCount > 6) {
                float f2 = 0.0f;
                for (int i5 = 0; i5 < 6; i5++) {
                    f2 += this.lastAmplitude[i5];
                }
                float f3 = f2 / 6.0f;
                if (f3 > 0.52f) {
                    fM = BotFullscreenButtons$$ExternalSyntheticOutline0.m(f3, 0.4f, this.ANIMATION_DURATION, fM);
                }
            }
            while (i < 7) {
                if (fArr == null) {
                ...
                if (this.parentView == null) {
                    ...
                    this.dt[i] = (this.animateTo[i] - this.current[i]) / fM;
                }
                i++;
            }
            ...
            fArr4[7] = f4;
            this.dt[7] = (f4 - this.current[7]) / 120.0f;
        }
    }
    ...
    public void draw(Canvas canvas, float f, float f2) {
        if (LiteMode.isEnabled(32)) {
            ...
            float fDp = (this.IDLE_RADIUS * f9) + (AndroidUtilities.dp(4.0f) * f10) + AndroidUtilities.dp(22.0f);
