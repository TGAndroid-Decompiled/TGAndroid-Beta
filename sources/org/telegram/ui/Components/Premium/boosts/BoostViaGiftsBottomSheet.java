...
public class BoostViaGiftsBottomSheet extends BottomSheetWithRecyclerListView implements SelectorBottomSheet.SelectedObjectsListener {
    ...
    public BoostViaGiftsBottomSheet(final BaseFragment baseFragment, boolean z, boolean z2, long j, final TL_stories$TL_prepaidGiveaway tL_stories$TL_prepaidGiveaway) {
        ...
        actionBtnCell.setOnClickListener(new View.OnClickListener() {
            ...
            public final void onClick(View view) {
                BoostViaGiftsBottomSheet.this.lambda$new$12(baseFragment, tL_stories$TL_prepaidGiveaway, view);
            }
        });
    ...
    public void lambda$new$2(int i) {
        ...
        this.actionBtn.updateCounter(selectedSliderValue);
        updateRows(false, false);
        this.adapter.updateBoostCounter(selectedSliderValue);
    }
    ...
    public void lambda$new$12(BaseFragment baseFragment, final TL_stories$TL_prepaidGiveaway tL_stories$TL_prepaidGiveaway, View view) {
        ...
        if (isPreparedGiveaway()) {
            ...
            BoostRepository.launchPreparedGiveaway(tL_stories$TL_prepaidGiveaway, this.selectedChats, this.selectedCountries, this.currentChat, prepareServerDate, z, new Utilities.Callback() {
                @Override
                public final void run(Object obj) {
                    BoostViaGiftsBottomSheet.this.lambda$new$6(tL_stories$TL_prepaidGiveaway, (Void) obj);
                }
            }, new Utilities.Callback() {
                @Override
                public final void run(Object obj) {
                    BoostViaGiftsBottomSheet.this.lambda$new$7((TLRPC$TL_error) obj);
                }
            });
            return;
        }
        for (int i2 = 0; i2 < filterGiftOptions2.size(); i2++) {
            ...
            if (tLRPC$TL_premiumGiftCodeOption2.months == this.selectedMonths) {
                if (BoostRepository.isGoogleBillingAvailable() && BoostDialogs.checkReduceQuantity(getContext(), this.resourcesProvider, this.giftCodeOptions, tLRPC$TL_premiumGiftCodeOption2, new Utilities.Callback() {
                    @Override
                    public final void run(Object obj) {
                        BoostViaGiftsBottomSheet.this.lambda$new$8((TLRPC$TL_premiumGiftCodeOption) obj);
                    }
                })) {
                    return;
                }
                ...
                BoostRepository.payGiveAway(this.selectedChats, this.selectedCountries, tLRPC$TL_premiumGiftCodeOption2, this.currentChat, prepareServerDate2, z2, baseFragment, new Utilities.Callback() {
                    @Override
                    public final void run(Object obj) {
                        BoostViaGiftsBottomSheet.this.lambda$new$10((Void) obj);
                    }
                }, new Utilities.Callback() {
                    @Override
                    public final void run(Object obj) {
    ...
    public void lambda$new$4(Void r5) {
        dismiss();
    ...
    public void lambda$new$5(TLRPC$TL_error tLRPC$TL_error) {
        this.actionBtn.updateLoading(false);
        BoostDialogs.showToastError(getContext(), tLRPC$TL_error);
    }

    public void lambda$new$6(TL_stories$TL_prepaidGiveaway tL_stories$TL_prepaidGiveaway, Void r6) {
        dismiss();
    ...
    public void lambda$new$7(TLRPC$TL_error tLRPC$TL_error) {
        this.actionBtn.updateLoading(false);
        BoostDialogs.showToastError(getContext(), tLRPC$TL_error);
    }

    public void lambda$new$8(TLRPC$TL_premiumGiftCodeOption tLRPC$TL_premiumGiftCodeOption) {
        this.selectedSliderIndex = this.sliderValues.indexOf(Integer.valueOf(tLRPC$TL_premiumGiftCodeOption.users));
        updateRows(true, true);
        updateActionButton(true);
    }

    public void lambda$new$10(Void r3) {
        dismiss();
        AndroidUtilities.runOnUIThread(new Runnable() {
            @Override
            public final void run() {
                BoostViaGiftsBottomSheet.this.lambda$new$9();
            }
        }, 220L);
    }
    ...
    private void loadOptions() {
        BoostRepository.loadGiftOptions(this.currentChat, new Utilities.Callback() {
            @Override
            public final void run(Object obj) {
                BoostViaGiftsBottomSheet.this.lambda$loadOptions$13((List) obj);
            }
        });
    }

    public void lambda$loadOptions$13(List list) {
    ...
    private void updateRows(boolean z, boolean z2) {
        ...
        if (isPreparedGiveaway()) {
            ...
            this.items.add(BoostAdapter.Item.asBoost(BoostTypeCell.TYPE_SPECIFIC_USERS, this.selectedUsers.size(), this.selectedUsers.size() > 0 ? (TLRPC$User) this.selectedUsers.get(0) : null, this.selectedBoostType));
        }
        this.items.add(BoostAdapter.Item.asDivider());
        if (this.selectedBoostType == BoostTypeCell.TYPE_GIVEAWAY) {
            if (!isPreparedGiveaway()) {
                this.items.add(BoostAdapter.Item.asSubTitleWithCounter(LocaleController.getString("BoostingQuantityPrizes", R.string.BoostingQuantityPrizes), getSelectedSliderValue()));
            ...
            this.items.add(BoostAdapter.Item.asChat(this.currentChat, false, getSelectedSliderValue()));
            for (TLObject tLObject : this.selectedChats) {
                if (tLObject instanceof TLRPC$Chat) {
                    this.items.add(BoostAdapter.Item.asChat((TLRPC$Chat) tLObject, true, getSelectedSliderValue()));
                }
                if (tLObject instanceof TLRPC$InputPeer) {
                    this.items.add(BoostAdapter.Item.asPeer((TLRPC$InputPeer) tLObject, true, getSelectedSliderValue()));
                }
            }
        ...
        this.items.add(BoostAdapter.Item.asDivider(AndroidUtilities.replaceSingleTag(isPreparedGiveaway() ? LocaleController.formatPluralString("BoostingChooseRandom", this.prepaidGiveaway.quantity, new Object[0]) + "\n\n" + LocaleController.getString("BoostingStoriesFeaturesAndTerms", R.string.BoostingStoriesFeaturesAndTerms) : LocaleController.getString("BoostingStoriesFeaturesAndTerms", R.string.BoostingStoriesFeaturesAndTerms), Theme.key_chat_messageLinkIn, 0, new Runnable() {
            @Override
            public final void run() {
                BoostViaGiftsBottomSheet.this.lambda$updateRows$16();
            }
    ...
    public void lambda$updateRows$16() {
        ...
        premiumPreviewBottomSheet.setOnDismissListener(new DialogInterface.OnDismissListener() {
            ...
            public final void onDismiss(DialogInterface dialogInterface) {
                BoostViaGiftsBottomSheet.this.lambda$updateRows$14(dialogInterface);
            }
        });
    ...
    public void lambda$updateRows$14(DialogInterface dialogInterface) {
        this.adapter.setPausedStars(false);
    }

    public void lambda$updateRows$15(DialogInterface dialogInterface) {
