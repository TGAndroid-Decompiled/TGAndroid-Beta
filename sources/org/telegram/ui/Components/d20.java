package org.telegram.ui.Components;

import android.animation.Animator;
import android.animation.AnimatorListenerAdapter;
import org.telegram.messenger.NotificationCenter;
public final class d20 extends AnimatorListenerAdapter {
    public final int f23185a;
    public final e20 f23186b;

    public d20(e20 e20Var, int i10) {
        this.f23185a = i10;
        this.f23186b = e20Var;
    }

    @Override
    public final void onAnimationEnd(Animator animator) {
        switch (this.f23185a) {
            case 0:
                e20 e20Var = this.f23186b;
                NotificationCenter.getInstance(e20Var.f23458r.f23728a).onAnimationFinish(e20Var.f23456f);
                e20Var.requestLayout();
                return;
            default:
                e20 e20Var2 = this.f23186b;
                e20Var2.d = null;
                e20Var2.f23453a = null;
                e20Var2.f23454b = false;
                return;
        }
    }
}
