...
public class ProfileActionsView extends View {
    ...
    private void applyVisibleActions() {
        ...
        AndroidUtilities.runOnUIThread(new UndoView$$ExternalSyntheticLambda3(7, this, arrayList));
    }
    ...
    private void drawAction(Canvas canvas, Action action, float f, float f2) {
        ...
        canvas.save();
        canvas.scale(action.textScale, action.textScale, fCenterX, Emoji$EmojiSpan$$ExternalSyntheticOutline0.m(action.text.getHeight(), action.textScale, 2.0f, height));
    ...
    private void drawLoading(Canvas canvas, Action action, float f) {
        ...
        if (action.isLoading) {
            ...
            if (loadingDrawable == null) {
                ...
                action.loadingDrawable.setColors(Theme.multAlpha(0.1f, -1), Theme.multAlpha(0.3f, -1), Theme.multAlpha(0.35f, -1), Theme.multAlpha(0.8f, -1));
    ...
    private void drawRenderNode(Canvas canvas) {
        ...
        if (avatarImageView != null) {
            ...
            this.clipAvatarPath.addRoundRect(x, y, scaleX + x, scaleY + y, this.avatarView.getRoundRadiusForExpand() * view.getScaleX(), this.avatarView.getRoundRadiusForExpand() * view.getScaleY(), Path.Direction.CCW);
    ...
    private void updateBounds(Action action) {
        ...
        action.textScale = f;
        float fMax = Math.max(0.0f, AndroidUtilities.dpf2(1.33f) + SvgHelper$SvgDrawable$$ExternalSyntheticOutline0.m(action.text.getHeight(), action.textScale, this.targetHeight, 3.0f));
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!action3.isDeleted) {
                ...
                if (this.renderNode != null) {
                    ...
                    rectF2.inset((f - action3.getScale()) * (action3.rect.width() / 2.0f), (f - action3.getScale()) * (action3.rect.height() / 2.0f));
        ...
        if (!this.ignoreRect) {
            for (int i2 = 0; i2 < size; i2++) {
                ...
                if (!action4.isDeleted) {
                    ...
                    rectF3.inset((1.0f - action4.getScale()) * (action4.rect.width() / 2.0f), (1.0f - action4.getScale()) * (action4.rect.height() / 2.0f));
                    ...
                    if (SharedConfig.shadowsInSections && isButtonColorLight() && this.parentExpanded < 0.5f) {
                        this.paint.setShadowLayer(AndroidUtilities.dpf2(1.5f), 0.0f, 0.0f, Theme.multAlpha((alpha2 / 255.0f) * (this.radialGradient == null ? 1.0f : 0.1f), 536870912));
                    } else {
        ...
        if (fClamp03 > 0.0f) {
            for (int i3 = 0; i3 < size; i3++) {
                drawAction(canvas, this.actions.get(i3), fClamp01, fClamp03);
            }
        }
    }
                ...
                if (action2 == 1 && this.hit.rect.contains(x, y)) {
                    ...
                    if (onActionClickListener != null) {
                        ...
                        if (i3 == 0) {
                            ...
                            onActionClickListener.onClick(i4, rectF.left, rectF.top);
                        } else {
                            postDelayed(new EmojiView$2$$ExternalSyntheticLambda0(16, this, action6), i3);
                        }
                    }
                }
                this.hit = null;
    ...
    public void set(int i, boolean z) {
        if (z ? this.allAvailableActions.add(Integer.valueOf(i)) : this.allAvailableActions.remove(Integer.valueOf(i))) {
            applyVisibleActions();
        }
    }
