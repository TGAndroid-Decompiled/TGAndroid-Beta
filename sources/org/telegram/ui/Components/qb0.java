package org.telegram.ui.Components;

import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.ColorFilter;
import android.graphics.Paint;
import android.graphics.Rect;
import android.graphics.drawable.Drawable;
import org.telegram.messenger.AndroidUtilities;
public final class qb0 extends Drawable {
    public pb0 f31878a;
    public final Paint f31879b;
    public int f31880c;
    public int d;
    public final long f31881e;
    public int f31882f;

    public qb0() {
        Paint paint = new Paint(1);
        this.f31879b = paint;
        this.f31880c = 255;
        this.d = 255;
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeCap(Paint.Cap.ROUND);
        paint.setStrokeWidth(AndroidUtilities.dp(1.0f));
        this.f31881e = System.currentTimeMillis();
    }

    public final void a(int i9) {
        if (i9 != this.f31882f) {
            int alpha = Color.alpha(i9);
            this.d = alpha;
            this.f31879b.setColor(i0.a.k(i9, (int) ((alpha / 255.0f) * this.f31880c)));
        }
        this.f31882f = i9;
    }

    @Override
    public final void draw(Canvas canvas) {
        Rect bounds = getBounds();
        int min = Math.min(bounds.width(), bounds.height());
        Paint paint = this.f31879b;
        canvas.drawCircle(bounds.centerX(), bounds.centerY(), (min >> 1) - AndroidUtilities.dp(0.5f), paint);
        long currentTimeMillis = System.currentTimeMillis();
        canvas.save();
        long j10 = this.f31881e;
        canvas.rotate(((((float) (currentTimeMillis - j10)) % 1500.0f) * 360.0f) / 1500.0f, bounds.centerX(), bounds.centerY());
        canvas.drawLine(bounds.centerX(), bounds.centerY(), bounds.centerX(), bounds.centerY() - AndroidUtilities.dp(3.0f), paint);
        canvas.restore();
        canvas.save();
        canvas.rotate(((((float) (currentTimeMillis - j10)) % 4500.0f) * 360.0f) / 4500.0f, bounds.centerX(), bounds.centerY());
        canvas.drawLine(bounds.centerX(), bounds.centerY(), AndroidUtilities.dp(2.3f) + bounds.centerX(), bounds.centerY(), paint);
        canvas.restore();
    }

    @Override
    public final Drawable.ConstantState getConstantState() {
        if (this.f31878a == null) {
            this.f31878a = new Drawable.ConstantState();
        }
        return this.f31878a;
    }

    @Override
    public final int getIntrinsicHeight() {
        return AndroidUtilities.dp(12.0f);
    }

    @Override
    public final int getIntrinsicWidth() {
        return AndroidUtilities.dp(12.0f);
    }

    @Override
    public final int getOpacity() {
        return -2;
    }

    @Override
    public final void setAlpha(int i9) {
        if (this.f31880c != i9) {
            this.f31880c = i9;
            this.f31879b.setAlpha((int) ((this.d / 255.0f) * i9));
        }
    }

    @Override
    public final void setColorFilter(ColorFilter colorFilter) {
    }
}
