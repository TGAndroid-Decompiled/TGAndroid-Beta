...
public class FireworksOverlay extends View {
    ...
    public class Particle {
        ...
        public boolean update(int i) {
            ...
            float f3 = this.moveX;
            this.x = (f3 * f) + f2;
            this.y = (this.moveY * f) + this.y;
            if (this.xFinished != 0) {
                float fDp = AndroidUtilities.dp(1.0f) * 0.5f;
                if (this.xFinished == 1) {
                    float f4 = (fDp * f * 0.05f) + this.moveX;
                    this.moveX = f4;
                    if (f4 >= fDp) {
            ...
            boolean z = f9 < f8;
            if (f9 > f8) {
                this.moveY = (FireworksOverlay.this.speedCoef * (AndroidUtilities.dp(1.0f) / 3.0f) * f) + f9;
            } else {
                this.moveY = zzir.m(AndroidUtilities.dp(1.0f), 3.0f, f, f9);
            }
            if (z && this.moveY > f8) {
            ...
            byte b = this.type;
            if (b == 1 || b == 2) {
                short s = (short) ((f * 10.0f) + this.rotation);
                this.rotation = s;
                if (s > 360) {
    ...
    public static int access$508(FireworksOverlay fireworksOverlay) {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.fallingDownCount >= particlesCount / 2 && this.speedCoef > 0.2f) {
            startFall();
            float fM = TextureRenderer$$ExternalSyntheticOutline0.m(i, 16.0f, 0.15f, this.speedCoef);
            this.speedCoef = fM;
            if (fM < 0.2f) {
                this.speedCoef = 0.2f;
            }
        ...
        this.started = false;
        AndroidUtilities.runOnUIThread(new GroupCallPip$$ExternalSyntheticLambda2(this, 14));
        onStop();
    }
