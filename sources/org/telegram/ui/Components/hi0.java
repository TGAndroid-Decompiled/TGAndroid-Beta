...
public final class hi0 extends Drawable {
    public Path f26764a;
    public Paint f26765b;
    public float f26766c;

    public final void a() {
        int dp = AndroidUtilities.dp(18.0f);
        Path path = this.f26764a;
        path.reset();
        float f7 = dp >> 1;
        ...
        Paint paint = this.f26765b;
        paint.setStyle(Paint.Style.FILL_AND_STROKE);
        paint.setStrokeJoin(Paint.Join.ROUND);
        paint.setStrokeWidth(AndroidUtilities.dpf2(1.0f));
        this.f26766c = AndroidUtilities.density;
    }

    @Override
    public final void draw(Canvas canvas) {
        Paint paint = this.f26765b;
        if (this.f26766c != AndroidUtilities.density) {
            a();
        }
        canvas.save();
        canvas.translate(getBounds().left, getBounds().top);
        canvas.drawPath(this.f26764a, paint);
