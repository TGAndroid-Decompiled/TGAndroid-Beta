...
public final class rw extends FrameLayout {
    ...
    public final boolean drawChild(Canvas canvas, View view, long j3) {
        kz kzVar = this.f30105b;
        xw xwVar = kzVar.B0;
        tw twVar = kzVar.D0;
        ww wwVar = kzVar.G0;
        if (!this.f30104a && (view == twVar || view == wwVar)) {
            canvas.save();
            float y3 = xwVar.getY() + xwVar.getMeasuredHeight() + 1.0f;
            if (view == twVar) {
                y3 = Math.max(y3, wwVar.getY() + wwVar.getMeasuredHeight() + 1.0f);
            }
            canvas.clipRect(0.0f, y3 - (AndroidUtilities.dp(16.0f) * kzVar.f27924a.f15368e), getMeasuredWidth(), getMeasuredHeight());
