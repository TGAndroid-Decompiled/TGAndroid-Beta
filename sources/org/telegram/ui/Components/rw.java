...
public final class rw extends FrameLayout {
    ...
    public final boolean drawChild(Canvas canvas, View view, long j3) {
        lz lzVar = this.f28054b;
        yw ywVar = lzVar.B0;
        tw twVar = lzVar.D0;
        xw xwVar = lzVar.G0;
        if (!this.f28053a && (view == twVar || view == xwVar)) {
            canvas.save();
            float y3 = ywVar.getY() + ywVar.getMeasuredHeight() + 1.0f;
            if (view == twVar) {
                y3 = Math.max(y3, xwVar.getY() + xwVar.getMeasuredHeight() + 1.0f);
            }
            canvas.clipRect(0.0f, y3 - (AndroidUtilities.dp(16.0f) * lzVar.f26228a.e), getMeasuredWidth(), getMeasuredHeight());
