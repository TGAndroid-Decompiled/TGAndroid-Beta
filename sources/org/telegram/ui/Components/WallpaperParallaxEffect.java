...
public class WallpaperParallaxEffect implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (length <= 1.0f) {
            ...
            fAtan2 = (float) (Math.atan2((f9 * (-1.0f)) - (f10 * 0.0f), (f9 * 0.0f) + (f10 * (-1.0f))) / 0.017453292519943295d);
            if (fAtan2 < 0.0f) {
                fAtan2 += 360.0f;
            }
        ...
        float fSqrt2 = (float) Math.sqrt((fMax3 * fMax3) + (fMax4 * fMax4));
        float f11 = fMax3 / fSqrt2;
        float f12 = fMax4 / fSqrt2;
        fAtan2 = (float) (Math.atan2((f11 * (-1.0f)) - (f12 * 0.0f), (f11 * 0.0f) + (f12 * (-1.0f))) / 0.017453292519943295d);
        if (fAtan2 < 0.0f) {
            fAtan2 += 360.0f;
        }
        callback = this.callback;
