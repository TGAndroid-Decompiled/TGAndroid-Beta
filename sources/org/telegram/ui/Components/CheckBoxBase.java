package org.telegram.ui.Components;

import android.animation.Animator;
import android.animation.AnimatorListenerAdapter;
import android.animation.ObjectAnimator;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.DashPathEffect;
import android.graphics.Matrix;
import android.graphics.Paint;
import android.graphics.Path;
import android.graphics.PorterDuff;
import android.graphics.PorterDuffXfermode;
import android.graphics.Rect;
import android.graphics.RectF;
import android.graphics.Shader;
import android.text.TextPaint;
import android.view.View;
import androidx.core.graphics.ColorUtils;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.GenericProvider;
import org.telegram.messenger.LocaleController;
import org.telegram.ui.ActionBar.MessageDrawable;
import org.telegram.ui.ActionBar.Theme;

public class CheckBoxBase {
    private static Paint forbidPaint;
    private static Paint paint;
    public long animationDuration;
    private boolean attachedToWindow;
    private int background2ColorKey;
    private int backgroundColor;
    private int backgroundColorKey;
    private Paint backgroundPaint;
    private int backgroundType;
    private ObjectAnimator checkAnimator;
    private Paint checkPaint;
    private String checkedText;
    private GenericProvider<Void, Paint> circlePaintProvider;
    private float customRadius;
    private float customRadiusFactor;
    private boolean cutCheck;
    private boolean drawUnchecked;
    private boolean forbidden;
    private boolean isChecked;
    private MessageDrawable messageDrawable;
    private View parentView;
    private float progress;
    private ProgressDelegate progressDelegate;
    private Theme.ResourcesProvider resourcesProvider;
    private float size;
    private int strokeBackgroundKey;
    private int strokeBackgroundWidth;
    private TextPaint textPaint;
    private boolean useDefaultCheck;
    public Rect bounds = new Rect();
    private RectF rect = new RectF();
    public float checkScale = 1.0f;
    private float alpha = 1.0f;
    private Path path = new Path();
    private boolean enabled = true;
    private float backgroundAlpha = 1.0f;
    private int checkColorKey = Theme.key_checkboxCheck;

    public interface ProgressDelegate {
        void setProgress(float f);
    }

    public CheckBoxBase(View view, int i, Theme.ResourcesProvider resourcesProvider) {
        int i2 = Theme.key_chat_serviceBackground;
        this.backgroundColorKey = i2;
        this.background2ColorKey = i2;
        this.strokeBackgroundKey = Theme.key_dialogBackground;
        this.strokeBackgroundWidth = -1;
        this.customRadius = 0.0f;
        this.customRadiusFactor = 1.0f;
        this.drawUnchecked = true;
        this.circlePaintProvider = new EmojiView$$ExternalSyntheticLambda30(13);
        this.animationDuration = 200L;
        this.resourcesProvider = resourcesProvider;
        this.parentView = view;
        this.size = i;
        if (paint == null) {
            paint = new Paint(1);
        }
        Paint paint2 = new Paint(1);
        this.checkPaint = paint2;
        paint2.setStrokeCap(Paint.Cap.ROUND);
        Paint paint3 = this.checkPaint;
        Paint.Style style = Paint.Style.STROKE;
        paint3.setStyle(style);
        this.checkPaint.setStrokeJoin(Paint.Join.ROUND);
        this.checkPaint.setStrokeWidth(AndroidUtilities.dp(1.9f));
        Paint paint4 = new Paint(1);
        this.backgroundPaint = paint4;
        paint4.setStyle(style);
        this.backgroundPaint.setStrokeWidth(AndroidUtilities.dp(1.2f));
    }

    private void animateToCheckedState(boolean z) {
        ObjectAnimator objectAnimatorOfFloat = ObjectAnimator.ofFloat(this, "progress", z ? 1.0f : 0.0f);
        this.checkAnimator = objectAnimatorOfFloat;
        objectAnimatorOfFloat.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animator) {
                if (animator.equals(CheckBoxBase.this.checkAnimator)) {
                    CheckBoxBase.this.checkAnimator = null;
                }
                if (CheckBoxBase.this.isChecked) {
                    return;
                }
                CheckBoxBase.this.checkedText = null;
            }
        });
        this.checkAnimator.setInterpolator(CubicBezierInterpolator.EASE_OUT);
        this.checkAnimator.setDuration(this.animationDuration);
        this.checkAnimator.start();
    }

    private int getThemedColor(int i) {
        return Theme.getColor(i, this.resourcesProvider);
    }

    private void invalidate() {
        View view = this.parentView;
        if (view == null) {
            return;
        }
        if (view.getParent() != null) {
            ((View) this.parentView.getParent()).invalidate();
        }
        this.parentView.invalidate();
    }

    public static Paint lambda$new$0(Void r0) {
        return paint;
    }

    public void cancelCheckAnimator() {
        ObjectAnimator objectAnimator = this.checkAnimator;
        if (objectAnimator != null) {
            objectAnimator.cancel();
            this.checkAnimator = null;
        }
    }

    public void draw(Canvas canvas) {
        float fDp;
        float f;
        float f2;
        int iCenterX;
        int iCenterY;
        boolean z;
        float f3;
        int i;
        float f4;
        float f5;
        Canvas canvas2;
        int i2;
        int i3;
        char c;
        int i4;
        int i5;
        int i6;
        float f6;
        int i7;
        int i8;
        float f7;
        int i9;
        int i10;
        float f8;
        boolean z2;
        int i11;
        int length;
        float f9;
        float f10;
        float fDp2;
        boolean z3;
        Paint paintProvide;
        int i12;
        int alpha;
        int i13;
        int i14;
        int i15;
        int i16;
        int i17;
        int i18;
        int i19;
        int i20;
        float fDp3 = AndroidUtilities.dp(this.size / 2.0f);
        int i21 = this.backgroundType;
        if (i21 != 12 && i21 != 13) {
            if (i21 != 0 && i21 != 11) {
                fDp = fDp3 - AndroidUtilities.dp(0.2f);
            }
            if (this.forbidden) {
                f = 1.0f;
            } else {
                f = this.progress;
            }
            if (f >= 0.5f) {
                f2 = 1.0f;
            } else {
                f2 = f / 0.5f;
            }
            iCenterX = this.bounds.centerX();
            iCenterY = this.bounds.centerY();
            if (this.cutCheck || f2 <= 0.0f || f < 0.5f || this.forbidden || this.checkedText != null) {
                z = false;
            } else {
                z = true;
            }
            if (z) {
                float f11 = iCenterX;
                float f12 = f;
                float f13 = f11 - fDp3;
                float f14 = iCenterY;
                float f15 = f14 - fDp3;
                float f16 = f11 + fDp3;
                float f17 = f14 + fDp3;
                i = iCenterY;
                f3 = f12;
                f4 = 0.0f;
                f5 = 1.0f;
                canvas2 = canvas;
                canvas2.saveLayerAlpha(f13, f15, f16, f17, 255, 31);
            } else {
                f3 = f;
                i = iCenterY;
                f4 = 0.0f;
                f5 = 1.0f;
                canvas2 = canvas;
            }
            i2 = this.backgroundColorKey;
            if (i2 >= 0) {
                if (this.drawUnchecked) {
                    if (this.backgroundColor != 0) {
                        this.backgroundPaint.setColor(0);
                    } else {
                        Paint paint2 = this.backgroundPaint;
                        i3 = this.background2ColorKey;
                        if (i3 < 0) {
                            i3 = this.checkColorKey;
                        }
                        c = 0;
                        paint2.setColor(AndroidUtilities.getOffsetColor(16777215, getThemedColor(i3), f3, this.backgroundAlpha));
                    }
                    if (this.drawUnchecked || (i18 = this.backgroundType) < 0 || i18 == 12 || i18 == 13) {
                        i4 = 8;
                        i5 = 10;
                        i6 = -1;
                        f6 = 1.5f;
                    } else if (i18 != 8 && i18 != 10 && i18 != 14) {
                        if (i18 == 6 || i18 == 7) {
                            float f18 = iCenterX;
                            float f19 = i;
                            canvas2.drawCircle(f18, f19, fDp3 - AndroidUtilities.dp(f5), paint);
                            canvas2.drawCircle(f18, f19, fDp3 - AndroidUtilities.dp(1.5f), this.backgroundPaint);
                        } else {
                            canvas2.drawCircle(iCenterX, i, fDp3, paint);
                        }
                        i4 = 8;
                        i5 = 10;
                        i6 = -1;
                        f6 = 1.5f;
                    } else if (this.customRadius > f4) {
                        float fDp4 = fDp3 - AndroidUtilities.dp(1.5f);
                        float fLerp = AndroidUtilities.lerp(fDp4, this.customRadius, this.customRadiusFactor);
                        float f20 = iCenterX;
                        float f21 = i;
                        i4 = 8;
                        i5 = 10;
                        i6 = -1;
                        f6 = 1.5f;
                        canvas2.drawRoundRect(f20 - fDp4, f21 - fDp4, f20 + fDp4, f21 + fDp4, fLerp, fLerp, this.backgroundPaint);
                    } else {
                        i4 = 8;
                        i5 = 10;
                        i6 = -1;
                        f6 = 1.5f;
                        canvas2.drawCircle(iCenterX, i, fDp3 - AndroidUtilities.dp(1.5f), this.backgroundPaint);
                    }
                    paint.setColor(getThemedColor(this.checkColorKey));
                    i7 = this.backgroundType;
                    if (i7 == i6 && i7 != 7 && i7 != i4 && i7 != 9 && i7 != i5 && i7 != 14) {
                        if (i7 == 12 || i7 == 13) {
                            i8 = 6;
                            this.backgroundPaint.setStyle(Paint.Style.FILL);
                            MessageDrawable messageDrawable = this.messageDrawable;
                            if (messageDrawable == null || !messageDrawable.hasGradient()) {
                                this.backgroundPaint.setShader(null);
                            } else {
                                Shader gradientShader = this.messageDrawable.getGradientShader();
                                Matrix matrix = this.messageDrawable.getMatrix();
                                matrix.reset();
                                this.messageDrawable.applyMatrixScale();
                                matrix.postTranslate(0.0f, (-this.messageDrawable.getTopY()) + this.bounds.top);
                                gradientShader.setLocalMatrix(matrix);
                                this.backgroundPaint.setShader(gradientShader);
                            }
                            canvas2.drawCircle(iCenterX, i, (fDp3 - AndroidUtilities.dp(f5)) * this.backgroundAlpha, this.backgroundPaint);
                            this.backgroundPaint.setStyle(Paint.Style.STROKE);
                        } else if (i7 == 0 || i7 == 11) {
                            i8 = 6;
                            canvas2.drawCircle(iCenterX, i, fDp3, this.backgroundPaint);
                        } else {
                            float f22 = iCenterX;
                            float f23 = i;
                            this.rect.set(f22 - fDp, f23 - fDp, f22 + fDp, f23 + fDp);
                            int i22 = this.backgroundType;
                            i8 = 6;
                            if (i22 == 6) {
                                i14 = (int) ((-360.0f) * f3);
                                i15 = 0;
                            } else if (i22 == 1) {
                                i14 = (int) ((-270.0f) * f3);
                                i15 = -90;
                            } else {
                                i14 = (int) (270.0f * f3);
                                if (LocaleController.isRTL) {
                                    i14 = -i14;
                                }
                                i15 = 90;
                            }
                            if (i22 == 6) {
                                int themedColor = getThemedColor(this.strokeBackgroundKey);
                                int iAlpha = Color.alpha(themedColor);
                                this.backgroundPaint.setColor(themedColor);
                                this.backgroundPaint.setAlpha((int) (iAlpha * f3));
                                i17 = i15;
                                i16 = i14;
                                canvas2.drawArc(this.rect, i15, i14, false, this.backgroundPaint);
                                int themedColor2 = getThemedColor(Theme.key_chat_attachPhotoBackground);
                                int iAlpha2 = Color.alpha(themedColor2);
                                this.backgroundPaint.setColor(themedColor2);
                                this.backgroundPaint.setAlpha((int) (iAlpha2 * f3));
                            } else {
                                i16 = i14;
                                i17 = i15;
                            }
                            canvas2 = canvas;
                            canvas2.drawArc(this.rect, i17, i16, false, this.backgroundPaint);
                        }
                        if (f2 > f4) {
                            if (f3 < 0.5f) {
                                f7 = 0.0f;
                            } else {
                                f7 = (f3 - 0.5f) / 0.5f;
                            }
                            i9 = this.backgroundType;
                            if (i9 == 9) {
                                paint.setColor(getThemedColor(this.background2ColorKey));
                            } else if (i9 != 11 || i9 == i8 || i9 == 7 || i9 == 10 || ((!this.drawUnchecked && this.backgroundColorKey >= 0) || i9 == 14)) {
                                paint.setColor(getThemedColor(this.backgroundColorKey));
                            } else {
                                int i23 = this.backgroundColor;
                                if (i23 != 0) {
                                    paint.setColor(i23);
                                } else {
                                    paint.setColor(getThemedColor(this.enabled ? Theme.key_checkbox : Theme.key_checkboxDisabled));
                                }
                            }
                            if (this.forbidden) {
                                paint.setColor(this.backgroundPaint.getColor());
                            } else if (this.alpha < f5) {
                                paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                            }
                            if (!this.useDefaultCheck || (i13 = this.checkColorKey) < 0) {
                                this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                            } else {
                                this.checkPaint.setColor(getThemedColor(i13));
                            }
                            if (this.alpha < f5 && Theme.currentTheme.isDark()) {
                                this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                            }
                            if (this.backgroundType != -1) {
                                fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                                int iSave = canvas2.save();
                                canvas2.translate(iCenterX - fDp2, i - fDp2);
                                if (f2 < f5) {
                                    z3 = true;
                                } else {
                                    z3 = false;
                                }
                                if (z3) {
                                    canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                                }
                                paintProvide = this.circlePaintProvider.provide(null);
                                i12 = this.backgroundType;
                                if (i12 != 12 || i12 == 13) {
                                    canvas2 = canvas;
                                    alpha = paintProvide.getAlpha();
                                    paintProvide.setAlpha((int) (f2 * 255.0f));
                                    canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                                    if (paintProvide != paint) {
                                        paintProvide.setAlpha(alpha);
                                    }
                                } else if (this.customRadius > 0.0f) {
                                    float fDp5 = fDp3 - AndroidUtilities.dp(0.5f);
                                    float fLerp2 = AndroidUtilities.lerp(fDp5, this.customRadius, this.customRadiusFactor);
                                    float f24 = fDp2 - fDp5;
                                    float f25 = fDp2 + fDp5;
                                    canvas.drawRoundRect(f24, f24, f25, f25, fLerp2, fLerp2, paintProvide);
                                    float f26 = (f5 - f2) * fDp5;
                                    float fLerp3 = AndroidUtilities.lerp(f26, this.customRadius, this.customRadiusFactor);
                                    if (!z3 || f26 <= 0.0f) {
                                        canvas2 = canvas;
                                    } else {
                                        float f27 = fDp2 - f26;
                                        float f28 = fDp2 + f26;
                                        canvas2 = canvas;
                                        canvas2.drawRoundRect(f27, f27, f28, f28, fLerp3, fLerp3, Theme.PAINT_CLEAR);
                                    }
                                } else {
                                    canvas2 = canvas;
                                    float fDp6 = fDp3 - AndroidUtilities.dp(0.5f);
                                    canvas2.drawCircle(fDp2, fDp2, fDp6, paintProvide);
                                    float f29 = (f5 - f2) * fDp6;
                                    if (z3 && f29 > 0.0f) {
                                        canvas2.drawCircle(fDp2, fDp2, f29, Theme.PAINT_CLEAR);
                                    }
                                }
                                canvas2.restoreToCount(iSave);
                            }
                            if (this.forbidden) {
                                if (forbidPaint == null) {
                                    Paint paint3 = new Paint(1);
                                    forbidPaint = paint3;
                                    paint3.setStyle(Paint.Style.STROKE);
                                    forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                                    forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                                    Paint paint4 = forbidPaint;
                                    float fDp7 = AndroidUtilities.dp(0.66f);
                                    float fDp8 = AndroidUtilities.dp(4.0f);
                                    float[] fArr = new float[2];
                                    fArr[c] = fDp7;
                                    fArr[1] = fDp8;
                                    paint4.setPathEffect(new DashPathEffect(fArr, 0.0f));
                                }
                                forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                                forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                                canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                            } else if (f7 != 0.0f) {
                                if (this.checkedText != null) {
                                    if (this.textPaint == null) {
                                        i11 = 1;
                                        TextPaint textPaint = new TextPaint(1);
                                        this.textPaint = textPaint;
                                        textPaint.setTypeface(AndroidUtilities.bold());
                                    } else {
                                        i11 = 1;
                                    }
                                    length = this.checkedText.length();
                                    if (length != 0 || length == i11 || length == 2) {
                                        f9 = 14.0f;
                                        f10 = 18.0f;
                                    } else if (length != 3) {
                                        f9 = 8.0f;
                                        f10 = 15.75f;
                                    } else {
                                        f10 = 16.5f;
                                        f9 = 10.0f;
                                    }
                                    this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                                    this.textPaint.setColor(getThemedColor(this.checkColorKey));
                                    canvas2.save();
                                    float f30 = iCenterX;
                                    canvas2.scale(f7, 1.0f, f30, i);
                                    String str = this.checkedText;
                                    canvas2.drawText(str, f30 - (this.textPaint.measureText(str) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                                    canvas2.restore();
                                } else {
                                    this.path.reset();
                                    i10 = this.backgroundType;
                                    if (i10 == -1) {
                                        f8 = 1.4f;
                                    } else if (i10 == 5) {
                                        f8 = 0.8f;
                                    } else {
                                        f8 = 1.0f;
                                    }
                                    float fDp9 = AndroidUtilities.dp(9.0f * f8) * f7;
                                    float fDp10 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                                    int iDp = iCenterX - AndroidUtilities.dp(f6);
                                    int iDp2 = AndroidUtilities.dp(4.0f) + i;
                                    float fSqrt = (float) Math.sqrt((fDp10 * fDp10) / 2.0f);
                                    float f31 = iDp;
                                    float f32 = iDp2;
                                    this.path.moveTo(f31 - fSqrt, f32 - fSqrt);
                                    this.path.lineTo(f31, f32);
                                    float fSqrt2 = (float) Math.sqrt((fDp9 * fDp9) / 2.0f);
                                    this.path.lineTo(f31 + fSqrt2, f32 - fSqrt2);
                                    if (z && this.checkScale == 1.0f) {
                                        z2 = false;
                                    } else {
                                        canvas2.save();
                                        float f33 = this.checkScale;
                                        canvas2.scale(f33, f33, iCenterX, i);
                                        z2 = true;
                                    }
                                    canvas2.drawPath(this.path, this.checkPaint);
                                    if (z2) {
                                        canvas2.restore();
                                    }
                                }
                            }
                        }
                        if (z) {
                            canvas2.restore();
                        }
                    }
                    i8 = 6;
                    f4 = 0.0f;
                    if (f2 > f4) {
                        if (f3 < 0.5f) {
                            f7 = 0.0f;
                        } else {
                            f7 = (f3 - 0.5f) / 0.5f;
                        }
                        i9 = this.backgroundType;
                        if (i9 == 9) {
                            paint.setColor(getThemedColor(this.background2ColorKey));
                        } else if (i9 != 11) {
                            paint.setColor(getThemedColor(this.backgroundColorKey));
                        } else {
                            paint.setColor(getThemedColor(this.backgroundColorKey));
                        }
                        if (this.forbidden) {
                            paint.setColor(this.backgroundPaint.getColor());
                        } else if (this.alpha < f5) {
                            paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                        }
                        if (this.useDefaultCheck) {
                            this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                        } else {
                            this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                        }
                        if (this.alpha < f5) {
                            this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                        }
                        if (this.backgroundType != -1) {
                            fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                            int iSave2 = canvas2.save();
                            canvas2.translate(iCenterX - fDp2, i - fDp2);
                            if (f2 < f5) {
                                z3 = true;
                            } else {
                                z3 = false;
                            }
                            if (z3) {
                                canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                            }
                            paintProvide = this.circlePaintProvider.provide(null);
                            i12 = this.backgroundType;
                            if (i12 != 12) {
                                canvas2 = canvas;
                                alpha = paintProvide.getAlpha();
                                paintProvide.setAlpha((int) (f2 * 255.0f));
                                canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                                if (paintProvide != paint) {
                                    paintProvide.setAlpha(alpha);
                                }
                            } else {
                                canvas2 = canvas;
                                alpha = paintProvide.getAlpha();
                                paintProvide.setAlpha((int) (f2 * 255.0f));
                                canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                                if (paintProvide != paint) {
                                    paintProvide.setAlpha(alpha);
                                }
                            }
                            canvas2.restoreToCount(iSave2);
                        }
                        if (this.forbidden) {
                            if (forbidPaint == null) {
                                Paint paint5 = new Paint(1);
                                forbidPaint = paint5;
                                paint5.setStyle(Paint.Style.STROKE);
                                forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                                forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                                Paint paint6 = forbidPaint;
                                float fDp11 = AndroidUtilities.dp(0.66f);
                                float fDp12 = AndroidUtilities.dp(4.0f);
                                float[] fArr2 = new float[2];
                                fArr2[c] = fDp11;
                                fArr2[1] = fDp12;
                                paint6.setPathEffect(new DashPathEffect(fArr2, 0.0f));
                            }
                            forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                            forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                            canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                        } else if (f7 != 0.0f) {
                            if (this.checkedText != null) {
                                if (this.textPaint == null) {
                                    i11 = 1;
                                    TextPaint textPaint2 = new TextPaint(1);
                                    this.textPaint = textPaint2;
                                    textPaint2.setTypeface(AndroidUtilities.bold());
                                } else {
                                    i11 = 1;
                                }
                                length = this.checkedText.length();
                                if (length != 0) {
                                    f9 = 14.0f;
                                    f10 = 18.0f;
                                } else {
                                    f9 = 14.0f;
                                    f10 = 18.0f;
                                }
                                this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                                this.textPaint.setColor(getThemedColor(this.checkColorKey));
                                canvas2.save();
                                float f34 = iCenterX;
                                canvas2.scale(f7, 1.0f, f34, i);
                                String str2 = this.checkedText;
                                canvas2.drawText(str2, f34 - (this.textPaint.measureText(str2) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                                canvas2.restore();
                            } else {
                                this.path.reset();
                                i10 = this.backgroundType;
                                if (i10 == -1) {
                                    f8 = 1.4f;
                                } else if (i10 == 5) {
                                    f8 = 0.8f;
                                } else {
                                    f8 = 1.0f;
                                }
                                float fDp13 = AndroidUtilities.dp(9.0f * f8) * f7;
                                float fDp14 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                                int iDp3 = iCenterX - AndroidUtilities.dp(f6);
                                int iDp4 = AndroidUtilities.dp(4.0f) + i;
                                float fSqrt3 = (float) Math.sqrt((fDp14 * fDp14) / 2.0f);
                                float f35 = iDp3;
                                float f36 = iDp4;
                                this.path.moveTo(f35 - fSqrt3, f36 - fSqrt3);
                                this.path.lineTo(f35, f36);
                                float fSqrt4 = (float) Math.sqrt((fDp13 * fDp13) / 2.0f);
                                this.path.lineTo(f35 + fSqrt4, f36 - fSqrt4);
                                if (z) {
                                    canvas2.save();
                                    float f37 = this.checkScale;
                                    canvas2.scale(f37, f37, iCenterX, i);
                                    z2 = true;
                                } else {
                                    canvas2.save();
                                    float f38 = this.checkScale;
                                    canvas2.scale(f38, f38, iCenterX, i);
                                    z2 = true;
                                }
                                canvas2.drawPath(this.path, this.checkPaint);
                                if (z2) {
                                    canvas2.restore();
                                }
                            }
                        }
                    }
                    if (z) {
                        canvas2.restore();
                    }
                }
                paint.setColor(Color.argb((int) (this.backgroundAlpha * 25.0f), 0, 0, 0));
                if (this.backgroundType == 8) {
                    this.backgroundPaint.setColor(getThemedColor(this.background2ColorKey));
                } else {
                    this.backgroundPaint.setColor(AndroidUtilities.getOffsetColor(-1, getThemedColor(this.checkColorKey), f3, this.backgroundAlpha));
                }
                c = 0;
                if (this.drawUnchecked) {
                    i4 = 8;
                    i5 = 10;
                    i6 = -1;
                    f6 = 1.5f;
                } else {
                    i4 = 8;
                    i5 = 10;
                    i6 = -1;
                    f6 = 1.5f;
                }
                paint.setColor(getThemedColor(this.checkColorKey));
                i7 = this.backgroundType;
                if (i7 == i6) {
                    i8 = 6;
                    f4 = 0.0f;
                } else {
                    i8 = 6;
                    f4 = 0.0f;
                }
                if (f2 > f4) {
                    if (f3 < 0.5f) {
                        f7 = 0.0f;
                    } else {
                        f7 = (f3 - 0.5f) / 0.5f;
                    }
                    i9 = this.backgroundType;
                    if (i9 == 9) {
                        paint.setColor(getThemedColor(this.background2ColorKey));
                    } else if (i9 != 11) {
                        paint.setColor(getThemedColor(this.backgroundColorKey));
                    } else {
                        paint.setColor(getThemedColor(this.backgroundColorKey));
                    }
                    if (this.forbidden) {
                        paint.setColor(this.backgroundPaint.getColor());
                    } else if (this.alpha < f5) {
                        paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                    }
                    if (this.useDefaultCheck) {
                        this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                    } else {
                        this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                    }
                    if (this.alpha < f5) {
                        this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                    }
                    if (this.backgroundType != -1) {
                        fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                        int iSave3 = canvas2.save();
                        canvas2.translate(iCenterX - fDp2, i - fDp2);
                        if (f2 < f5) {
                            z3 = true;
                        } else {
                            z3 = false;
                        }
                        if (z3) {
                            canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                        }
                        paintProvide = this.circlePaintProvider.provide(null);
                        i12 = this.backgroundType;
                        if (i12 != 12) {
                            canvas2 = canvas;
                            alpha = paintProvide.getAlpha();
                            paintProvide.setAlpha((int) (f2 * 255.0f));
                            canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                            if (paintProvide != paint) {
                                paintProvide.setAlpha(alpha);
                            }
                        } else {
                            canvas2 = canvas;
                            alpha = paintProvide.getAlpha();
                            paintProvide.setAlpha((int) (f2 * 255.0f));
                            canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                            if (paintProvide != paint) {
                                paintProvide.setAlpha(alpha);
                            }
                        }
                        canvas2.restoreToCount(iSave3);
                    }
                    if (this.forbidden) {
                        if (forbidPaint == null) {
                            Paint paint7 = new Paint(1);
                            forbidPaint = paint7;
                            paint7.setStyle(Paint.Style.STROKE);
                            forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                            forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                            Paint paint8 = forbidPaint;
                            float fDp15 = AndroidUtilities.dp(0.66f);
                            float fDp16 = AndroidUtilities.dp(4.0f);
                            float[] fArr3 = new float[2];
                            fArr3[c] = fDp15;
                            fArr3[1] = fDp16;
                            paint8.setPathEffect(new DashPathEffect(fArr3, 0.0f));
                        }
                        forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                        forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                        canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                    } else if (f7 != 0.0f) {
                        if (this.checkedText != null) {
                            if (this.textPaint == null) {
                                i11 = 1;
                                TextPaint textPaint3 = new TextPaint(1);
                                this.textPaint = textPaint3;
                                textPaint3.setTypeface(AndroidUtilities.bold());
                            } else {
                                i11 = 1;
                            }
                            length = this.checkedText.length();
                            if (length != 0) {
                                f9 = 14.0f;
                                f10 = 18.0f;
                            } else {
                                f9 = 14.0f;
                                f10 = 18.0f;
                            }
                            this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                            this.textPaint.setColor(getThemedColor(this.checkColorKey));
                            canvas2.save();
                            float f39 = iCenterX;
                            canvas2.scale(f7, 1.0f, f39, i);
                            String str3 = this.checkedText;
                            canvas2.drawText(str3, f39 - (this.textPaint.measureText(str3) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                            canvas2.restore();
                        } else {
                            this.path.reset();
                            i10 = this.backgroundType;
                            if (i10 == -1) {
                                f8 = 1.4f;
                            } else if (i10 == 5) {
                                f8 = 0.8f;
                            } else {
                                f8 = 1.0f;
                            }
                            float fDp17 = AndroidUtilities.dp(9.0f * f8) * f7;
                            float fDp18 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                            int iDp5 = iCenterX - AndroidUtilities.dp(f6);
                            int iDp6 = AndroidUtilities.dp(4.0f) + i;
                            float fSqrt5 = (float) Math.sqrt((fDp18 * fDp18) / 2.0f);
                            float f310 = iDp5;
                            float f311 = iDp6;
                            this.path.moveTo(f310 - fSqrt5, f311 - fSqrt5);
                            this.path.lineTo(f310, f311);
                            float fSqrt6 = (float) Math.sqrt((fDp17 * fDp17) / 2.0f);
                            this.path.lineTo(f310 + fSqrt6, f311 - fSqrt6);
                            if (z) {
                                canvas2.save();
                                float f312 = this.checkScale;
                                canvas2.scale(f312, f312, iCenterX, i);
                                z2 = true;
                            } else {
                                canvas2.save();
                                float f313 = this.checkScale;
                                canvas2.scale(f313, f313, iCenterX, i);
                                z2 = true;
                            }
                            canvas2.drawPath(this.path, this.checkPaint);
                            if (z2) {
                                canvas2.restore();
                            }
                        }
                    }
                }
                if (z) {
                    canvas2.restore();
                }
            }
            if (this.drawUnchecked) {
                i20 = this.backgroundType;
                if (i20 != 12 || i20 == 13) {
                    paint.setColor(getThemedColor(i2));
                    paint.setAlpha((int) (this.backgroundAlpha * 255.0f));
                    this.backgroundPaint.setColor(getThemedColor(this.checkColorKey));
                } else if (i20 == 6 || i20 == 7) {
                    paint.setColor(getThemedColor(this.background2ColorKey));
                    this.backgroundPaint.setColor(getThemedColor(this.checkColorKey));
                } else if (i20 == 10 || i20 == 14) {
                    this.backgroundPaint.setColor(getThemedColor(this.background2ColorKey));
                } else {
                    Paint paint9 = paint;
                    int iIndexOfKey = Theme.currentColors.indexOfKey(Theme.key_chat_serviceBackground);
                    paint9.setColor(((iIndexOfKey >= 0 ? Theme.currentColors.valueAt(iIndexOfKey) : Theme.serviceMessageColor) & 16777215) | 671088640);
                    this.backgroundPaint.setColor(getThemedColor(this.checkColorKey));
                }
            } else {
                Paint paint10 = this.backgroundPaint;
                i19 = this.background2ColorKey;
                if (i19 < 0) {
                    i19 = this.checkColorKey;
                }
                paint10.setColor(AndroidUtilities.getOffsetColor(16777215, getThemedColor(i19), f3, this.backgroundAlpha));
            }
            c = 0;
            if (this.drawUnchecked) {
                i4 = 8;
                i5 = 10;
                i6 = -1;
                f6 = 1.5f;
            } else {
                i4 = 8;
                i5 = 10;
                i6 = -1;
                f6 = 1.5f;
            }
            paint.setColor(getThemedColor(this.checkColorKey));
            i7 = this.backgroundType;
            if (i7 == i6) {
                i8 = 6;
                f4 = 0.0f;
            } else {
                i8 = 6;
                f4 = 0.0f;
            }
            if (f2 > f4) {
                if (f3 < 0.5f) {
                    f7 = 0.0f;
                } else {
                    f7 = (f3 - 0.5f) / 0.5f;
                }
                i9 = this.backgroundType;
                if (i9 == 9) {
                    paint.setColor(getThemedColor(this.background2ColorKey));
                } else if (i9 != 11) {
                    paint.setColor(getThemedColor(this.backgroundColorKey));
                } else {
                    paint.setColor(getThemedColor(this.backgroundColorKey));
                }
                if (this.forbidden) {
                    paint.setColor(this.backgroundPaint.getColor());
                } else if (this.alpha < f5) {
                    paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                }
                if (this.useDefaultCheck) {
                    this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                } else {
                    this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                }
                if (this.alpha < f5) {
                    this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                }
                if (this.backgroundType != -1) {
                    fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                    int iSave4 = canvas2.save();
                    canvas2.translate(iCenterX - fDp2, i - fDp2);
                    if (f2 < f5) {
                        z3 = true;
                    } else {
                        z3 = false;
                    }
                    if (z3) {
                        canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                    }
                    paintProvide = this.circlePaintProvider.provide(null);
                    i12 = this.backgroundType;
                    if (i12 != 12) {
                        canvas2 = canvas;
                        alpha = paintProvide.getAlpha();
                        paintProvide.setAlpha((int) (f2 * 255.0f));
                        canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                        if (paintProvide != paint) {
                            paintProvide.setAlpha(alpha);
                        }
                    } else {
                        canvas2 = canvas;
                        alpha = paintProvide.getAlpha();
                        paintProvide.setAlpha((int) (f2 * 255.0f));
                        canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                        if (paintProvide != paint) {
                            paintProvide.setAlpha(alpha);
                        }
                    }
                    canvas2.restoreToCount(iSave4);
                }
                if (this.forbidden) {
                    if (forbidPaint == null) {
                        Paint paint11 = new Paint(1);
                        forbidPaint = paint11;
                        paint11.setStyle(Paint.Style.STROKE);
                        forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                        forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                        Paint paint12 = forbidPaint;
                        float fDp19 = AndroidUtilities.dp(0.66f);
                        float fDp110 = AndroidUtilities.dp(4.0f);
                        float[] fArr4 = new float[2];
                        fArr4[c] = fDp19;
                        fArr4[1] = fDp110;
                        paint12.setPathEffect(new DashPathEffect(fArr4, 0.0f));
                    }
                    forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                    forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                    canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                } else if (f7 != 0.0f) {
                    if (this.checkedText != null) {
                        if (this.textPaint == null) {
                            i11 = 1;
                            TextPaint textPaint4 = new TextPaint(1);
                            this.textPaint = textPaint4;
                            textPaint4.setTypeface(AndroidUtilities.bold());
                        } else {
                            i11 = 1;
                        }
                        length = this.checkedText.length();
                        if (length != 0) {
                            f9 = 14.0f;
                            f10 = 18.0f;
                        } else {
                            f9 = 14.0f;
                            f10 = 18.0f;
                        }
                        this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                        this.textPaint.setColor(getThemedColor(this.checkColorKey));
                        canvas2.save();
                        float f314 = iCenterX;
                        canvas2.scale(f7, 1.0f, f314, i);
                        String str4 = this.checkedText;
                        canvas2.drawText(str4, f314 - (this.textPaint.measureText(str4) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                        canvas2.restore();
                    } else {
                        this.path.reset();
                        i10 = this.backgroundType;
                        if (i10 == -1) {
                            f8 = 1.4f;
                        } else if (i10 == 5) {
                            f8 = 0.8f;
                        } else {
                            f8 = 1.0f;
                        }
                        float fDp111 = AndroidUtilities.dp(9.0f * f8) * f7;
                        float fDp112 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                        int iDp7 = iCenterX - AndroidUtilities.dp(f6);
                        int iDp8 = AndroidUtilities.dp(4.0f) + i;
                        float fSqrt7 = (float) Math.sqrt((fDp112 * fDp112) / 2.0f);
                        float f315 = iDp7;
                        float f316 = iDp8;
                        this.path.moveTo(f315 - fSqrt7, f316 - fSqrt7);
                        this.path.lineTo(f315, f316);
                        float fSqrt8 = (float) Math.sqrt((fDp111 * fDp111) / 2.0f);
                        this.path.lineTo(f315 + fSqrt8, f316 - fSqrt8);
                        if (z) {
                            canvas2.save();
                            float f317 = this.checkScale;
                            canvas2.scale(f317, f317, iCenterX, i);
                            z2 = true;
                        } else {
                            canvas2.save();
                            float f318 = this.checkScale;
                            canvas2.scale(f318, f318, iCenterX, i);
                            z2 = true;
                        }
                        canvas2.drawPath(this.path, this.checkPaint);
                        if (z2) {
                            canvas2.restore();
                        }
                    }
                }
            }
            if (z) {
                canvas2.restore();
            }
        }
        fDp3 = AndroidUtilities.dp(10.0f);
        fDp = fDp3;
        if (this.forbidden) {
            f = 1.0f;
        } else {
            f = this.progress;
        }
        if (f >= 0.5f) {
            f2 = 1.0f;
        } else {
            f2 = f / 0.5f;
        }
        iCenterX = this.bounds.centerX();
        iCenterY = this.bounds.centerY();
        if (this.cutCheck) {
            z = false;
        } else {
            z = false;
        }
        if (z) {
            float f110 = iCenterX;
            float f111 = f;
            float f112 = f110 - fDp3;
            float f113 = iCenterY;
            float f114 = f113 - fDp3;
            float f115 = f110 + fDp3;
            float f116 = f113 + fDp3;
            i = iCenterY;
            f3 = f111;
            f4 = 0.0f;
            f5 = 1.0f;
            canvas2 = canvas;
            canvas2.saveLayerAlpha(f112, f114, f115, f116, 255, 31);
        } else {
            f3 = f;
            i = iCenterY;
            f4 = 0.0f;
            f5 = 1.0f;
            canvas2 = canvas;
        }
        i2 = this.backgroundColorKey;
        if (i2 >= 0) {
            if (this.drawUnchecked) {
                if (this.backgroundColor != 0) {
                    this.backgroundPaint.setColor(0);
                } else {
                    Paint paint13 = this.backgroundPaint;
                    i3 = this.background2ColorKey;
                    if (i3 < 0) {
                        i3 = this.checkColorKey;
                    }
                    c = 0;
                    paint13.setColor(AndroidUtilities.getOffsetColor(16777215, getThemedColor(i3), f3, this.backgroundAlpha));
                }
                if (this.drawUnchecked) {
                    i4 = 8;
                    i5 = 10;
                    i6 = -1;
                    f6 = 1.5f;
                } else {
                    i4 = 8;
                    i5 = 10;
                    i6 = -1;
                    f6 = 1.5f;
                }
                paint.setColor(getThemedColor(this.checkColorKey));
                i7 = this.backgroundType;
                if (i7 == i6) {
                    i8 = 6;
                    f4 = 0.0f;
                } else {
                    i8 = 6;
                    f4 = 0.0f;
                }
                if (f2 > f4) {
                    if (f3 < 0.5f) {
                        f7 = 0.0f;
                    } else {
                        f7 = (f3 - 0.5f) / 0.5f;
                    }
                    i9 = this.backgroundType;
                    if (i9 == 9) {
                        paint.setColor(getThemedColor(this.background2ColorKey));
                    } else if (i9 != 11) {
                        paint.setColor(getThemedColor(this.backgroundColorKey));
                    } else {
                        paint.setColor(getThemedColor(this.backgroundColorKey));
                    }
                    if (this.forbidden) {
                        paint.setColor(this.backgroundPaint.getColor());
                    } else if (this.alpha < f5) {
                        paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                    }
                    if (this.useDefaultCheck) {
                        this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                    } else {
                        this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                    }
                    if (this.alpha < f5) {
                        this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                    }
                    if (this.backgroundType != -1) {
                        fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                        int iSave5 = canvas2.save();
                        canvas2.translate(iCenterX - fDp2, i - fDp2);
                        if (f2 < f5) {
                            z3 = true;
                        } else {
                            z3 = false;
                        }
                        if (z3) {
                            canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                        }
                        paintProvide = this.circlePaintProvider.provide(null);
                        i12 = this.backgroundType;
                        if (i12 != 12) {
                            canvas2 = canvas;
                            alpha = paintProvide.getAlpha();
                            paintProvide.setAlpha((int) (f2 * 255.0f));
                            canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                            if (paintProvide != paint) {
                                paintProvide.setAlpha(alpha);
                            }
                        } else {
                            canvas2 = canvas;
                            alpha = paintProvide.getAlpha();
                            paintProvide.setAlpha((int) (f2 * 255.0f));
                            canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                            if (paintProvide != paint) {
                                paintProvide.setAlpha(alpha);
                            }
                        }
                        canvas2.restoreToCount(iSave5);
                    }
                    if (this.forbidden) {
                        if (forbidPaint == null) {
                            Paint paint14 = new Paint(1);
                            forbidPaint = paint14;
                            paint14.setStyle(Paint.Style.STROKE);
                            forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                            forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                            Paint paint15 = forbidPaint;
                            float fDp113 = AndroidUtilities.dp(0.66f);
                            float fDp114 = AndroidUtilities.dp(4.0f);
                            float[] fArr5 = new float[2];
                            fArr5[c] = fDp113;
                            fArr5[1] = fDp114;
                            paint15.setPathEffect(new DashPathEffect(fArr5, 0.0f));
                        }
                        forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                        forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                        canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                    } else if (f7 != 0.0f) {
                        if (this.checkedText != null) {
                            if (this.textPaint == null) {
                                i11 = 1;
                                TextPaint textPaint5 = new TextPaint(1);
                                this.textPaint = textPaint5;
                                textPaint5.setTypeface(AndroidUtilities.bold());
                            } else {
                                i11 = 1;
                            }
                            length = this.checkedText.length();
                            if (length != 0) {
                                f9 = 14.0f;
                                f10 = 18.0f;
                            } else {
                                f9 = 14.0f;
                                f10 = 18.0f;
                            }
                            this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                            this.textPaint.setColor(getThemedColor(this.checkColorKey));
                            canvas2.save();
                            float f319 = iCenterX;
                            canvas2.scale(f7, 1.0f, f319, i);
                            String str5 = this.checkedText;
                            canvas2.drawText(str5, f319 - (this.textPaint.measureText(str5) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                            canvas2.restore();
                        } else {
                            this.path.reset();
                            i10 = this.backgroundType;
                            if (i10 == -1) {
                                f8 = 1.4f;
                            } else if (i10 == 5) {
                                f8 = 0.8f;
                            } else {
                                f8 = 1.0f;
                            }
                            float fDp115 = AndroidUtilities.dp(9.0f * f8) * f7;
                            float fDp116 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                            int iDp9 = iCenterX - AndroidUtilities.dp(f6);
                            int iDp10 = AndroidUtilities.dp(4.0f) + i;
                            float fSqrt9 = (float) Math.sqrt((fDp116 * fDp116) / 2.0f);
                            float f3110 = iDp9;
                            float f3111 = iDp10;
                            this.path.moveTo(f3110 - fSqrt9, f3111 - fSqrt9);
                            this.path.lineTo(f3110, f3111);
                            float fSqrt10 = (float) Math.sqrt((fDp115 * fDp115) / 2.0f);
                            this.path.lineTo(f3110 + fSqrt10, f3111 - fSqrt10);
                            if (z) {
                                canvas2.save();
                                float f3112 = this.checkScale;
                                canvas2.scale(f3112, f3112, iCenterX, i);
                                z2 = true;
                            } else {
                                canvas2.save();
                                float f3113 = this.checkScale;
                                canvas2.scale(f3113, f3113, iCenterX, i);
                                z2 = true;
                            }
                            canvas2.drawPath(this.path, this.checkPaint);
                            if (z2) {
                                canvas2.restore();
                            }
                        }
                    }
                }
                if (z) {
                    canvas2.restore();
                }
            }
            paint.setColor(Color.argb((int) (this.backgroundAlpha * 25.0f), 0, 0, 0));
            if (this.backgroundType == 8) {
                this.backgroundPaint.setColor(getThemedColor(this.background2ColorKey));
            } else {
                this.backgroundPaint.setColor(AndroidUtilities.getOffsetColor(-1, getThemedColor(this.checkColorKey), f3, this.backgroundAlpha));
            }
            c = 0;
            if (this.drawUnchecked) {
                i4 = 8;
                i5 = 10;
                i6 = -1;
                f6 = 1.5f;
            } else {
                i4 = 8;
                i5 = 10;
                i6 = -1;
                f6 = 1.5f;
            }
            paint.setColor(getThemedColor(this.checkColorKey));
            i7 = this.backgroundType;
            if (i7 == i6) {
                i8 = 6;
                f4 = 0.0f;
            } else {
                i8 = 6;
                f4 = 0.0f;
            }
            if (f2 > f4) {
                if (f3 < 0.5f) {
                    f7 = 0.0f;
                } else {
                    f7 = (f3 - 0.5f) / 0.5f;
                }
                i9 = this.backgroundType;
                if (i9 == 9) {
                    paint.setColor(getThemedColor(this.background2ColorKey));
                } else if (i9 != 11) {
                    paint.setColor(getThemedColor(this.backgroundColorKey));
                } else {
                    paint.setColor(getThemedColor(this.backgroundColorKey));
                }
                if (this.forbidden) {
                    paint.setColor(this.backgroundPaint.getColor());
                } else if (this.alpha < f5) {
                    paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
                }
                if (this.useDefaultCheck) {
                    this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                } else {
                    this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
                }
                if (this.alpha < f5) {
                    this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
                }
                if (this.backgroundType != -1) {
                    fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                    int iSave6 = canvas2.save();
                    canvas2.translate(iCenterX - fDp2, i - fDp2);
                    if (f2 < f5) {
                        z3 = true;
                    } else {
                        z3 = false;
                    }
                    if (z3) {
                        canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                    }
                    paintProvide = this.circlePaintProvider.provide(null);
                    i12 = this.backgroundType;
                    if (i12 != 12) {
                        canvas2 = canvas;
                        alpha = paintProvide.getAlpha();
                        paintProvide.setAlpha((int) (f2 * 255.0f));
                        canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                        if (paintProvide != paint) {
                            paintProvide.setAlpha(alpha);
                        }
                    } else {
                        canvas2 = canvas;
                        alpha = paintProvide.getAlpha();
                        paintProvide.setAlpha((int) (f2 * 255.0f));
                        canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                        if (paintProvide != paint) {
                            paintProvide.setAlpha(alpha);
                        }
                    }
                    canvas2.restoreToCount(iSave6);
                }
                if (this.forbidden) {
                    if (forbidPaint == null) {
                        Paint paint16 = new Paint(1);
                        forbidPaint = paint16;
                        paint16.setStyle(Paint.Style.STROKE);
                        forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                        forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                        Paint paint17 = forbidPaint;
                        float fDp117 = AndroidUtilities.dp(0.66f);
                        float fDp118 = AndroidUtilities.dp(4.0f);
                        float[] fArr6 = new float[2];
                        fArr6[c] = fDp117;
                        fArr6[1] = fDp118;
                        paint17.setPathEffect(new DashPathEffect(fArr6, 0.0f));
                    }
                    forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                    forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                    canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
                } else if (f7 != 0.0f) {
                    if (this.checkedText != null) {
                        if (this.textPaint == null) {
                            i11 = 1;
                            TextPaint textPaint6 = new TextPaint(1);
                            this.textPaint = textPaint6;
                            textPaint6.setTypeface(AndroidUtilities.bold());
                        } else {
                            i11 = 1;
                        }
                        length = this.checkedText.length();
                        if (length != 0) {
                            f9 = 14.0f;
                            f10 = 18.0f;
                        } else {
                            f9 = 14.0f;
                            f10 = 18.0f;
                        }
                        this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                        this.textPaint.setColor(getThemedColor(this.checkColorKey));
                        canvas2.save();
                        float f3114 = iCenterX;
                        canvas2.scale(f7, 1.0f, f3114, i);
                        String str6 = this.checkedText;
                        canvas2.drawText(str6, f3114 - (this.textPaint.measureText(str6) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                        canvas2.restore();
                    } else {
                        this.path.reset();
                        i10 = this.backgroundType;
                        if (i10 == -1) {
                            f8 = 1.4f;
                        } else if (i10 == 5) {
                            f8 = 0.8f;
                        } else {
                            f8 = 1.0f;
                        }
                        float fDp119 = AndroidUtilities.dp(9.0f * f8) * f7;
                        float fDp1110 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                        int iDp11 = iCenterX - AndroidUtilities.dp(f6);
                        int iDp12 = AndroidUtilities.dp(4.0f) + i;
                        float fSqrt11 = (float) Math.sqrt((fDp1110 * fDp1110) / 2.0f);
                        float f3115 = iDp11;
                        float f3116 = iDp12;
                        this.path.moveTo(f3115 - fSqrt11, f3116 - fSqrt11);
                        this.path.lineTo(f3115, f3116);
                        float fSqrt12 = (float) Math.sqrt((fDp119 * fDp119) / 2.0f);
                        this.path.lineTo(f3115 + fSqrt12, f3116 - fSqrt12);
                        if (z) {
                            canvas2.save();
                            float f3117 = this.checkScale;
                            canvas2.scale(f3117, f3117, iCenterX, i);
                            z2 = true;
                        } else {
                            canvas2.save();
                            float f3118 = this.checkScale;
                            canvas2.scale(f3118, f3118, iCenterX, i);
                            z2 = true;
                        }
                        canvas2.drawPath(this.path, this.checkPaint);
                        if (z2) {
                            canvas2.restore();
                        }
                    }
                }
            }
            if (z) {
                canvas2.restore();
            }
        }
        if (this.drawUnchecked) {
            i20 = this.backgroundType;
            if (i20 != 12) {
                paint.setColor(getThemedColor(i2));
                paint.setAlpha((int) (this.backgroundAlpha * 255.0f));
                this.backgroundPaint.setColor(getThemedColor(this.checkColorKey));
            } else {
                paint.setColor(getThemedColor(i2));
                paint.setAlpha((int) (this.backgroundAlpha * 255.0f));
                this.backgroundPaint.setColor(getThemedColor(this.checkColorKey));
            }
        } else {
            Paint paint18 = this.backgroundPaint;
            i19 = this.background2ColorKey;
            if (i19 < 0) {
                i19 = this.checkColorKey;
            }
            paint18.setColor(AndroidUtilities.getOffsetColor(16777215, getThemedColor(i19), f3, this.backgroundAlpha));
        }
        c = 0;
        if (this.drawUnchecked) {
            i4 = 8;
            i5 = 10;
            i6 = -1;
            f6 = 1.5f;
        } else {
            i4 = 8;
            i5 = 10;
            i6 = -1;
            f6 = 1.5f;
        }
        paint.setColor(getThemedColor(this.checkColorKey));
        i7 = this.backgroundType;
        if (i7 == i6) {
            i8 = 6;
            f4 = 0.0f;
        } else {
            i8 = 6;
            f4 = 0.0f;
        }
        if (f2 > f4) {
            if (f3 < 0.5f) {
                f7 = 0.0f;
            } else {
                f7 = (f3 - 0.5f) / 0.5f;
            }
            i9 = this.backgroundType;
            if (i9 == 9) {
                paint.setColor(getThemedColor(this.background2ColorKey));
            } else if (i9 != 11) {
                paint.setColor(getThemedColor(this.backgroundColorKey));
            } else {
                paint.setColor(getThemedColor(this.backgroundColorKey));
            }
            if (this.forbidden) {
                paint.setColor(this.backgroundPaint.getColor());
            } else if (this.alpha < f5) {
                paint.setColor(ColorUtils.blendARGB(this.alpha, this.backgroundPaint.getColor(), paint.getColor()));
            }
            if (this.useDefaultCheck) {
                this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
            } else {
                this.checkPaint.setColor(getThemedColor(Theme.key_checkboxCheck));
            }
            if (this.alpha < f5) {
                this.checkPaint.setColor(ColorUtils.blendARGB(this.alpha, paint.getColor(), this.checkPaint.getColor()));
            }
            if (this.backgroundType != -1) {
                fDp2 = AndroidUtilities.dp(this.size) / 2.0f;
                int iSave7 = canvas2.save();
                canvas2.translate(iCenterX - fDp2, i - fDp2);
                if (f2 < f5) {
                    z3 = true;
                } else {
                    z3 = false;
                }
                if (z3) {
                    canvas2.saveLayerAlpha(0.0f, 0.0f, AndroidUtilities.dp(this.size), AndroidUtilities.dp(this.size), 255, 31);
                }
                paintProvide = this.circlePaintProvider.provide(null);
                i12 = this.backgroundType;
                if (i12 != 12) {
                    canvas2 = canvas;
                    alpha = paintProvide.getAlpha();
                    paintProvide.setAlpha((int) (f2 * 255.0f));
                    canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                    if (paintProvide != paint) {
                        paintProvide.setAlpha(alpha);
                    }
                } else {
                    canvas2 = canvas;
                    alpha = paintProvide.getAlpha();
                    paintProvide.setAlpha((int) (f2 * 255.0f));
                    canvas2.drawCircle(fDp2, fDp2, fDp3 * f2, paintProvide);
                    if (paintProvide != paint) {
                        paintProvide.setAlpha(alpha);
                    }
                }
                canvas2.restoreToCount(iSave7);
            }
            if (this.forbidden) {
                if (forbidPaint == null) {
                    Paint paint19 = new Paint(1);
                    forbidPaint = paint19;
                    paint19.setStyle(Paint.Style.STROKE);
                    forbidPaint.setStrokeCap(Paint.Cap.ROUND);
                    forbidPaint.setStrokeJoin(Paint.Join.ROUND);
                    Paint paint110 = forbidPaint;
                    float fDp1111 = AndroidUtilities.dp(0.66f);
                    float fDp1112 = AndroidUtilities.dp(4.0f);
                    float[] fArr7 = new float[2];
                    fArr7[c] = fDp1111;
                    fArr7[1] = fDp1112;
                    paint110.setPathEffect(new DashPathEffect(fArr7, 0.0f));
                }
                forbidPaint.setStrokeWidth(AndroidUtilities.dp(1.66f));
                forbidPaint.setColor(getThemedColor(Theme.key_switchTrack));
                canvas2.drawCircle(iCenterX, i, AndroidUtilities.dp(9.0f), forbidPaint);
            } else if (f7 != 0.0f) {
                if (this.checkedText != null) {
                    if (this.textPaint == null) {
                        i11 = 1;
                        TextPaint textPaint7 = new TextPaint(1);
                        this.textPaint = textPaint7;
                        textPaint7.setTypeface(AndroidUtilities.bold());
                    } else {
                        i11 = 1;
                    }
                    length = this.checkedText.length();
                    if (length != 0) {
                        f9 = 14.0f;
                        f10 = 18.0f;
                    } else {
                        f9 = 14.0f;
                        f10 = 18.0f;
                    }
                    this.textPaint.setTextSize(AndroidUtilities.dp(f9));
                    this.textPaint.setColor(getThemedColor(this.checkColorKey));
                    canvas2.save();
                    float f3119 = iCenterX;
                    canvas2.scale(f7, 1.0f, f3119, i);
                    String str7 = this.checkedText;
                    canvas2.drawText(str7, f3119 - (this.textPaint.measureText(str7) / 2.0f), AndroidUtilities.dp(f10), this.textPaint);
                    canvas2.restore();
                } else {
                    this.path.reset();
                    i10 = this.backgroundType;
                    if (i10 == -1) {
                        f8 = 1.4f;
                    } else if (i10 == 5) {
                        f8 = 0.8f;
                    } else {
                        f8 = 1.0f;
                    }
                    float fDp1113 = AndroidUtilities.dp(9.0f * f8) * f7;
                    float fDp1114 = AndroidUtilities.dp(f8 * 4.0f) * f7;
                    int iDp13 = iCenterX - AndroidUtilities.dp(f6);
                    int iDp14 = AndroidUtilities.dp(4.0f) + i;
                    float fSqrt13 = (float) Math.sqrt((fDp1114 * fDp1114) / 2.0f);
                    float f31110 = iDp13;
                    float f31111 = iDp14;
                    this.path.moveTo(f31110 - fSqrt13, f31111 - fSqrt13);
                    this.path.lineTo(f31110, f31111);
                    float fSqrt14 = (float) Math.sqrt((fDp1113 * fDp1113) / 2.0f);
                    this.path.lineTo(f31110 + fSqrt14, f31111 - fSqrt14);
                    if (z) {
                        canvas2.save();
                        float f31112 = this.checkScale;
                        canvas2.scale(f31112, f31112, iCenterX, i);
                        z2 = true;
                    } else {
                        canvas2.save();
                        float f31113 = this.checkScale;
                        canvas2.scale(f31113, f31113, iCenterX, i);
                        z2 = true;
                    }
                    canvas2.drawPath(this.path, this.checkPaint);
                    if (z2) {
                        canvas2.restore();
                    }
                }
            }
        }
        if (z) {
            canvas2.restore();
        }
    }

    public boolean getDrawUnchecked() {
        return this.drawUnchecked;
    }

    public View getParentView() {
        return this.parentView;
    }

    public float getProgress() {
        return this.progress;
    }

    public boolean isChecked() {
        return this.isChecked;
    }

    public void onAttachedToWindow() {
        this.attachedToWindow = true;
    }

    public void onDetachedFromWindow() {
        this.attachedToWindow = false;
    }

    public void setAlpha(float f) {
        if (this.alpha == f) {
            return;
        }
        this.alpha = f;
        invalidate();
    }

    public void setBackgroundAlpha(float f) {
        if (this.backgroundAlpha == f) {
            return;
        }
        this.backgroundAlpha = f;
        invalidate();
    }

    public void setBackgroundColor(int i) {
        if (this.backgroundColor == i) {
            return;
        }
        this.backgroundColor = i;
        invalidate();
    }

    public void setBackgroundDrawable(MessageDrawable messageDrawable) {
        if (this.messageDrawable == messageDrawable) {
            return;
        }
        this.messageDrawable = messageDrawable;
        invalidate();
    }

    public void setBackgroundType(int i) {
        if (this.backgroundType == i) {
            return;
        }
        this.backgroundType = i;
        if (i == 12 || i == 13) {
            this.backgroundPaint.setStrokeWidth(AndroidUtilities.dp(1.0f));
        } else if (i == 4 || i == 5) {
            this.backgroundPaint.setStrokeWidth(AndroidUtilities.dp(1.9f));
            if (i == 5) {
                this.checkPaint.setStrokeWidth(AndroidUtilities.dp(1.5f));
            }
        } else if (i == 3) {
            this.backgroundPaint.setStrokeWidth(AndroidUtilities.dp(3.0f));
        } else if (i != 0) {
            this.backgroundPaint.setStrokeWidth(AndroidUtilities.dp(1.5f));
        }
        invalidate();
    }

    public void setBounds(int i, int i2, int i3, int i4) {
        int i5 = i3 + i;
        int i6 = i4 + i2;
        Rect rect = this.bounds;
        if (rect.left == i && rect.top == i2 && rect.right == i5 && rect.bottom == i6) {
            return;
        }
        rect.left = i;
        rect.top = i2;
        rect.right = i5;
        rect.bottom = i6;
        invalidate();
    }

    public void setChecked(boolean z, boolean z2) {
        setChecked(-1, z, z2);
    }

    public void setCirclePaintProvider(GenericProvider<Void, Paint> genericProvider) {
        if (this.circlePaintProvider == genericProvider) {
            return;
        }
        this.circlePaintProvider = genericProvider;
        invalidate();
    }

    public void setColor(int i, int i2, int i3) {
        if (this.backgroundColorKey == i && this.background2ColorKey == i2 && this.checkColorKey == i3) {
            return;
        }
        this.backgroundColorKey = i;
        this.background2ColorKey = i2;
        this.checkColorKey = i3;
        invalidate();
    }

    public void setCustomRadius(float f) {
        if (this.customRadius == f) {
            return;
        }
        this.customRadius = f;
        invalidate();
    }

    public void setCustomRadiusFactor(float f) {
        if (this.customRadiusFactor == f) {
            return;
        }
        this.customRadiusFactor = f;
        invalidate();
    }

    public void setCuttingCheck(boolean z) {
        if (this.cutCheck == z) {
            return;
        }
        this.cutCheck = z;
        this.checkPaint.setXfermode(z ? new PorterDuffXfermode(PorterDuff.Mode.CLEAR) : null);
        invalidate();
    }

    public void setDrawUnchecked(boolean z) {
        if (this.drawUnchecked == z) {
            return;
        }
        this.drawUnchecked = z;
        invalidate();
    }

    public void setEnabled(boolean z) {
        if (this.enabled == z) {
            return;
        }
        this.enabled = z;
        invalidate();
    }

    public void setForbidden(boolean z) {
        if (this.forbidden == z) {
            return;
        }
        this.forbidden = z;
        invalidate();
    }

    public void setNum(int i) {
        String str;
        if (i >= 0) {
            str = "" + (i + 1);
        } else {
            str = this.checkAnimator != null ? this.checkedText : null;
        }
        String str2 = this.checkedText;
        if (str2 == null) {
            if (str == null) {
                return;
            }
        } else if (str2.equals(str)) {
            return;
        }
        this.checkedText = str;
        invalidate();
    }

    public void setParentView(View view) {
        this.parentView = view;
    }

    public void setProgress(float f) {
        if (this.progress == f) {
            return;
        }
        this.progress = f;
        invalidate();
        ProgressDelegate progressDelegate = this.progressDelegate;
        if (progressDelegate != null) {
            progressDelegate.setProgress(f);
        }
    }

    public void setProgressDelegate(ProgressDelegate progressDelegate) {
        this.progressDelegate = progressDelegate;
    }

    public void setResourcesProvider(Theme.ResourcesProvider resourcesProvider) {
        if (this.resourcesProvider == resourcesProvider) {
            return;
        }
        this.resourcesProvider = resourcesProvider;
        invalidate();
    }

    public void setSize(float f) {
        if (this.size == f) {
            return;
        }
        this.size = f;
        invalidate();
    }

    public void setStrokeBackgroundColor(int i) {
        if (this.strokeBackgroundKey == i) {
            return;
        }
        this.strokeBackgroundKey = i;
        invalidate();
    }

    public void setStrokeBackgroundWidth(int i) {
        if (this.strokeBackgroundWidth == i) {
            return;
        }
        this.strokeBackgroundWidth = i;
        invalidate();
    }

    public void setUseDefaultCheck(boolean z) {
        if (this.useDefaultCheck == z) {
            return;
        }
        this.useDefaultCheck = z;
        invalidate();
    }

    public void setChecked(int i, boolean z, boolean z2) {
        if (i >= 0) {
            String str = "" + (i + 1);
            String str2 = this.checkedText;
            if (str2 == null || !str2.equals(str)) {
                this.checkedText = str;
                invalidate();
            }
        }
        if (z == this.isChecked) {
            return;
        }
        this.isChecked = z;
        if (this.attachedToWindow && z2) {
            animateToCheckedState(z);
        } else {
            cancelCheckAnimator();
            setProgress(z ? 1.0f : 0.0f);
        }
    }
}
