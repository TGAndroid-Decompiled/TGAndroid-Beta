package org.telegram.ui.Components;

import android.animation.AnimatorSet;
import android.animation.TimeInterpolator;
import android.animation.ValueAnimator;
import android.content.Context;
import android.graphics.Rect;
import android.transition.ChangeBounds;
import android.transition.Fade;
import android.transition.TransitionManager;
import android.transition.TransitionSet;
import android.util.SparseArray;
import android.view.MotionEvent;
import android.view.VelocityTracker;
import android.view.View;
import android.view.ViewConfiguration;
import android.view.ViewGroup;
import android.widget.FrameLayout;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.AnimationNotificationsLocker;
import org.telegram.messenger.Utilities;
public class n71 extends FrameLayout {
    public static final rq0 O = new rq0(1);
    public float A;
    public boolean B;
    public final int C;
    public boolean D;
    public boolean E;
    public final AnimationNotificationsLocker F;
    public final float G;
    public e71 H;
    public c71 I;
    public final f2.f0 J;
    public final Rect K;
    public boolean L;
    public ValueAnimator M;
    public float N;
    public final org.telegram.ui.ActionBar.b6 f31032a;
    public int f31033b;
    public float f31034c;
    public int d;
    public final View[] f31035e;
    public final int[] f31036f;
    public final SparseArray h;
    public int f31037n;
    public int f31038r;
    public int f31039s;
    public VelocityTracker v;
    public AnimatorSet f31040w;
    public boolean f31041x;
    public boolean f31042y;

    public n71(Context context, org.telegram.ui.ActionBar.b6 b6Var) {
        super(context);
        this.f31034c = 1.0f;
        this.h = new SparseArray();
        this.F = new AnimationNotificationsLocker();
        this.J = new f2.f0(this, 8);
        this.K = new Rect();
        this.L = true;
        this.f31032a = b6Var;
        this.G = AndroidUtilities.getPixelsInCM(0.3f, true);
        this.C = ViewConfiguration.get(context).getScaledMaximumFlingVelocity();
        this.f31036f = new int[2];
        this.f31035e = new View[2];
        setClipChildren(true);
    }

    public static wk0 p(View view) {
        if (view instanceof ViewGroup) {
            ViewGroup viewGroup = (ViewGroup) view;
            for (int i9 = 0; i9 < viewGroup.getChildCount(); i9++) {
                View childAt = viewGroup.getChildAt(i9);
                if (childAt instanceof wk0) {
                    return (wk0) childAt;
                }
                if (childAt instanceof ViewGroup) {
                    p(childAt);
                }
            }
            return null;
        }
        return null;
    }

    public final boolean A(android.view.MotionEvent r12) {
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.ui.Components.n71.A(android.view.MotionEvent):boolean");
    }

    public final boolean B(MotionEvent motionEvent, boolean z10) {
        int i9;
        if (!z10 && this.f31033b == 0) {
            this.N = 0.0f;
            return false;
        } else if ((z10 && this.f31033b == this.H.e() - 1) || this.M != null || !i(motionEvent) || ((z10 && !k(motionEvent)) || (!z10 && !j(motionEvent)))) {
            return false;
        } else {
            getParent().requestDisallowInterceptTouchEvent(true);
            this.E = false;
            this.D = true;
            v();
            this.f31038r = (int) (motionEvent.getX() + this.A);
            c71 c71Var = this.I;
            if (c71Var != null) {
                c71Var.setEnabled(false);
            }
            this.F.lock();
            this.f31042y = z10;
            int i10 = this.f31033b;
            if (z10) {
                i9 = 1;
            } else {
                i9 = -1;
            }
            this.d = i10 + i9;
            I(1);
            View[] viewArr = this.f31035e;
            View view = viewArr[1];
            if (view != null) {
                if (z10) {
                    E(view, viewArr[0].getMeasuredWidth());
                } else {
                    E(view, -viewArr[0].getMeasuredWidth());
                }
            }
            w(false);
            return true;
        }
    }

    public final void C(boolean z10) {
        int i9;
        int intValue;
        onTouchEvent(null);
        e71 e71Var = this.H;
        e71Var.getClass();
        if (!(e71Var instanceof org.telegram.ui.d7)) {
            z10 = false;
        }
        AnimatorSet animatorSet = this.f31040w;
        if (animatorSet != null) {
            animatorSet.cancel();
            this.f31040w = null;
        }
        View[] viewArr = this.f31035e;
        View view = viewArr[1];
        if (view != null) {
            removeView(view);
            viewArr[1] = null;
        }
        View view2 = viewArr[0];
        viewArr[1] = view2;
        if (view2 != null && view2.getTag() != null) {
            i9 = ((Integer) viewArr[1].getTag()).intValue();
        } else {
            i9 = 0;
        }
        if (this.H.e() == 0) {
            View view3 = viewArr[1];
            if (view3 != null) {
                removeView(view3);
                viewArr[1] = null;
            }
            View view4 = viewArr[0];
            if (view4 != null) {
                removeView(view4);
                viewArr[0] = null;
                return;
            }
            return;
        }
        if (this.f31033b > this.H.e() - 1) {
            this.f31033b = this.H.e() - 1;
        }
        if (this.f31033b < 0) {
            this.f31033b = 0;
        }
        int h = this.H.h(this.f31033b);
        int[] iArr = this.f31036f;
        iArr[0] = h;
        View d = this.H.d(h);
        viewArr[0] = d;
        this.H.b(d, this.f31033b, iArr[0]);
        addView(viewArr[0]);
        viewArr[0].setVisibility(0);
        if (viewArr[0].getTag() == null) {
            intValue = 0;
        } else {
            intValue = ((Integer) viewArr[0].getTag()).intValue();
        }
        if (intValue == i9) {
            z10 = false;
        }
        if (z10) {
            this.I.getClass();
        }
        o(z10);
        if (z10) {
            this.f31040w = new AnimatorSet();
            View view5 = viewArr[1];
            if (view5 != null) {
                E(view5, 0.0f);
            }
            View view6 = viewArr[0];
            if (view6 != null) {
                E(view6, -getMeasuredWidth());
            }
            View view7 = viewArr[1];
            if (view7 != null) {
                this.f31040w.playTogether(H(view7, getMeasuredWidth()));
            }
            View view8 = viewArr[0];
            if (view8 != null) {
                this.f31040w.playTogether(H(view8, 0.0f));
            }
            w(true);
            c71 c71Var = this.I;
            c71Var.f30736a = 0.0f;
            c71Var.v.f1();
            this.I.invalidate();
            ValueAnimator ofFloat = ValueAnimator.ofFloat(0.0f, 1.0f);
            ofFloat.addUpdateListener(new a71(this, 2));
            this.f31040w.playTogether(ofFloat);
            this.f31040w.setInterpolator(O);
            this.f31040w.setDuration(220L);
            this.f31040w.addListener(new b71(this, 2));
            this.I.setEnabled(false);
            this.f31041x = true;
            this.f31040w.start();
            return;
        }
        View view9 = viewArr[1];
        if (view9 != null) {
            removeView(view9);
            viewArr[1] = null;
        }
    }

    public final void D(int i9) {
        boolean z10;
        int i10;
        if (i9 != this.f31033b) {
            ValueAnimator valueAnimator = this.M;
            if (valueAnimator == null || this.d != i9) {
                if (valueAnimator != null) {
                    valueAnimator.cancel();
                    this.M = null;
                }
                if (this.f31033b < i9) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                this.f31042y = z10;
                this.d = i9;
                I(1);
                y(i9, z10);
                View[] viewArr = this.f31035e;
                View view = viewArr[0];
                if (view != null) {
                    i10 = view.getMeasuredWidth();
                } else {
                    i10 = 0;
                }
                if (z10) {
                    E(viewArr[1], i10);
                } else {
                    E(viewArr[1], -i10);
                }
                ValueAnimator ofFloat = ValueAnimator.ofFloat(0.0f, 1.0f);
                this.M = ofFloat;
                ofFloat.addUpdateListener(new a71(this, 0));
                this.M.addListener(new b71(this, 0));
                this.M.setDuration(getManualScrollDuration());
                this.M.setInterpolator(gr.h);
                this.M.start();
            }
        }
    }

    public void E(View view, float f10) {
        view.setTranslationX(f10);
    }

    public void F() {
        View[] viewArr = this.f31035e;
        View view = viewArr[0];
        View view2 = viewArr[1];
        viewArr[0] = view2;
        viewArr[1] = view;
        int i9 = this.f31033b;
        int i10 = this.d;
        this.f31033b = i10;
        this.d = i9;
        this.f31034c = 1.0f - this.f31034c;
        int[] iArr = this.f31036f;
        int i11 = iArr[0];
        iArr[0] = iArr[1];
        iArr[1] = i11;
        t(view2, view, i10, i9);
    }

    public int G() {
        return 16;
    }

    public final ValueAnimator H(View view, float f10) {
        ValueAnimator ofFloat = ValueAnimator.ofFloat(view.getTranslationX(), f10);
        ofFloat.addUpdateListener(new kw0(this, view, 1));
        ofFloat.addListener(new d71(this, view, f10));
        return ofFloat;
    }

    public final void I(int i9) {
        int i10;
        if (i9 == 0) {
            i10 = this.f31033b;
        } else {
            i10 = this.d;
        }
        if (i10 >= 0 && i10 < this.H.e()) {
            View[] viewArr = this.f31035e;
            View view = viewArr[i9];
            SparseArray sparseArray = this.h;
            int[] iArr = this.f31036f;
            if (view == null) {
                int h = this.H.h(i10);
                iArr[i9] = h;
                View view2 = (View) sparseArray.get(h);
                if (view2 == null) {
                    view2 = this.H.d(iArr[i9]);
                } else {
                    sparseArray.remove(iArr[i9]);
                }
                if (view2.getParent() != null) {
                    ((ViewGroup) view2.getParent()).removeView(view2);
                }
                addView(view2);
                view2.setTranslationX(getMeasuredWidth());
                viewArr[i9] = view2;
                this.H.b(view2, i10, iArr[i9]);
                viewArr[i9].setVisibility(0);
            } else if (iArr[i9] == this.H.h(i10)) {
                this.H.b(viewArr[i9], i10, iArr[i9]);
                viewArr[i9].setVisibility(0);
            } else {
                sparseArray.put(iArr[i9], viewArr[i9]);
                viewArr[i9].setVisibility(8);
                removeView(viewArr[i9]);
                int h10 = this.H.h(i10);
                iArr[i9] = h10;
                View view3 = (View) sparseArray.get(h10);
                if (view3 == null) {
                    view3 = this.H.d(iArr[i9]);
                } else {
                    sparseArray.remove(iArr[i9]);
                }
                addView(view3);
                viewArr[i9] = view3;
                view3.setVisibility(0);
                e71 e71Var = this.H;
                e71Var.b(viewArr[i9], i10, e71Var.h(i10));
            }
        }
    }

    @Override
    public final boolean canScrollHorizontally(int i9) {
        boolean z10;
        if (i9 != 0) {
            if (!this.f31041x && !this.D) {
                if (i9 > 0) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                if ((z10 || this.f31033b != 0) && (!z10 || this.f31033b != this.H.e() - 1)) {
                }
            }
            return true;
        }
        return false;
    }

    public float getAvailableTranslationX() {
        return AndroidUtilities.displaySize.x;
    }

    public int getCurrentPosition() {
        return this.f31033b;
    }

    public float getCurrentPositionAlpha() {
        View[] viewArr = this.f31035e;
        View view = viewArr[0];
        if (view == null || view.getVisibility() != 0) {
            return 0.0f;
        }
        return Utilities.clamp(1.0f - Math.abs(viewArr[0].getTranslationX() / getAvailableTranslationX()), 1.0f, 0.0f);
    }

    public View getCurrentView() {
        return this.f31035e[0];
    }

    public long getManualScrollDuration() {
        return 540L;
    }

    public int getNextPosition() {
        return this.d;
    }

    public float getNextPositionAlpha() {
        View[] viewArr = this.f31035e;
        View view = viewArr[1];
        if (view == null || view.getVisibility() != 0) {
            return 0.0f;
        }
        return Utilities.clamp(1.0f - Math.abs(viewArr[1].getTranslationX() / getAvailableTranslationX()), 1.0f, 0.0f);
    }

    public float getPositionAnimated() {
        float f10;
        View[] viewArr = this.f31035e;
        View view = viewArr[0];
        if (view != null && view.getVisibility() == 0) {
            f10 = (this.f31033b * Utilities.clamp(1.0f - Math.abs(viewArr[0].getTranslationX() / getAvailableTranslationX()), 1.0f, 0.0f)) + 0.0f;
        } else {
            f10 = 0.0f;
        }
        View view2 = viewArr[1];
        if (view2 != null && view2.getVisibility() == 0) {
            return (this.d * Utilities.clamp(1.0f - Math.abs(viewArr[1].getTranslationX() / getAvailableTranslationX()), 1.0f, 0.0f)) + f10;
        }
        return f10;
    }

    public View[] getViewPages() {
        return this.f31035e;
    }

    public boolean i(MotionEvent motionEvent) {
        return true;
    }

    public boolean j(MotionEvent motionEvent) {
        return true;
    }

    public boolean k(MotionEvent motionEvent) {
        return i(motionEvent);
    }

    public final void l() {
        boolean z10;
        float f10;
        View view;
        View view2;
        int measuredWidth;
        int measuredWidth2;
        float f11;
        boolean z11;
        boolean z12;
        VelocityTracker velocityTracker = this.v;
        if (velocityTracker != null) {
            velocityTracker.computeCurrentVelocity(1000, this.C);
        }
        if (this.D) {
            View[] viewArr = this.f31035e;
            float x10 = viewArr[0].getX();
            this.f31040w = new AnimatorSet();
            if (this.A != 0.0f) {
                if (Math.abs(0.0f) > 1500.0f) {
                    this.B = false;
                } else if (this.f31042y) {
                    View view3 = viewArr[1];
                    if (view3 != null) {
                        if (view3.getX() > (viewArr[0].getMeasuredWidth() >> 1)) {
                            z12 = true;
                        } else {
                            z12 = false;
                        }
                        this.B = z12;
                    } else {
                        this.B = false;
                    }
                } else {
                    if (viewArr[0].getX() < (viewArr[0].getMeasuredWidth() >> 1)) {
                        z11 = true;
                    } else {
                        z11 = false;
                    }
                    this.B = z11;
                }
            } else {
                if (Math.abs(x10) < viewArr[0].getMeasuredWidth() / 3.0f && (Math.abs(0.0f) < 3500.0f || Math.abs(0.0f) < Math.abs(0.0f))) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                this.B = z10;
            }
            if (this.B) {
                f10 = Math.abs(x10);
                if (this.f31042y) {
                    this.f31040w.playTogether(H(viewArr[0], 0.0f));
                    View view4 = viewArr[1];
                    if (view4 != null) {
                        this.f31040w.playTogether(H(view4, view4.getMeasuredWidth()));
                    }
                } else {
                    this.f31040w.playTogether(H(viewArr[0], 0.0f));
                    View view5 = viewArr[1];
                    if (view5 != null) {
                        this.f31040w.playTogether(H(view5, -view5.getMeasuredWidth()));
                    }
                }
            } else if (this.d >= 0) {
                f10 = viewArr[0].getMeasuredWidth() - Math.abs(x10);
                if (this.f31042y) {
                    this.f31040w.playTogether(H(viewArr[0], -view2.getMeasuredWidth()));
                    View view6 = viewArr[1];
                    if (view6 != null) {
                        this.f31040w.playTogether(H(view6, 0.0f));
                    }
                } else {
                    this.f31040w.playTogether(H(viewArr[0], view.getMeasuredWidth()));
                    View view7 = viewArr[1];
                    if (view7 != null) {
                        this.f31040w.playTogether(H(view7, 0.0f));
                    }
                }
            } else {
                f10 = 0.0f;
            }
            if (this.d < 0) {
                float f12 = this.N;
                if (this.B) {
                    f11 = 0.0f;
                } else {
                    f11 = 1.0f;
                }
                ValueAnimator ofFloat = ValueAnimator.ofFloat(f12, f11);
                ofFloat.addUpdateListener(new a71(this, 3));
                this.f31040w.playTogether(ofFloat);
            }
            ValueAnimator ofFloat2 = ValueAnimator.ofFloat(0.0f, 1.0f);
            ofFloat2.addUpdateListener(this.J);
            this.f31040w.playTogether(ofFloat2);
            this.f31040w.setInterpolator(O);
            float measuredWidth3 = getMeasuredWidth() / 2;
            float sin = (((float) Math.sin((Math.min(1.0f, (f10 * 1.0f) / measuredWidth) - 0.5f) * 0.47123894f)) * measuredWidth3) + measuredWidth3;
            float abs = Math.abs(0.0f);
            if (abs > 0.0f) {
                measuredWidth2 = Math.round(Math.abs(sin / abs) * 1000.0f) * 4;
            } else {
                measuredWidth2 = (int) (((f10 / getMeasuredWidth()) + 1.0f) * 100.0f);
            }
            this.f31040w.setDuration(Math.max(150, Math.min(measuredWidth2, 600)));
            this.f31040w.addListener(new b71(this, 3));
            this.f31040w.start();
            this.f31041x = true;
            this.D = false;
            w(false);
        } else {
            this.E = false;
            c71 c71Var = this.I;
            if (c71Var != null) {
                c71Var.setEnabled(true);
            }
        }
        VelocityTracker velocityTracker2 = this.v;
        if (velocityTracker2 != null) {
            velocityTracker2.recycle();
            this.v = null;
        }
    }

    public final boolean m() {
        boolean z10;
        if (!this.f31041x) {
            return false;
        }
        boolean z11 = this.B;
        int i9 = -1;
        View[] viewArr = this.f31035e;
        if (z11) {
            if (Math.abs(viewArr[0].getTranslationX()) < 1.0f) {
                E(viewArr[0], 0.0f);
                View view = viewArr[1];
                if (view != null) {
                    int measuredWidth = viewArr[0].getMeasuredWidth();
                    if (this.f31042y) {
                        i9 = 1;
                    }
                    E(view, measuredWidth * i9);
                }
                z10 = true;
            }
            z10 = false;
        } else {
            if (Math.abs(viewArr[1].getTranslationX()) < 1.0f) {
                View view2 = viewArr[0];
                int measuredWidth2 = view2.getMeasuredWidth();
                if (!this.f31042y) {
                    i9 = 1;
                }
                E(view2, measuredWidth2 * i9);
                View view3 = viewArr[1];
                if (view3 != null) {
                    E(view3, 0.0f);
                }
                z10 = true;
            }
            z10 = false;
        }
        w(true);
        if (z10) {
            AnimatorSet animatorSet = this.f31040w;
            if (animatorSet != null) {
                animatorSet.cancel();
                this.f31040w = null;
            }
            this.f31041x = false;
        }
        return this.f31041x;
    }

    public final m71 n(int i9, boolean z10) {
        c71 c71Var = new c71(this, getContext(), z10, i9, this.f31032a);
        this.I = c71Var;
        c71Var.f30755r = G();
        this.I.setDelegate(new n2.p(this, 11));
        o(false);
        return this.I;
    }

    public final void o(boolean z10) {
        c71 c71Var;
        if (this.H != null && (c71Var = this.I) != null) {
            c71Var.h.clear();
            c71Var.U.clear();
            c71Var.V.clear();
            c71Var.W.clear();
            c71Var.f30737a0.clear();
            c71Var.D = 0;
            for (int i9 = 0; i9 < this.H.e(); i9++) {
                this.H.getClass();
                this.I.a(this.H.f(i9), this.H.g(i9));
            }
            h();
            if (z10) {
                gh.f1 f1Var = this.I.v;
                TransitionSet transitionSet = new TransitionSet();
                ChangeBounds changeBounds = new ChangeBounds();
                changeBounds.setDuration(150L);
                transitionSet.addTransition(new Fade().setDuration(150L)).addTransition(changeBounds);
                transitionSet.setOrdering(0);
                transitionSet.setInterpolator((TimeInterpolator) gr.f28844f);
                TransitionManager.beginDelayedTransition(f1Var, transitionSet);
            }
            this.I.f30758x.l();
        }
    }

    @Override
    public final boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        c71 c71Var = this.I;
        if (c71Var != null && c71Var.F) {
            return false;
        }
        if (m()) {
            return true;
        }
        onTouchEvent(motionEvent);
        return this.D;
    }

    @Override
    public boolean onTouchEvent(MotionEvent motionEvent) {
        return A(motionEvent);
    }

    public final View q(ViewGroup viewGroup, float f10, float f11) {
        View q10;
        int childCount = viewGroup.getChildCount();
        for (int i9 = 0; i9 < childCount; i9++) {
            View childAt = viewGroup.getChildAt(i9);
            if (childAt.getVisibility() == 0) {
                Rect rect = this.K;
                childAt.getHitRect(rect);
                if (!rect.contains((int) f10, (int) f11)) {
                    continue;
                } else if (childAt.canScrollHorizontally(-1)) {
                    return childAt;
                } else {
                    if ((childAt instanceof ViewGroup) && (q10 = q((ViewGroup) childAt, f10 - rect.left, f11 - rect.top)) != null) {
                        return q10;
                    }
                }
            }
        }
        return null;
    }

    public final float r(int i9) {
        if (getMeasuredWidth() == 0) {
            return g7.n.b(1 - Math.abs(getCurrentPosition() - i9), 0, 1);
        }
        return g7.n.a(1.0f - Math.abs(getPositionAnimated() - i9), 0.0f, 1.0f);
    }

    @Override
    public final void requestDisallowInterceptTouchEvent(boolean z10) {
        if (this.L && this.E && !this.D) {
            onTouchEvent(null);
        }
        super.requestDisallowInterceptTouchEvent(z10);
    }

    public void setAdapter(e71 e71Var) {
        this.H = e71Var;
        int h = e71Var.h(this.f31033b);
        int[] iArr = this.f31036f;
        iArr[0] = h;
        View d = e71Var.d(h);
        View[] viewArr = this.f31035e;
        viewArr[0] = d;
        if (d == null && this.f31033b != 0) {
            this.f31033b = 0;
            int h10 = e71Var.h(0);
            iArr[0] = h10;
            viewArr[0] = e71Var.d(h10);
        }
        e71Var.b(viewArr[0], this.f31033b, iArr[0]);
        addView(viewArr[0]);
        viewArr[0].setVisibility(0);
        o(false);
    }

    public void setAllowDisallowInterceptTouch(boolean z10) {
        this.L = z10;
    }

    public void setPosition(int i9) {
        if (this.H == null) {
            this.f31033b = i9;
            w(false);
        }
        AnimatorSet animatorSet = this.f31040w;
        if (animatorSet != null) {
            animatorSet.cancel();
        }
        View[] viewArr = this.f31035e;
        View view = viewArr[1];
        if (view != null) {
            this.h.put(this.f31036f[1], view);
            removeView(viewArr[1]);
            viewArr[1] = null;
        }
        int i10 = this.f31033b;
        if (i10 != i9) {
            this.f31033b = i9;
            this.d = 0;
            this.f31034c = 1.0f;
            View view2 = viewArr[0];
            I(0);
            t(viewArr[0], view2, this.f31033b, i10);
            E(viewArr[0], 0.0f);
            c71 c71Var = this.I;
            if (c71Var != null) {
                c71Var.e(this.f31034c, this.f31033b, this.d);
            }
            w(true);
        }
    }

    public void y(int i9, boolean z10) {
        x(i9);
    }

    public void h() {
    }

    public void s() {
    }

    public void u() {
    }

    public void v() {
    }

    public void w(boolean z10) {
    }

    public void x(int i9) {
    }

    public void z(int i9) {
    }

    public void t(View view, View view2, int i9, int i10) {
    }
}
