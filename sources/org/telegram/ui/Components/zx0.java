package org.telegram.ui.Components;

import androidx.recyclerview.widget.RecyclerView;
import org.telegram.messenger.MediaDataController;
import org.telegram.tgnet.TLRPC;
public final class zx0 extends s4.x {
    public int e;
    public final gy0 f30998f;

    public zx0(gy0 gy0Var) {
        this.f30998f = gy0Var;
        this.d = 15;
        this.e = -1;
    }

    @Override
    public final boolean n(RecyclerView recyclerView, s4.c1 c1Var, s4.c1 c1Var2) {
        int i10 = c1Var.f42962f;
        if (i10 != 3 && i10 == c1Var2.f42962f) {
            gy0 gy0Var = this.f30998f;
            if (gy0Var.S == null) {
                return false;
            }
            int b10 = c1Var.b();
            int b11 = c1Var2.b();
            gy0Var.S.documents.add(b11, gy0Var.S.documents.remove(b10));
            gy0Var.d.p(b10, b11);
            this.e = b11;
            return true;
        }
        return false;
    }

    @Override
    public final void p(s4.c1 c1Var, int i10) {
        gy0 gy0Var = this.f30998f;
        if (i10 == 0 && gy0Var.f24605f != null && this.e > 0) {
            TLRPC.TL_stickers_changeStickerPosition tL_stickers_changeStickerPosition = new TLRPC.TL_stickers_changeStickerPosition();
            tL_stickers_changeStickerPosition.position = this.e;
            tL_stickers_changeStickerPosition.sticker = MediaDataController.getInputStickerSetItem(gy0Var.f24605f, "").document;
            this.e = -1;
            gy0Var.f24605f = null;
        } else if (i10 == 2) {
            gy0Var.f24605f = ((org.telegram.ui.Cells.f8) c1Var.f42959a).getSticker();
        }
    }

    @Override
    public final void q(s4.c1 c1Var) {
    }

    @Override
    public final void o(RecyclerView recyclerView, s4.c1 c1Var, s4.c1 c1Var2, int i10, int i11, int i12) {
    }
}
