package org.telegram.ui.Components;

import android.view.View;
public final class sq0 implements View.OnClickListener {
    public final int f30426a;
    public final xu0 f30427b;

    public sq0(xu0 xu0Var, int i10) {
        this.f30426a = i10;
        this.f30427b = xu0Var;
    }

    @Override
    public final void onClick(View view) {
        switch (this.f30426a) {
            case 0:
                this.f30427b.L(true);
                return;
            case 1:
                this.f30427b.C0(102, view);
                return;
            case 2:
                this.f30427b.C0(100, view);
                return;
            case 3:
                this.f30427b.C0(103, view);
                return;
            case 4:
                this.f30427b.C0(104, view);
                return;
            case 5:
                this.f30427b.C0(101, view);
                return;
            case 6:
                xu0 xu0Var = this.f30427b;
                sr0 sr0Var = xu0Var.W;
                nr0 nr0Var = xu0Var.V;
                if (xu0Var.f32740q0.getAlpha() >= 0.1f) {
                    if (nr0Var != null && nr0Var.g()) {
                        nr0Var.i();
                    }
                    if (sr0Var != null && sr0Var.f35656w) {
                        wu0 i12 = xu0Var.i1(xu0Var.h1(xu0Var.getClosestTab()));
                        qt0 W = xu0Var.W(i12.f32369a);
                        if (W != null) {
                            sr0Var.setReorderingAlbums(false);
                            wr0 wr0Var = W.h;
                            for (int i10 = 0; i10 < wr0Var.getChildCount(); i10++) {
                                View childAt = wr0Var.getChildAt(i10);
                                if (childAt instanceof org.telegram.ui.Cells.t7) {
                                    ((org.telegram.ui.Cells.t7) childAt).l(false, true);
                                }
                            }
                            vu0 vu0Var = i12.f32371c;
                            if (vu0Var != null && vu0Var.f31013x) {
                                vu0Var.f31013x = false;
                                return;
                            }
                            return;
                        }
                        return;
                    }
                    return;
                }
                return;
            default:
                org.telegram.ui.ActionBar.n2 n2Var = this.f30427b.f32753v1;
                n2Var.getMessagesController().getMainSettings().edit().putBoolean("story_keep", true).apply();
                di.pc.E(n2Var.getParentActivity(), n2Var.getCurrentAccount()).R(null);
                return;
        }
    }
}
