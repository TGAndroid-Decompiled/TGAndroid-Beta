...
public class RecordStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.save();
        canvas.translate(0.0f, AndroidUtilities.dp(this.isChat ? 1.0f : 2.0f) + (getIntrinsicHeight() / 2));
        for (int i = 0; i < 4; i++) {
            if (i == 0) {
            ...
            float fDp = (AndroidUtilities.dp(4.0f) * this.progress) + (AndroidUtilities.dp(4.0f) * i);
            float f = -fDp;
