package org.telegram.ui.ActionBar;

import android.graphics.Rect;
import android.graphics.RectF;
import android.text.TextUtils;
import java.util.ArrayList;
import org.telegram.messenger.LocaleController;
import org.telegram.messenger.R;
public final class l3 extends j1.b {
    public static final int f19355r = 0;
    public final RectF f19356o;
    public final Rect f19357p;
    public final n3 f19358q;

    public l3(n3 n3Var, n3 n3Var2) {
        super(n3Var2);
        this.f19358q = n3Var;
        this.f19356o = new RectF();
        this.f19357p = new Rect();
    }

    @Override
    public final int g(float f7, float f10) {
        k3 c10;
        n3 n3Var = this.f19358q;
        if (n3Var.f19413b) {
            ArrayList<m3> tabs = n3Var.getTabs();
            if (!tabs.isEmpty() && (c10 = n3Var.c(tabs.get(0))) != null) {
                float c11 = c10.c();
                RectF rectF = this.f19356o;
                n3Var.d(rectF, c11);
                Rect bounds = c10.f19330k.getBounds();
                if (!bounds.isEmpty() && bounds.contains((int) (f7 - rectF.left), (int) (f10 - rectF.centerY()))) {
                    return 2;
                }
                if (rectF.contains(f7, f10)) {
                    return 1;
                }
                return -1;
            }
            return -1;
        }
        return -1;
    }

    @Override
    public final void h(ArrayList arrayList) {
        n3 n3Var = this.f19358q;
        if (n3Var.f19413b) {
            ArrayList<m3> tabs = n3Var.getTabs();
            if (tabs.isEmpty() || n3Var.c(tabs.get(0)) == null) {
                return;
            }
            arrayList.add(1);
            arrayList.add(2);
        }
    }

    @Override
    public final boolean k(int i10, int i11) {
        if (i11 == 16) {
            n3 n3Var = this.f19358q;
            ArrayList<m3> tabs = n3Var.getTabs();
            if (!tabs.isEmpty()) {
                m3 m3Var = tabs.get(0);
                if (i10 == 1) {
                    n3Var.b();
                    return true;
                } else if (i10 == 2) {
                    n3Var.g(m3Var, new ai.i(7));
                    return true;
                }
            }
        }
        return false;
    }

    @Override
    public final void l(int i10, s0.d dVar) {
        m3 m3Var;
        String str;
        String str2;
        n3 n3Var = this.f19358q;
        ArrayList<m3> tabs = n3Var.getTabs();
        k3 k3Var = null;
        if (tabs.isEmpty()) {
            m3Var = null;
        } else {
            m3Var = tabs.get(0);
        }
        if (m3Var != null) {
            k3Var = n3Var.c(m3Var);
        }
        dVar.i("android.widget.Button");
        dVar.b(s0.c.f42570c);
        String str3 = "";
        Rect rect = this.f19357p;
        if (k3Var == null) {
            rect.set(0, 0, 1, 1);
            dVar.h(rect);
            dVar.j("");
            dVar.p(false);
            return;
        }
        float c10 = k3Var.c();
        RectF rectF = this.f19356o;
        n3Var.d(rectF, c10);
        if (m3Var.b() != null) {
            str3 = m3Var.b();
        }
        if (i10 == 2) {
            Rect bounds = k3Var.f19330k.getBounds();
            rect.set((int) (rectF.left + bounds.left), (int) (rectF.centerY() + bounds.top), (int) (rectF.left + bounds.right), (int) (rectF.centerY() + bounds.bottom));
            dVar.h(rect);
            if (TextUtils.isEmpty(str3)) {
                str2 = LocaleController.getString(R.string.Close);
            } else {
                str2 = LocaleController.getString(R.string.Close) + ", " + str3;
            }
            dVar.j(str2);
            return;
        }
        rect.set((int) rectF.left, (int) rectF.top, (int) rectF.right, (int) rectF.bottom);
        dVar.h(rect);
        if (TextUtils.isEmpty(str3)) {
            str = LocaleController.getString(R.string.Open);
        } else {
            str = LocaleController.getString(R.string.Open) + ", " + str3;
        }
        dVar.j(str);
    }
}
