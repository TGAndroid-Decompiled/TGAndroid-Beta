package org.telegram.ui.ActionBar;

import android.animation.ValueAnimator;
public final class w2 implements Runnable {
    public final int f19635a;
    public final Object f19636b;

    public w2(Object obj, int i10) {
        this.f19635a = i10;
        this.f19636b = obj;
    }

    @Override
    public final void run() {
        switch (this.f19635a) {
            case 0:
                g3 g3Var = (g3) this.f19636b;
                if (g3Var.startAnimationRunnable == this && !g3.access$000(g3Var)) {
                    g3Var.startAnimationRunnable = null;
                    g3.access$2400(g3Var);
                    return;
                }
                return;
            case 1:
                ActionBarLayout actionBarLayout = (ActionBarLayout) this.f19636b;
                if (actionBarLayout.d == this) {
                    actionBarLayout.d = null;
                    actionBarLayout.d0(false, true, false);
                    return;
                }
                return;
            case 2:
                q1 q1Var = (q1) this.f19636b;
                ValueAnimator valueAnimator = q1Var.f19517m;
                if (valueAnimator != null && !valueAnimator.isRunning()) {
                    q1Var.f19517m.start();
                    return;
                }
                return;
            default:
                w4 w4Var = (w4) this.f19636b;
                w4Var.k();
                w4Var.j();
                w4Var.f19640f.setAlpha(1.0f);
                return;
        }
    }
}
