...
public class SponsoredMessageInfoView extends FrameLayout {
    public SponsoredMessageInfoView(final Context context, final Runnable runnable, Theme.ResourcesProvider resourcesProvider) {
        ...
        linksTextView2.setOnLinkPressListener(new LinkSpanDrawable.LinksTextView.OnLinkPress() {
            @Override
            public final void run(ClickableSpan clickableSpan) {
                SponsoredMessageInfoView.m4071$r8$lambda$pxURcfHOfjULqOH58B8ckLRDcM(this.f$0, runnable, clickableSpan);
            }
        });
    ...
    public static void m4071$r8$lambda$pxURcfHOfjULqOH58B8ckLRDcM(SponsoredMessageInfoView sponsoredMessageInfoView, Runnable runnable, ClickableSpan clickableSpan) {
        sponsoredMessageInfoView.getClass();
        if (runnable != null) {
            runnable.run();
        }
