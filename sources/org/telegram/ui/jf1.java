package org.telegram.ui;

import android.view.View;
import androidx.recyclerview.widget.RecyclerView;
import java.util.ArrayList;
import org.telegram.messenger.ChatObject;
import org.telegram.messenger.TopicsController;
import org.telegram.tgnet.TLRPC;
public final class jf1 extends f2.b0 {
    public boolean d;
    public final kf1 e;

    public jf1(kf1 kf1Var) {
        this.e = kf1Var;
    }

    @Override
    public final void a(RecyclerView recyclerView, f2.l1 l1Var) {
        super.a(recyclerView, l1Var);
        l1Var.f5785a.setPressed(false);
    }

    @Override
    public final int e(RecyclerView recyclerView, f2.l1 l1Var) {
        int l10 = f2.b0.l(0, 0);
        int b10 = l1Var.b();
        if (b10 >= 0) {
            kf1 kf1Var = this.e;
            if (b10 < kf1Var.f35671b.size() && ((bf1) kf1Var.f35671b.get(b10)).f32951c != null && ChatObject.canManageTopics(kf1Var.g())) {
                TLRPC.TL_forumTopic tL_forumTopic = ((bf1) kf1Var.f35671b.get(b10)).f32951c;
                if (kf1Var.X.isEmpty()) {
                    View view = l1Var.f5785a;
                    if ((view instanceof hf1) && tL_forumTopic.f19236id == 1) {
                        this.d = true;
                        ((hf1) view).setSliding(true);
                        return f2.b0.l(0, 4);
                    }
                }
                if (!tL_forumTopic.pinned) {
                    return l10;
                }
                return f2.b0.l(3, 0);
            }
        }
        return l10;
    }

    @Override
    public final boolean k() {
        return !this.e.X.isEmpty();
    }

    @Override
    public final boolean n(RecyclerView recyclerView, f2.l1 l1Var, f2.l1 l1Var2) {
        int b10;
        kf1 kf1Var = this.e;
        ArrayList arrayList = kf1Var.f35671b;
        if (l1Var.f5788f == l1Var2.f5788f && (b10 = l1Var2.b()) >= 0 && b10 < arrayList.size() && ((bf1) arrayList.get(b10)).f32951c != null && ((bf1) arrayList.get(b10)).f32951c.pinned) {
            ze1 ze1Var = kf1Var.f35699r;
            int b11 = l1Var.b();
            int b12 = l1Var2.b();
            kf1 kf1Var2 = ze1Var.d;
            ArrayList arrayList2 = kf1Var2.f35671b;
            arrayList2.add(b12, (bf1) arrayList2.remove(b11));
            f2.t0 itemAnimator = kf1Var2.K.getItemAnimator();
            xe1 xe1Var = kf1Var2.F0;
            if (itemAnimator != xe1Var) {
                kf1Var2.K.setItemAnimator(xe1Var);
            }
            ze1Var.p(b11, b12);
            return true;
        }
        return false;
    }

    @Override
    public final void p(f2.l1 l1Var, int i10) {
        kf1 kf1Var = this.e;
        if (i10 == 0) {
            ArrayList arrayList = kf1Var.f35671b;
            ArrayList<Integer> arrayList2 = new ArrayList<>();
            for (int i11 = 0; i11 < arrayList.size(); i11++) {
                TLRPC.TL_forumTopic tL_forumTopic = ((bf1) arrayList.get(i11)).f32951c;
                if (tL_forumTopic != null && tL_forumTopic.pinned) {
                    arrayList2.add(Integer.valueOf(tL_forumTopic.f19236id));
                }
            }
            kf1Var.getMessagesController().getTopicsController().reorderPinnedTopics(kf1Var.f35668a, arrayList2);
            return;
        }
        kf1Var.K.I0(false);
        l1Var.f5785a.setPressed(true);
    }

    @Override
    public final void q(f2.l1 l1Var) {
        if (l1Var != null) {
            hf1 hf1Var = (hf1) l1Var.f5785a;
            TLRPC.TL_forumTopic tL_forumTopic = hf1Var.K;
            kf1 kf1Var = this.e;
            if (tL_forumTopic != null) {
                TopicsController topicsController = kf1Var.getMessagesController().getTopicsController();
                long j10 = kf1Var.f35668a;
                TLRPC.TL_forumTopic tL_forumTopic2 = hf1Var.K;
                topicsController.toggleShowTopic(j10, tL_forumTopic2.f19236id, tL_forumTopic2.hidden);
            }
            kf1Var.Y0 = hf1Var;
            int i10 = if1.f35110e3;
            kf1Var.K.z1(!hf1Var.K.hidden, hf1Var);
            kf1Var.U0(true, true);
            TLRPC.TL_forumTopic tL_forumTopic3 = hf1Var.V4;
            if (tL_forumTopic3 != null) {
                hf1Var.setTopicIcon(tL_forumTopic3);
            }
        }
    }
}
