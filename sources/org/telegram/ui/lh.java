package org.telegram.ui;

import org.telegram.messenger.Utilities;
public final class lh implements Utilities.Callback {
    public final int f34978a;
    public final long f34979b;
    public final long f34980c;
    public final Long d;
    public final Object e;

    public lh(Object obj, long j3, long j10, Long l4, int i10) {
        this.f34978a = i10;
        this.e = obj;
        this.f34979b = j3;
        this.f34980c = j10;
        this.d = l4;
    }

    @Override
    public final void run(Object obj) {
        int i10;
        boolean z10;
        switch (this.f34978a) {
            case 0:
                xn.Y((xn) this.e, this.f34979b, this.f34980c, this.d, (Boolean) obj);
                return;
            default:
                Boolean bool = (Boolean) obj;
                i10 = ((org.telegram.ui.ActionBar.n2) ((kj) this.e).f34737b).currentAccount;
                yh.t5 y3 = yh.t5.y(i10, false);
                if (this.d.longValue() > 0 && bool.booleanValue()) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                y3.i0(this.f34979b, this.f34980c, z10, true);
                return;
        }
    }
}
