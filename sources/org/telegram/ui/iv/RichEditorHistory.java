...
public class RichEditorHistory {
    ...
    private Snapshot capture() {
        ...
        while (i < rows.size()) {
            ...
            if (rowState2 != null && rowState2.level == blockRow.level && rowState2.num == blockRow.num && rowState2.checkbox == blockRow.checkbox && rowState2.checked == blockRow.checked && rowState2.detailsEnd == blockRow.detailsEnd && rowState2.media == blockRow.media && sameMedias(rowState2.medias, blockRow.medias) && Arrays.equals(rowState2.blockData, bArrSerializeBlock)) {
                rowStateArr[i] = rowState2;
                arrayList = rows;
            } else {
                arrayList = rows;
                rowStateArr[i] = new RowState(blockRow.id, bArrSerializeBlock, blockRow.level, blockRow.num, blockRow.checkbox, blockRow.checked, blockRow.detailsEnd, blockRow.media, blockRow.medias != null ? new ArrayList(blockRow.medias) : null);
            }
            i++;
            rows = arrayList;
        }
