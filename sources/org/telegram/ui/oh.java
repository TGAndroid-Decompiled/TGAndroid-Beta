package org.telegram.ui;

import org.telegram.messenger.Utilities;
public final class oh implements Utilities.Callback {
    public final int f39274a;
    public final long f39275b;
    public final long f39276c;
    public final Long d;
    public final Object f39277e;

    public oh(Object obj, long j3, long j10, Long l4, int i10) {
        this.f39274a = i10;
        this.f39277e = obj;
        this.f39275b = j3;
        this.f39276c = j10;
        this.d = l4;
    }

    @Override
    public final void run(Object obj) {
        int i10;
        boolean z10;
        switch (this.f39274a) {
            case 0:
                co.Y((co) this.f39277e, this.f39275b, this.f39276c, this.d, (Boolean) obj);
                return;
            default:
                Boolean bool = (Boolean) obj;
                i10 = ((org.telegram.ui.ActionBar.n2) ((nj) this.f39277e).f38995b).currentAccount;
                zh.s5 y3 = zh.s5.y(i10, false);
                if (this.d.longValue() > 0 && bool.booleanValue()) {
                    z10 = true;
                } else {
                    z10 = false;
                }
                y3.i0(this.f39275b, this.f39276c, z10, true);
                return;
        }
    }
}
