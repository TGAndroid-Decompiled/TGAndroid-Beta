package org.telegram.ui;

import android.animation.ValueAnimator;
import android.app.Dialog;
import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapShader;
import android.graphics.Matrix;
import android.graphics.Paint;
import android.os.Build;
import android.os.Bundle;
import android.view.ViewGroup;
import android.view.Window;
import android.view.WindowManager;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.BuildVars;
import org.telegram.messenger.FileLog;
import org.telegram.messenger.R;
public final class o50 extends Dialog {
    public final ai.n4 f35683a;
    public final p50 f35684b;
    public Bitmap f35685c;
    public Paint d;
    public BitmapShader e;
    public final Matrix f35686f;
    public float h;
    public ValueAnimator f35687n;
    public boolean f35688r;

    public o50(Context context, p50 p50Var) {
        super(context, R.style.TransparentDialog);
        this.f35686f = new Matrix();
        this.f35684b = p50Var;
        p50Var.setVisibility(4);
        AndroidUtilities.makeGlobalBlurBitmap(new dt(4, this, p50Var), 14.0f);
        ai.n4 n4Var = new ai.n4(this, context, p50Var);
        this.f35683a = n4Var;
        n4Var.setOnClickListener(new a(this, 29));
    }

    public final void b(float f7, n50 n50Var) {
        ValueAnimator valueAnimator = this.f35687n;
        if (valueAnimator != null) {
            valueAnimator.cancel();
            this.f35687n = null;
        }
        ValueAnimator ofFloat = ValueAnimator.ofFloat(this.h, f7);
        this.f35687n = ofFloat;
        ofFloat.addUpdateListener(new c3(this, 16));
        this.f35687n.addListener(new ai.t2(this, f7, n50Var, 3));
        this.f35687n.setDuration(420L);
        this.f35687n.setInterpolator(org.telegram.ui.Components.rr.h);
        this.f35687n.start();
    }

    @Override
    public final void dismiss() {
        if (this.f35688r) {
            return;
        }
        this.f35688r = true;
        b(0.0f, new n50(this, 0));
        try {
            WindowManager.LayoutParams attributes = getWindow().getAttributes();
            attributes.flags |= 16;
            getWindow().setAttributes(attributes);
        } catch (Exception e) {
            FileLog.e(e);
        }
    }

    @Override
    public final void onCreate(Bundle bundle) {
        super.onCreate(bundle);
        Window window = getWindow();
        window.setWindowAnimations(R.style.DialogNoAnimation);
        setContentView(this.f35683a, new ViewGroup.LayoutParams(-1, -1));
        WindowManager.LayoutParams attributes = window.getAttributes();
        attributes.width = -1;
        attributes.height = -1;
        attributes.gravity = 119;
        attributes.dimAmount = 0.0f;
        int i10 = attributes.flags & (-3);
        attributes.softInputMode = 48;
        attributes.flags = (-2013069056) | i10;
        if (!BuildVars.DEBUG_PRIVATE_VERSION) {
            attributes.flags = i10 | (-2013060864);
            AndroidUtilities.logFlagSecure();
        }
        attributes.flags |= 1152;
        if (Build.VERSION.SDK_INT >= 28) {
            attributes.layoutInDisplayCutoutMode = 1;
        }
        window.setAttributes(attributes);
    }

    @Override
    public final void show() {
        super.show();
        b(1.0f, null);
        AndroidUtilities.runOnUIThread(new n50(this, 1), 16L);
    }
}
