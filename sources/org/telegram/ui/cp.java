package org.telegram.ui;

import android.graphics.Canvas;
import android.util.SparseArray;
import android.view.View;
import androidx.recyclerview.widget.RecyclerView;
import java.util.ArrayList;
import org.telegram.messenger.DownloadController;
import org.telegram.messenger.MessageObject;
import org.telegram.messenger.Utilities;
import org.telegram.tgnet.TLRPC;
public final class cp extends f2.b0 {
    public final int d;
    public final Object f35839e;

    public cp(Object obj, int i10) {
        this.d = i10;
        this.f35839e = obj;
    }

    @Override
    public void a(RecyclerView recyclerView, f2.m1 m1Var) {
        switch (this.d) {
            case 0:
                super.a(recyclerView, m1Var);
                m1Var.f5875a.setPressed(false);
                return;
            case 1:
                super.a(recyclerView, m1Var);
                View view = m1Var.f5875a;
                view.setPressed(false);
                view.setBackground(null);
                return;
            case 2:
                super.a(recyclerView, m1Var);
                m1Var.f5875a.setPressed(false);
                return;
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                super.a(recyclerView, m1Var);
                View view2 = m1Var.f5875a;
                view2.setPressed(false);
                if (view2.getBackground() instanceof wh.w1) {
                    wh.w1 w1Var = (wh.w1) view2.getBackground();
                    if (w1Var.f50166c) {
                        w1Var.f50166c = false;
                        w1Var.invalidateSelf();
                    }
                }
                if (h61Var.z1()) {
                    h61Var.E1(m1Var);
                    view2.animate().scaleX(0.5f).scaleY(0.5f).setDuration(200L).setInterpolator(org.telegram.ui.Components.pr.h).start();
                    return;
                }
                return;
            case 4:
            default:
                super.a(recyclerView, m1Var);
                return;
            case 5:
                super.a(recyclerView, m1Var);
                m1Var.f5875a.setPressed(false);
                return;
            case 6:
                super.a(recyclerView, m1Var);
                m1Var.f5875a.setPressed(false);
                return;
        }
    }

    @Override
    public final int e(RecyclerView recyclerView, f2.m1 m1Var) {
        switch (this.d) {
            case 0:
                if (m1Var.f5879f == 1 && ((ma) m1Var.f5875a).D) {
                    return f2.b0.l(3, 0);
                }
                return f2.b0.l(0, 0);
            case 1:
                if (m1Var.f5879f != 5) {
                    return f2.b0.l(0, 0);
                }
                return f2.b0.l(3, 0);
            case 2:
                int b10 = m1Var.b();
                org.telegram.ui.Components.dn0 dn0Var = (org.telegram.ui.Components.dn0) this.f35839e;
                if (b10 >= dn0Var.v && m1Var.b() < dn0Var.f26317w) {
                    return f2.b0.l(3, 0);
                }
                return f2.b0.l(0, 0);
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                if (h61Var.Z2 && h61Var.V2.H(m1Var.b()) >= 0) {
                    int i10 = 15;
                    if (h61Var.U2.f5816o == 0) {
                        if (!h61Var.Y2) {
                            i10 = 12;
                        }
                    } else if (!h61Var.Y2) {
                        i10 = 3;
                    }
                    return f2.b0.l(i10, 0);
                }
                return f2.b0.l(0, 0);
            case 4:
                if (((oh.h4) ((org.telegram.ui.Components.k81) this.f35839e).f28346y).Q(m1Var.b())) {
                    return f2.b0.l(12, 0);
                }
                return f2.b0.l(0, 0);
            case 5:
                if (m1Var.f5879f == 5 && r(m1Var.b())) {
                    return f2.b0.l(3, 0);
                }
                return f2.b0.l(0, 0);
            default:
                ph.l lVar = (ph.l) this.f35839e;
                if (lVar.T.D.f33666z1 && lVar.v.L(m1Var.b())) {
                    lVar.f44426f.setItemAnimator(lVar.f44427n);
                    return f2.b0.l(15, 0);
                }
                return f2.b0.l(0, 0);
        }
    }

    @Override
    public boolean k() {
        switch (this.d) {
            case 2:
                return ((org.telegram.ui.Components.dn0) this.f35839e).F.f();
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                if (h61Var.Z2 && h61Var.f27390b3) {
                    return true;
                }
                return false;
            case 4:
            default:
                return super.k();
            case 5:
                return true;
            case 6:
                return ((ph.l) this.f35839e).T.D.f33666z1;
        }
    }

    @Override
    public void m(Canvas canvas, RecyclerView recyclerView, f2.m1 m1Var, float f10, float f11, int i10, boolean z4) {
        switch (this.d) {
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                if (i10 != 2 || z4 || !h61Var.z1()) {
                    super.m(canvas, recyclerView, m1Var, f10, f11, i10, z4);
                    if (i10 == 2 && z4) {
                        h61Var.D1(m1Var);
                        return;
                    }
                    return;
                }
                return;
            case 4:
                super.m(canvas, recyclerView, m1Var, f10, f11, i10, z4);
                ((org.telegram.ui.Components.k81) this.f35839e).invalidate();
                return;
            default:
                super.m(canvas, recyclerView, m1Var, f10, f11, i10, z4);
                return;
        }
    }

    @Override
    public final boolean n(RecyclerView recyclerView, f2.m1 m1Var, f2.m1 m1Var2) {
        int i10;
        int i11;
        int i12;
        ArrayList arrayList;
        switch (this.d) {
            case 0:
                if (m1Var.f5879f == m1Var2.f5879f) {
                    View view = m1Var2.f5875a;
                    if (!(view instanceof ma) || ((ma) view).D) {
                        bp bpVar = ((dp) this.f35839e).U2;
                        int b10 = m1Var.b();
                        int b11 = m1Var2.b();
                        int i13 = b10 - 1;
                        int i14 = b11 - 1;
                        dp dpVar = bpVar.f35584c;
                        ep epVar = dpVar.X2;
                        ArrayList arrayList2 = epVar.K;
                        if (i13 >= epVar.K.size() || i14 >= arrayList2.size()) {
                            return true;
                        }
                        if (b10 != b11) {
                            dpVar.V2 = true;
                        }
                        arrayList2.set(i13, (TLRPC.TL_username) arrayList2.get(i14));
                        arrayList2.set(i14, (TLRPC.TL_username) arrayList2.get(i13));
                        bpVar.p(b10, b11);
                        int size = arrayList2.size();
                        if (b10 != size && b11 != size) {
                            return true;
                        }
                        bpVar.n(b10, 3);
                        bpVar.n(b11, 3);
                        return true;
                    }
                }
                return false;
            case 1:
                if (m1Var.f5879f != m1Var2.f5879f) {
                    return false;
                }
                org.telegram.ui.Components.pn pnVar = ((org.telegram.ui.Components.rn) this.f35839e).f30840r;
                int b12 = m1Var.b();
                int b13 = m1Var2.b();
                org.telegram.ui.Components.rn rnVar = pnVar.d;
                int i15 = rnVar.f30839q0;
                eh.i iVar = rnVar.f30831i1;
                int i16 = b12 - i15;
                int i17 = b13 - i15;
                if (i16 >= 0 && i17 >= 0 && i16 < (i10 = rnVar.J) && i17 < i10) {
                    eh.h b14 = iVar.b(i16);
                    SparseArray sparseArray = iVar.f5638a;
                    sparseArray.put(i16, iVar.b(i17));
                    sparseArray.put(i17, b14);
                    CharSequence[] charSequenceArr = rnVar.H;
                    CharSequence charSequence = charSequenceArr[i16];
                    charSequenceArr[i16] = charSequenceArr[i17];
                    charSequenceArr[i17] = charSequence;
                    boolean[] zArr = rnVar.I;
                    boolean z4 = zArr[i16];
                    zArr[i16] = zArr[i17];
                    zArr[i17] = z4;
                    pnVar.p(b12, b13);
                }
                return true;
            case 2:
                int b15 = m1Var2.b();
                org.telegram.ui.Components.dn0 dn0Var = (org.telegram.ui.Components.dn0) this.f35839e;
                ArrayList arrayList3 = dn0Var.f26312e;
                if (b15 >= dn0Var.v && m1Var2.b() < dn0Var.f26317w) {
                    int b16 = m1Var.b();
                    int b17 = m1Var2.b();
                    int i18 = dn0Var.v;
                    int i19 = b16 - i18;
                    int i20 = b17 - i18;
                    arrayList3.indexOf(Integer.valueOf(i19));
                    arrayList3.get(b16 - dn0Var.v);
                    MessageObject messageObject = (MessageObject) arrayList3.get(i19);
                    MessageObject messageObject2 = (MessageObject) arrayList3.get(i20);
                    arrayList3.set(i19, messageObject2);
                    arrayList3.set(i20, messageObject);
                    DownloadController.getInstance(dn0Var.d).swapLoadingPriority(messageObject, messageObject2);
                    dn0Var.f26311c.p(b16, b17);
                    return false;
                }
                return false;
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                org.telegram.ui.Components.w51 w51Var = h61Var.V2;
                if (w51Var.H(m1Var.b()) >= 0 && w51Var.H(m1Var.b()) == w51Var.H(m1Var2.b())) {
                    int b18 = m1Var.b();
                    int b19 = m1Var2.b();
                    ArrayList arrayList4 = w51Var.f32654x;
                    if (w51Var.I != null) {
                        int H = w51Var.H(b18);
                        int H2 = w51Var.H(b19);
                        if (H >= 0 && H == H2) {
                            boolean J = w51Var.J(b18);
                            boolean J2 = w51Var.J(b19);
                            arrayList4.add(b19, (org.telegram.ui.Components.h51) arrayList4.remove(b18));
                            w51Var.p(b18, b19);
                            if (w51Var.J(b19) != J) {
                                w51Var.n(b19, 3);
                            }
                            if (w51Var.J(b18) != J2) {
                                w51Var.n(b18, 3);
                            }
                            if (w51Var.H && (i11 = w51Var.G) != H) {
                                w51Var.F(i11);
                            }
                            w51Var.H = true;
                            w51Var.G = H;
                        }
                    }
                    h61Var.G1();
                    return true;
                }
                return false;
            case 4:
                int b20 = m1Var.b();
                int b21 = m1Var2.b();
                org.telegram.ui.Components.k81 k81Var = (org.telegram.ui.Components.k81) this.f35839e;
                ArrayList arrayList5 = k81Var.h;
                boolean z10 = false;
                int i21 = 0;
                z10 = false;
                if (((oh.h4) k81Var.f28346y).Q(b20) && ((oh.h4) k81Var.f28346y).Q(b21)) {
                    Utilities.swapItems(arrayList5, b20, b21);
                    k81Var.f28345x.p(b20, b21);
                    ArrayList arrayList6 = new ArrayList();
                    int size2 = arrayList5.size();
                    while (i21 < size2) {
                        Object obj = arrayList5.get(i21);
                        i21++;
                        arrayList6.add(Integer.valueOf(((org.telegram.ui.Components.h81) obj).f27413a));
                    }
                    org.telegram.ui.Components.d81 d81Var = ((org.telegram.ui.Components.l81) ((oh.h4) k81Var.f28346y).f17182b).I;
                    z10 = true;
                    z10 = true;
                    if (d81Var != null) {
                        d81Var.a(arrayList6);
                    }
                }
                return z10;
            case 5:
                if (m1Var.f5879f == m1Var2.f5879f && r(m1Var.b()) && r(m1Var2.b())) {
                    kv0 kv0Var = ((mv0) this.f35839e).f39091b;
                    int b22 = m1Var.b();
                    int b23 = m1Var2.b();
                    mv0 mv0Var = kv0Var.d;
                    int i22 = mv0Var.f39104k0;
                    int i23 = b22 - i22;
                    int i24 = b23 - i22;
                    if (i23 >= 0 && i24 >= 0 && i23 < (i12 = mv0Var.f39122y) && i24 < i12) {
                        CharSequence[] charSequenceArr2 = mv0Var.v;
                        CharSequence charSequence2 = charSequenceArr2[i23];
                        charSequenceArr2[i23] = charSequenceArr2[i24];
                        charSequenceArr2[i24] = charSequence2;
                        int[] iArr = mv0Var.f39111r;
                        if (iArr != null) {
                            int i25 = iArr[i23];
                            iArr[i23] = iArr[i24];
                            iArr[i24] = i25;
                        }
                        boolean[] zArr2 = mv0Var.f39118w;
                        boolean z11 = zArr2[i23];
                        zArr2[i23] = zArr2[i24];
                        zArr2[i24] = z11;
                        kv0Var.p(b22, b23);
                    }
                    return true;
                }
                return false;
            default:
                ph.g gVar = ((ph.l) this.f35839e).v;
                if (!gVar.L(m1Var.b()) || !gVar.L(m1Var2.b())) {
                    return false;
                }
                int b24 = m1Var.b();
                int b25 = m1Var2.b();
                ArrayList arrayList7 = gVar.f44419n;
                oh.l6 l6Var = gVar.f44417e;
                if (l6Var != null && b24 >= 0 && b24 < l6Var.f17404i.size() && b25 >= 0 && b25 < gVar.f44417e.f17404i.size()) {
                    if (gVar.f44417e instanceof oh.d6) {
                        arrayList = new ArrayList();
                        for (int i26 = 0; i26 < gVar.f44417e.f17404i.size(); i26++) {
                            arrayList.add(Integer.valueOf(((MessageObject) gVar.f44417e.f17404i.get(i26)).getId()));
                        }
                    } else {
                        arrayList = new ArrayList(gVar.f44417e.f17403g);
                    }
                    if (!gVar.f44420r) {
                        arrayList7.clear();
                        arrayList7.addAll(arrayList);
                        gVar.f44420r = true;
                    }
                    MessageObject messageObject3 = (MessageObject) gVar.f44417e.f17404i.get(b24);
                    MessageObject messageObject4 = (MessageObject) gVar.f44417e.f17404i.get(b25);
                    arrayList.remove(Integer.valueOf(messageObject3.getId()));
                    arrayList.add(Utilities.clamp(b25, arrayList.size(), 0), Integer.valueOf(messageObject3.getId()));
                    gVar.f44417e.C(arrayList, false);
                    gVar.p(b24, b25);
                }
                return true;
        }
    }

    @Override
    public void o(RecyclerView recyclerView, f2.m1 m1Var, f2.m1 m1Var2, int i10, int i11, int i12) {
        switch (this.d) {
            case 3:
                return;
            default:
                super.o(recyclerView, m1Var, m1Var2, i10, i11, i12);
                return;
        }
    }

    @Override
    public void p(f2.m1 m1Var, int i10) {
        ArrayList arrayList;
        boolean z4;
        switch (this.d) {
            case 0:
                dp dpVar = (dp) this.f35839e;
                ep epVar = dpVar.X2;
                if (i10 == 0) {
                    epVar.I = false;
                    if (dpVar.V2) {
                        TLRPC.Chat chat = epVar.U;
                        ArrayList arrayList2 = epVar.K;
                        ArrayList arrayList3 = epVar.J;
                        if (chat != null) {
                            dpVar.V2 = false;
                            TLRPC.TL_channels_reorderUsernames tL_channels_reorderUsernames = new TLRPC.TL_channels_reorderUsernames();
                            TLRPC.TL_inputChannel tL_inputChannel = new TLRPC.TL_inputChannel();
                            TLRPC.Chat chat2 = epVar.U;
                            tL_inputChannel.channel_id = chat2.f20845id;
                            tL_inputChannel.access_hash = chat2.access_hash;
                            tL_channels_reorderUsernames.channel = tL_inputChannel;
                            ArrayList<String> arrayList4 = new ArrayList<>();
                            for (int i11 = 0; i11 < arrayList3.size(); i11++) {
                                if (((TLRPC.TL_username) arrayList3.get(i11)).active) {
                                    arrayList4.add(((TLRPC.TL_username) arrayList3.get(i11)).username);
                                }
                            }
                            for (int i12 = 0; i12 < arrayList2.size(); i12++) {
                                if (((TLRPC.TL_username) arrayList2.get(i12)).active) {
                                    arrayList4.add(((TLRPC.TL_username) arrayList2.get(i12)).username);
                                }
                            }
                            tL_channels_reorderUsernames.order = arrayList4;
                            epVar.getConnectionsManager().sendRequest(tL_channels_reorderUsernames, new oh.p5(6));
                            epVar.U.usernames.clear();
                            epVar.U.usernames.addAll(arrayList3);
                            epVar.U.usernames.addAll(arrayList2);
                            epVar.getMessagesController().putChat(epVar.U, true);
                            return;
                        }
                        return;
                    }
                    return;
                }
                epVar.I = true;
                dpVar.I0(false);
                m1Var.f5875a.setPressed(true);
                return;
            case 1:
                org.telegram.ui.Components.rn rnVar = (org.telegram.ui.Components.rn) this.f35839e;
                qb1 qb1Var = rnVar.f30842s;
                if (i10 != 0) {
                    qb1Var.setItemAnimator(rnVar.v);
                    qb1Var.I0(false);
                    m1Var.f5875a.setPressed(true);
                    m1Var.f5875a.setBackgroundColor(org.telegram.ui.ActionBar.k6.v0(org.telegram.ui.ActionBar.k6.f21733h5, rnVar.f26589a));
                    return;
                }
                return;
            case 2:
                if (i10 != 0) {
                    ((org.telegram.ui.Components.dn0) this.f35839e).f26310b.I0(false);
                    m1Var.f5875a.setPressed(true);
                    return;
                }
                return;
            case 3:
                org.telegram.ui.Components.h61 h61Var = (org.telegram.ui.Components.h61) this.f35839e;
                if (m1Var != null) {
                    h61Var.c1(false);
                }
                if (i10 == 0) {
                    org.telegram.ui.Components.w51 w51Var = h61Var.V2;
                    if (w51Var.H) {
                        w51Var.F(w51Var.G);
                    }
                    if (h61Var.f27389a3 != null) {
                        h61Var.C1();
                        h61Var.f27389a3 = null;
                        return;
                    }
                    return;
                }
                h61Var.I0(false);
                if (m1Var != null) {
                    View view = m1Var.f5875a;
                    view.setPressed(true);
                    if (view.getBackground() instanceof wh.w1) {
                        wh.w1 w1Var = (wh.w1) view.getBackground();
                        if (!w1Var.f50166c) {
                            w1Var.f50166c = true;
                            w1Var.invalidateSelf();
                        }
                    }
                    if (i10 == 2) {
                        h61Var.f27389a3 = m1Var;
                        h61Var.F1(m1Var);
                        return;
                    }
                    return;
                }
                return;
            case 4:
            default:
                return;
            case 5:
                if (i10 != 0) {
                    ((mv0) this.f35839e).f39093c.I0(false);
                    m1Var.f5875a.setPressed(true);
                    return;
                }
                return;
            case 6:
                ph.l lVar = (ph.l) this.f35839e;
                ph.e eVar = lVar.f44426f;
                if (m1Var != null) {
                    eVar.c1(false);
                }
                if (i10 == 0) {
                    ph.g gVar = lVar.v;
                    ArrayList arrayList5 = gVar.f44419n;
                    oh.l6 l6Var = gVar.f44417e;
                    if (l6Var != null && gVar.f44420r) {
                        if (l6Var instanceof oh.d6) {
                            arrayList = new ArrayList();
                            for (int i13 = 0; i13 < gVar.f44417e.f17404i.size(); i13++) {
                                arrayList.add(Integer.valueOf(((MessageObject) gVar.f44417e.f17404i.get(i13)).getId()));
                            }
                        } else {
                            arrayList = l6Var.f17403g;
                        }
                        if (arrayList5.size() != arrayList.size()) {
                            z4 = true;
                        } else {
                            z4 = false;
                        }
                        if (!z4) {
                            int i14 = 0;
                            while (true) {
                                if (i14 < arrayList5.size()) {
                                    if (arrayList5.get(i14) != arrayList.get(i14)) {
                                        z4 = true;
                                    } else {
                                        i14++;
                                    }
                                }
                            }
                        }
                        if (z4) {
                            gVar.f44417e.C(arrayList, true);
                        }
                        gVar.f44420r = false;
                    }
                    eVar.setItemAnimator(null);
                    return;
                }
                eVar.I0(false);
                if (m1Var != null) {
                    m1Var.f5875a.setPressed(true);
                    return;
                }
                return;
        }
    }

    @Override
    public final void q(f2.m1 m1Var) {
        int i10 = this.d;
    }

    public boolean r(int i10) {
        mv0 mv0Var = (mv0) this.f35839e;
        if (!mv0Var.F || i10 - mv0Var.f39104k0 >= mv0Var.f39120x) {
            return true;
        }
        return false;
    }

    private final void t(f2.m1 m1Var) {
    }

    private final void u(f2.m1 m1Var) {
    }

    private final void v(f2.m1 m1Var) {
    }

    private final void w(f2.m1 m1Var) {
    }

    private final void x(f2.m1 m1Var) {
    }

    private final void y(f2.m1 m1Var) {
    }

    private final void z(f2.m1 m1Var) {
    }

    private final void s(RecyclerView recyclerView, f2.m1 m1Var, f2.m1 m1Var2, int i10, int i11, int i12) {
    }
}
