...
public class AudioVisualizerDrawable {
    ...
    private final float[] f1023dt = new float[8];
    ...
    public void setWaveform(boolean z, boolean z2, float[] fArr) {
        ...
        while (i < 7) {
            ...
            if (this.parentView == null) {
                ...
                this.f1023dt[i] = (this.animateTo[i] - this.current[i]) / (this.ANIMATION_DURATION + 80.0f);
            } else {
                this.f1023dt[i] = (this.animateTo[i] - this.current[i]) / f2;
            }
            i++;
        }
        float[] fArr4 = this.animateTo;
        fArr4[7] = z ? 1.0f : 0.0f;
        this.f1023dt[7] = (fArr4[7] - this.current[7]) / 120.0f;
    }

    public void draw(Canvas canvas, float f, float f2, boolean z, float f3, Theme.ResourcesProvider resourcesProvider) {
        if (z) {
            this.f1024p1.setColor(Theme.getColor("chat_outLoader", resourcesProvider));
            this.f1024p1.setAlpha((int) (this.ALPHA * f3));
        } else {
            this.f1024p1.setColor(Theme.getColor("chat_inLoader", resourcesProvider));
            this.f1024p1.setAlpha((int) (this.ALPHA * f3));
        }
        draw(canvas, f, f2);
    }
    ...
    public void draw(Canvas canvas, float f, float f2) {
        ...
        this.drawables[0].draw(f, f2, canvas, this.f1024p1);
        canvas.restore();
        canvas.rotate(60.0f, f, f2);
        float f11 = ((1.0f - this.idleScale) * 0.04f) + 1.0f;
        canvas.scale(f11, f11, f, f2);
        this.drawables[1].draw(f, f2, canvas, this.f1024p1);
        canvas.restore();
    }
