...
public class SeekBarWaveform {
    ...
    public void draw(android.graphics.Canvas r29, android.view.View r30) {
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p009ui.Components.SeekBarWaveform.draw(android.graphics.Canvas, android.view.View):void");
    }

    private void drawLine(Canvas canvas, float f, int i, float f2, Paint paint) {
        float f3 = f2 * this.waveScaling;
        if (f3 == 0.0f) {
            canvas.drawPoint(f + AndroidUtilities.dpf2(1.0f), i + AndroidUtilities.m34dp(7.0f), paint);
        } else {
            canvas.drawLine(f + AndroidUtilities.dpf2(1.0f), (AndroidUtilities.m34dp(7.0f) + i) - f3, f + AndroidUtilities.dpf2(1.0f), i + AndroidUtilities.m34dp(7.0f) + f3, paint);
        }
    }
