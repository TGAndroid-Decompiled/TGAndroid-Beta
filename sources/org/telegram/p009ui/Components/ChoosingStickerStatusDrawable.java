...
public class ChoosingStickerStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float f3 = 2.0f;
        if (this.increment) {
            f2 = (AndroidUtilities.m34dp(2.1f) * interpolation) + ((AndroidUtilities.m34dp(7.0f) - AndroidUtilities.m34dp(2.1f)) * (1.0f - interpolation));
            f = AndroidUtilities.dpf2(1.5f) * (1.0f - cubicBezierInterpolator.getInterpolation(this.progress / 2.0f));
        } else {
            f2 = (AndroidUtilities.m34dp(2.1f) * (1.0f - interpolation)) + ((AndroidUtilities.m34dp(7.0f) - AndroidUtilities.m34dp(2.1f)) * interpolation);
            f = AndroidUtilities.dpf2(1.5f) * CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(this.progress / 2.0f);
        }
        float dp = AndroidUtilities.m34dp(11.0f) / 2.0f;
        ...
        if (paint.getStrokeWidth() != AndroidUtilities.m34dp(0.8f)) {
            paint.setStrokeWidth(AndroidUtilities.m34dp(0.8f));
        }
        int i = 0;
        while (i < 2) {
            canvas.save();
            canvas.translate((paint.getStrokeWidth() / f3) + f + (AndroidUtilities.m34dp(9.0f) * i) + getBounds().left + AndroidUtilities.dpf2(0.2f), (paint.getStrokeWidth() / f3) + AndroidUtilities.dpf2(f3) + getBounds().top);
            RectF rectF = AndroidUtilities.rectTmp;
            rectF.set(0.0f, dpf22, AndroidUtilities.m34dp(7.0f), AndroidUtilities.m34dp(11.0f) - dpf22);
