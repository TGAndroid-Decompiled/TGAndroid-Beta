...
public class SpeedLineParticles$Drawable {
    ...
    private final float f1101dt = 1000.0f / AndroidUtilities.screenRefreshRate;
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.particles.size(); i++) {
            ...
            if (currentTimeMillis > particle.lifeTime || !this.screenRect.contains(particle.f1102x, particle.f1103y)) {
    ...
    private class Particle {
        ...
        public void draw(Canvas canvas, int i, long j) {
            int i2 = i * 4;
            SpeedLineParticles$Drawable.this.lines[i2] = this.f1102x;
            SpeedLineParticles$Drawable.this.lines[i2 + 1] = this.f1103y;
            SpeedLineParticles$Drawable.this.lines[i2 + 2] = this.f1102x + (AndroidUtilities.m35dp(30.0f) * this.vecX);
            SpeedLineParticles$Drawable.this.lines[i2 + 3] = this.f1103y + (AndroidUtilities.m35dp(30.0f) * this.vecY);
            if (SpeedLineParticles$Drawable.this.paused) {
                return;
            }
            float m35dp = AndroidUtilities.m35dp(4.0f) * (SpeedLineParticles$Drawable.this.f1101dt / 660.0f);
            ...
            this.f1102x += this.vecX * f;
            this.f1103y += this.vecY * f;
            float f2 = this.inProgress;
            if (f2 != 1.0f) {
                float f3 = f2 + (speedLineParticles$Drawable.f1101dt / 200.0f);
                this.inProgress = f3;
                if (f3 > 1.0f) {
        ...
        public void genPosition(long j, boolean z) {
            ...
            this.f1102x = abs;
            this.f1103y = rectF.top + Math.abs(Utilities.fastRandom.nextInt() % rectF.height());
            double atan2 = Math.atan2(abs - SpeedLineParticles$Drawable.this.rect.centerX(), this.f1103y - SpeedLineParticles$Drawable.this.rect.centerY());
