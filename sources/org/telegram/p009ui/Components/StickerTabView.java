...
public class StickerTabView extends FrameLayout {
    ...
    public void setExpanded(boolean z) {
        ...
        if (i != 2) {
            ...
            this.visibleView.getLayoutParams().width = AndroidUtilities.m34dp(z ? f2 : f);
            ViewGroup.LayoutParams layoutParams = this.visibleView.getLayoutParams();
            if (z) {
                f = f2;
            }
            layoutParams.height = AndroidUtilities.m34dp(f);
            this.textView.setVisibility(z ? 0 : 8);
            if (this.type != 1 && this.roundImage) {
                this.imageView.setRoundRadius(AndroidUtilities.m34dp(this.visibleView.getLayoutParams().width / 2.0f));
            }
        }
    }
    ...
    public void updateExpandProgress(float f) {
        ...
        if (i != 2) {
            if (this.expanded) {
                ...
                float f4 = 1.0f - f;
                this.visibleView.setTranslationY((((AndroidUtilities.m34dp(48.0f - f2) / 2.0f) - (AndroidUtilities.m34dp(98.0f - f3) / 2.0f)) * f4) - (AndroidUtilities.m34dp(8.0f) * f));
                this.visibleView.setTranslationX(((AndroidUtilities.m34dp(52.0f - f2) / 2.0f) - (AndroidUtilities.m34dp(86.0f - f3) / 2.0f)) * f4);
                this.textView.setAlpha(Math.max(0.0f, (f - 0.5f) / 0.5f));
                this.textView.setTranslationY((-AndroidUtilities.m34dp(40.0f)) * f4);
                this.textView.setTranslationX((-AndroidUtilities.m34dp(12.0f)) * f4);
