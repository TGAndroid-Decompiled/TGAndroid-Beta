...
public class TimerParticles {
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1099x;
                float f4 = particle.f1097vx;
                float f5 = particle.velocity;
                float f6 = (float) j;
                particle.f1099x = f3 + (((f4 * f5) * f6) / 500.0f);
                particle.f1100y += ((particle.f1098vy * f5) * f6) / 500.0f;
    ...
    public void draw(Canvas canvas, Paint paint, RectF rectF, float f, float f2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            canvas.drawPoint(particle2.f1099x, particle2.f1100y, paint);
        }
        double d = f - 90.0f;
        Double.isNaN(d);
        ...
        for (int i2 = 0; i2 < 1; i2++) {
            ...
            particle.f1097vx = (float) ((Math.cos(d4) * sin) - (Math.sin(d4) * d3));
            particle.f1098vy = (float) ((Math.sin(d4) * sin) + (Math.cos(d4) * d3));
            particle.alpha = 1.0f;
