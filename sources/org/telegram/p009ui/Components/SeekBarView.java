...
public class SeekBarView extends FrameLayout {
    ...
    public boolean onTouch(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            this.f1125sx = motionEvent.getX();
            this.f1126sy = motionEvent.getY();
            return true;
        }
        if (motionEvent.getAction() == 1 || motionEvent.getAction() == 3) {
            this.captured = false;
            if (motionEvent.getAction() == 1) {
                if (Math.abs(motionEvent.getY() - this.f1126sy) < ViewConfiguration.get(getContext()).getScaledTouchSlop()) {
            ...
            if (!this.captured) {
                ...
                if (Math.abs(motionEvent.getY() - this.f1126sy) <= viewConfiguration.getScaledTouchSlop() && Math.abs(motionEvent.getX() - this.f1125sx) > viewConfiguration.getScaledTouchSlop()) {
                    this.captured = true;
