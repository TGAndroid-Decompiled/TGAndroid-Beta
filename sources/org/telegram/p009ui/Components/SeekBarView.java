...
public class SeekBarView extends FrameLayout {
    ...
    public SeekBarView(Context context, boolean z, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.selectorWidth = AndroidUtilities.m34dp(32.0f);
        this.thumbSize = AndroidUtilities.m34dp(24.0f);
        this.currentRadius = AndroidUtilities.m34dp(6.0f);
        if (Build.VERSION.SDK_INT >= 21) {
            Drawable createSelectorDrawable = Theme.createSelectorDrawable(ColorUtils.setAlphaComponent(getThemedColor("player_progress"), 40), 1, AndroidUtilities.m34dp(16.0f));
    ...
    public boolean onTouch(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            this.f1084sx = motionEvent.getX();
            this.f1085sy = motionEvent.getY();
            return true;
        }
        if (motionEvent.getAction() == 1 || motionEvent.getAction() == 3) {
            this.captured = false;
            if (motionEvent.getAction() == 1) {
                if (Math.abs(motionEvent.getY() - this.f1085sy) < ViewConfiguration.get(getContext()).getScaledTouchSlop()) {
            ...
            if (!this.captured) {
                ...
                if (Math.abs(motionEvent.getY() - this.f1085sy) <= viewConfiguration.getScaledTouchSlop() && Math.abs(motionEvent.getX() - this.f1084sx) > viewConfiguration.getScaledTouchSlop()) {
                    this.captured = true;
