...
public class BlobDrawable {
    ...
    private final float f1027L;
    private final float f1028N;
    ...
    private final Matrix f1029m = new Matrix();
    ...
    public BlobDrawable(int i) {
        ...
        double d = f * 2.0f;
        Double.isNaN(d);
        this.f1027L = (float) (Math.tan(3.141592653589793d / d) * 1.3333333333333333d);
        ...
        for (int i2 = 0; i2 < this.f1028N; i2++) {
    ...
    private void generateBlob(float[] fArr, float[] fArr2, int i) {
        ...
        fArr2[i] = ((360.0f / this.f1028N) * i) + (((this.random.nextInt() % 100.0f) / 100.0f) * (360.0f / this.f1028N) * 0.05f);
    ...
    public void draw(float f, float f2, Canvas canvas, Paint paint) {
        ...
        while (true) {
            ...
            if (i < f3) {
                ...
                float min = this.f1027L * (Math.min(f7, f9) + ((Math.max(f7, f9) - Math.min(f7, f9)) / 2.0f)) * this.cubicBezierK;
                this.f1029m.reset();
                this.f1029m.setRotate(f10 + (fArr5[i] * f4), f, f2);
                ...
                fArr7[3] = f13;
                this.f1029m.reset();
                this.f1029m.setRotate(f11, f, f2);
                this.f1029m.mapPoints(this.pointEnd);
                if (i == 0) {
                    Path path = this.path;
