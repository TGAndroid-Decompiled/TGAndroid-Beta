...
public class CallSwipeView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        if (this.dragFromRight) {
            canvas.translate((getWidth() - getHeight()) - AndroidUtilities.m34dp(18.0f), getHeight() / 2);
        } else {
            canvas.translate(getHeight() + AndroidUtilities.m34dp(12.0f), getHeight() / 2);
        }
        float abs = Math.abs(this.viewToDrag.getTranslationX());
        for (int i = 0; i < 3; i++) {
            float f = 16.0f;
            float f2 = 1.0f;
            if (abs > AndroidUtilities.m34dp(i * 16)) {
                f2 = 1.0f - Math.min(1.0f, Math.max(0.0f, (abs - (AndroidUtilities.m34dp(16.0f) * i)) / AndroidUtilities.m34dp(16.0f)));
            }
            ...
            canvas.translate(AndroidUtilities.m34dp(f), 0.0f);
        }
        canvas.restore();
        invalidate();
    }
