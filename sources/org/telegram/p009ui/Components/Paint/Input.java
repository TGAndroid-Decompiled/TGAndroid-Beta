...
public class Input {
    ...
    public void process(MotionEvent motionEvent, float f) {
            ...
            if (actionMasked == 1) {
                if (!this.hasMoved) {
                    ...
                    if (this.renderView.getCurrentBrush() instanceof Brush.Arrow) {
                        ...
                        Point point3 = new Point(point2.f1067x, point2.f1068y, 0.800000011920929d);
                        double d = point2.f1067x;
                        double d2 = f2;
                        Double.isNaN(d2);
        ...
        if (!this.beganDrawing) {
        ...
        } else if (point.getDistanceTo(this.lastLocation) < AndroidUtilities.m35dp(5.0f) / f) {
        } else {
            if (!this.hasMoved) {
            ...
            this.pointsCount = i2;
            if (i2 == 3) {
                this.lastAngle = (float) Math.atan2(pointArr[2].f1068y - pointArr[1].f1068y, pointArr[2].f1067x - pointArr[1].f1067x);
    ...
    private Point smoothPoint(Point point, Point point2, Point point3, float f) {
        ...
        double d = f2 * 2.0f * f;
        double d2 = f * f;
        double d3 = point3.f1067x;
        Double.isNaN(d);
        double d4 = (point.f1067x * pow) + (d3 * d);
        double d5 = point2.f1067x;
        Double.isNaN(d2);
        double d6 = point.f1068y * pow;
        double d7 = point3.f1068y;
        Double.isNaN(d);
        double d8 = point2.f1068y;
        Double.isNaN(d2);
