...
public class ReactedHeaderView extends FrameLayout {
    ...
    public ReactedHeaderView(Context context, int i, MessageObject messageObject, long j) {
        ...
        Drawable mutate = ContextCompat.getDrawable(context, C0952R.C0953drawable.msg_reactions).mutate();
    ...
    public void lambda$onAttachedToWindow$5(long j, TLRPC$Chat tLRPC$Chat, TLObject tLObject, TLRPC$TL_error tLRPC$TL_error) {
        if (tLObject instanceof TLRPC$Vector) {
            ...
            if (ChatObject.isChannel(tLRPC$Chat)) {
                ...
                tLRPC$TL_channels_getParticipants.channel = MessagesController.getInstance(this.currentAccount).getInputChannel(tLRPC$Chat.f854id);
            ...
            tLRPC$TL_messages_getFullChat.chat_id = tLRPC$Chat.f854id;
        ...
        while (it.hasNext()) {
            ...
            while (true) {
                if (i >= this.users.size()) {
                    break;
                } else if (this.users.get(i).f985id == tLRPC$User.f985id) {
                    z = true;
                    break;
                } else {
                    i++;
    ...
    public void lambda$onAttachedToWindow$1(TLObject tLObject, List list, List list2, Runnable runnable) {
        if (tLObject != null) {
            ...
            for (int i = 0; i < tLRPC$TL_channels_channelParticipants.users.size(); i++) {
                ...
                if (!tLRPC$User.self && list.contains(Long.valueOf(tLRPC$User.f985id))) {
    ...
    public void lambda$loadReactions$6(int i, TLRPC$TL_messages_messageReactionsList tLRPC$TL_messages_messageReactionsList) {
        ...
        while (it.hasNext()) {
            ...
            TLRPC$Peer tLRPC$Peer = this.message.messageOwner.from_id;
            if (!(tLRPC$Peer == null || next.f985id == tLRPC$Peer.user_id)) {
                int i2 = 0;
                while (true) {
                    if (i2 >= this.users.size()) {
                        z2 = false;
                        break;
                    } else if (this.users.get(i2).f985id == next.f985id) {
                        z2 = true;
                        break;
                    } else {
                        i2++;
    ...
    private void updateView() {
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p009ui.Components.ReactedHeaderView.updateView():void");
    }
