...
public class BackButtonMenu {
    ...
    public static ActionBarPopupWindow show(final BaseFragment baseFragment, View view, long j) {
        ...
        while (i < stackedHistoryDialogs.size()) {
            ...
            frameLayout.setMinimumWidth(AndroidUtilities.m34dp(200.0f));
        ...
        int dp = AndroidUtilities.m34dp(8.0f) - rect.left;
        if (AndroidUtilities.isTablet()) {
            int[] iArr = new int[2];
            fragmentView.getLocationInWindow(iArr);
            dp += iArr[0];
        }
        actionBarPopupWindow.showAtLocation(fragmentView, 51, dp, (view.getBottom() - rect.top) - AndroidUtilities.m34dp(8.0f));
        try {
