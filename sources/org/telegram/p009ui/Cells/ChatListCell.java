...
public class ChatListCell extends LinearLayout {
    private C1381ListView[] listView;

    protected void didSelectChatType(boolean z) {
    }

    public class C1381ListView extends FrameLayout {
        private RadioButton button;
        private boolean isThreeLines;
        private RectF rect;
        private TextPaint textPaint;

        public C1381ListView(ChatListCell chatListCell, Context context, boolean z) {
            super(context);
            int i;
            String str;
            this.rect = new RectF();
    ...
    public ChatListCell(Context context) {
        super(context);
        this.listView = new C1381ListView[2];
        setOrientation(0);
        setPadding(AndroidUtilities.m36dp(21.0f), AndroidUtilities.m36dp(10.0f), AndroidUtilities.m36dp(21.0f), 0);
        int i = 0;
        while (true) {
            C1381ListView[] c1381ListViewArr = this.listView;
            if (i >= c1381ListViewArr.length) {
                return;
            }
            final boolean z = i == 1;
            c1381ListViewArr[i] = new C1381ListView(this, context, z);
