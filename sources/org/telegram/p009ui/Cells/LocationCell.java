...
public class LocationCell extends FrameLayout {
    ...
    public LocationCell(Context context, boolean z, Theme.ResourcesProvider resourcesProvider) {
        ...
        ShapeDrawable createCircleDrawable = Theme.createCircleDrawable(AndroidUtilities.m34dp(42.0f), -1);
        this.circleDrawable = createCircleDrawable;
        backupImageView.setBackground(createCircleDrawable);
        this.imageView.setSize(AndroidUtilities.m34dp(30.0f), AndroidUtilities.m34dp(30.0f));
    ...
    protected void onMeasure(int i, int i2) {
        if (this.wrapContent) {
            super.onMeasure(i, View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m34dp(64.0f) + (this.needDivider ? 1 : 0), 1073741824));
        } else {
    ...
    public void lambda$setLocation$0(long j, long j2, float f, float f2, ValueAnimator valueAnimator) {
        float f3 = 1.0f;
        f3 = Math.min(Math.max(((float) (SystemClock.elapsedRealtime() - j)) / ((float) j2), 0.0f), 1.0f);
        if (j2 > 0) {
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        globalGradientView.setParentSize(getMeasuredWidth(), getMeasuredHeight(), (-(getParent() instanceof ViewGroup ? ((ViewGroup) getParent()).indexOfChild(this) : 0)) * AndroidUtilities.m34dp(56.0f));
        ...
        canvas.translate(AndroidUtilities.m34dp(2.0f), (getMeasuredHeight() - AndroidUtilities.m34dp(56.0f)) / 2);
        globalGradientView.draw(canvas);
        canvas.restore();
        super.onDraw(canvas);
        if (this.needDivider) {
            canvas.drawLine(LocaleController.isRTL ? 0.0f : AndroidUtilities.m34dp(72.0f), getHeight() - 1, LocaleController.isRTL ? getWidth() - AndroidUtilities.m34dp(72.0f) : getWidth(), getHeight() - 1, Theme.dividerPaint);
        }
    }
