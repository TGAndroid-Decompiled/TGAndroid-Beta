package bh;

import java.util.ArrayList;
import org.telegram.messenger.DialogObject;
import org.telegram.messenger.MessagesController;
import org.telegram.messenger.Utilities;
import org.telegram.tgnet.TLObject;
import org.telegram.tgnet.TLRPC;
import org.telegram.ui.Components.k51;
import org.telegram.ui.Components.w41;
public final class m implements Utilities.Callback2 {
    public final int f2689a;
    public final t f2690b;

    public m(t tVar, int i10) {
        this.f2689a = i10;
        this.f2690b = tVar;
    }

    @Override
    public final void run(Object obj, Object obj2) {
        int i10 = this.f2689a;
        t tVar = this.f2690b;
        boolean z10 = false;
        switch (i10) {
            case 0:
                ArrayList arrayList = (ArrayList) obj;
                k51 k51Var = (k51) obj2;
                arrayList.clear();
                ArrayList arrayList2 = tVar.f2707j;
                int size = arrayList2.size();
                int i11 = 0;
                while (i11 < size) {
                    Object obj3 = arrayList2.get(i11);
                    i11++;
                    TLRPC.MessagePeerVote messagePeerVote = (TLRPC.MessagePeerVote) obj3;
                    long peerDialogId = DialogObject.getPeerDialogId(messagePeerVote.peer);
                    TLObject userOrChat = MessagesController.getInstance(tVar.f2700a).getUserOrChat(peerDialogId);
                    int i12 = messagePeerVote.date;
                    s sVar = new s(tVar, peerDialogId, 0);
                    int i13 = p.f2694a;
                    w41 J = w41.J(p.class);
                    J.G = userOrChat;
                    J.B = peerDialogId;
                    J.f34313z = i12;
                    J.D = sVar;
                    arrayList.add(J);
                }
                if (!tVar.h) {
                    if (arrayList2.isEmpty()) {
                        int i14 = q.f2695a;
                        arrayList.add(w41.J(q.class));
                        arrayList.add(w41.J(q.class));
                        arrayList.add(w41.J(q.class));
                        arrayList.add(w41.J(q.class));
                        arrayList.add(w41.J(q.class));
                        return;
                    }
                    int i15 = r.f2696a;
                    arrayList.add(w41.J(r.class));
                    return;
                }
                return;
            default:
                TLRPC.TL_messages_votesList tL_messages_votesList = (TLRPC.TL_messages_votesList) obj;
                TLRPC.TL_error tL_error = (TLRPC.TL_error) obj2;
                int i16 = tVar.f2700a;
                tVar.f2706i = false;
                if (tL_messages_votesList != null) {
                    MessagesController.getInstance(i16).putUsers(tL_messages_votesList.users, false);
                    MessagesController.getInstance(i16).putChats(tL_messages_votesList.chats, false);
                    String str = tL_messages_votesList.next_offset;
                    tVar.f2705g = str;
                    if (str == null) {
                        z10 = true;
                    }
                    tVar.h = z10;
                    tVar.f2707j.addAll(tL_messages_votesList.votes);
                    a4.g gVar = tVar.f2703e;
                    if (gVar != null) {
                        gVar.run();
                        return;
                    }
                    return;
                }
                tVar.f2705g = null;
                tVar.h = true;
                return;
        }
    }
}
