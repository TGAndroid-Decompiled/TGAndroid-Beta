package a4;

import h5.w;
import j3.r1;
import java.io.EOFException;
public final class g {
    public int f67a;
    public long f68b;
    public int f69c;
    public int d;
    public int e;
    public final int[] f70f = new int[255];
    public final w f71g = new w(255);

    public final boolean a(r3.l lVar, boolean z4) {
        boolean z10;
        boolean z11;
        this.f67a = 0;
        this.f68b = 0L;
        this.f69c = 0;
        this.d = 0;
        this.e = 0;
        w wVar = this.f71g;
        wVar.C(27);
        try {
            z10 = lVar.h(wVar.f7000a, 0, 27, z4);
        } catch (EOFException e) {
            if (z4) {
                z10 = false;
            } else {
                throw e;
            }
        }
        if (z10 && wVar.v() == 1332176723) {
            if (wVar.u() != 0) {
                if (!z4) {
                    throw r1.c("unsupported bit stream revision");
                }
            } else {
                this.f67a = wVar.u();
                this.f68b = wVar.j();
                wVar.l();
                wVar.l();
                wVar.l();
                int u10 = wVar.u();
                this.f69c = u10;
                this.d = u10 + 27;
                wVar.C(u10);
                try {
                    z11 = lVar.h(wVar.f7000a, 0, this.f69c, z4);
                } catch (EOFException e6) {
                    if (z4) {
                        z11 = false;
                    } else {
                        throw e6;
                    }
                }
                if (z11) {
                    for (int i10 = 0; i10 < this.f69c; i10++) {
                        int u11 = wVar.u();
                        this.f70f[i10] = u11;
                        this.e += u11;
                    }
                    return true;
                }
            }
        }
        return false;
    }

    public final boolean b(r3.l lVar, long j10) {
        boolean z4;
        int i10;
        boolean z10;
        if (lVar.getPosition() == lVar.i()) {
            z4 = true;
        } else {
            z4 = false;
        }
        h5.a.f(z4);
        w wVar = this.f71g;
        wVar.C(4);
        while (true) {
            i10 = (j10 > (-1L) ? 1 : (j10 == (-1L) ? 0 : -1));
            if (i10 != 0 && lVar.getPosition() + 4 >= j10) {
                break;
            }
            try {
                z10 = lVar.h(wVar.f7000a, 0, 4, true);
            } catch (EOFException unused) {
                z10 = false;
            }
            if (!z10) {
                break;
            }
            wVar.F(0);
            if (wVar.v() == 1332176723) {
                lVar.t();
                return true;
            }
            lVar.u(1);
        }
        do {
            if (i10 != 0 && lVar.getPosition() >= j10) {
                break;
            }
        } while (lVar.skip(1) != -1);
        return false;
    }
}
