package lh;

import android.animation.Animator;
import android.animation.AnimatorListenerAdapter;
import android.graphics.RectF;
import android.view.View;
import org.telegram.ui.LaunchActivity;
import org.telegram.ui.wa0;
public final class ua extends AnimatorListenerAdapter {
    public final mg.o0 f13227a;
    public final View f13228b;
    public final nh.o0[] f13229c;
    public final boolean[] d;
    public final RectF e;
    public final Runnable f13230f;
    public final eb h;

    public ua(eb ebVar, mg.o0 o0Var, View view, nh.o0[] o0VarArr, boolean[] zArr, RectF rectF, Runnable runnable) {
        this.h = ebVar;
        this.f13227a = o0Var;
        this.f13228b = view;
        this.f13229c = o0VarArr;
        this.d = zArr;
        this.e = rectF;
        this.f13230f = runnable;
    }

    @Override
    public final void onAnimationEnd(Animator animator) {
        wa0 wa0Var;
        eb ebVar = this.h;
        eg.i iVar = ebVar.G;
        iVar.setVisibility(4);
        iVar.setPaused(true);
        mg.o0 o0Var = this.f13227a;
        if (o0Var != null) {
            o0Var.f14088l = true;
        }
        View view = this.f13228b;
        if (view != null) {
            view.invalidate();
        }
        nh.o0 o0Var2 = this.f13229c[0];
        if (o0Var2 != null) {
            o0Var2.setDrawStar(true);
        }
        super/*org.telegram.ui.ActionBar.g3*/.dismissInternal();
        boolean[] zArr = this.d;
        if (!zArr[0]) {
            zArr[0] = true;
            RectF rectF = this.e;
            LaunchActivity.b0(rectF.centerX(), rectF.centerY(), 1.5f);
            try {
                ebVar.container.performHapticFeedback(0, 1);
            } catch (Exception unused) {
            }
            Runnable runnable = this.f13230f;
            if (runnable != null) {
                runnable.run();
            }
        }
        LaunchActivity launchActivity = LaunchActivity.D1;
        if (launchActivity != null && (wa0Var = launchActivity.f31652u0) != null) {
            wa0Var.c(true);
        }
    }
}
