package f5;
public final class q {
    public final long f8923a;
    public final long f8924b;
    public final long f8925c;

    public q(long j3, long j10, long j11) {
        this.f8923a = j3;
        this.f8924b = j10;
        this.f8925c = j11;
    }

    public final boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (obj == null || q.class != obj.getClass()) {
            return false;
        }
        q qVar = (q) obj;
        if (this.f8923a == qVar.f8923a && this.f8925c == qVar.f8925c && this.f8924b == qVar.f8924b) {
            return true;
        }
        return false;
    }

    public final int hashCode() {
        long j3 = this.f8923a;
        long j10 = this.f8924b;
        long j11 = this.f8925c;
        return (((((int) (j3 ^ (j3 >>> 32))) * 31) + ((int) (j10 ^ (j10 >>> 32)))) * 31) + ((int) (j11 ^ (j11 >>> 32)));
    }

    public final String toString() {
        return "Entry{firstChunk=" + this.f8923a + ", samplesPerChunk=" + this.f8924b + ", sampleDescriptionIndex=" + this.f8925c + '}';
    }
}
