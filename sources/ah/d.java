package ah;

import android.graphics.drawable.Drawable;
import android.view.View;
import ii.u0;
import org.telegram.ui.Components.ed;
import org.telegram.ui.Components.eq;
import org.telegram.ui.Components.t31;
import org.telegram.ui.Components.zo0;
import yh.m3;
import zg.n0;
public final class d implements Drawable.Callback {
    public final int f425a;
    public Object f426b;

    @Override
    public final void invalidateDrawable(Drawable drawable) {
        switch (this.f425a) {
            case 0:
                ((e) this.f426b).invalidateSelf();
                return;
            case 1:
                return;
            case 2:
                ((u0) this.f426b).f11651b.invalidate();
                return;
            case 3:
                ((eq) this.f426b).invalidateSelf();
                return;
            case 4:
                ((zo0) this.f426b).f30912b.run();
                return;
            case 5:
                ((ed) this.f426b).invalidateSelf();
                return;
            case 6:
                ((t31) this.f426b).invalidateSelf();
                return;
            case 7:
                ((wg.a) this.f426b).f45342c.invalidate();
                return;
            case 8:
                ((wg.c) this.f426b).f45367c.invalidate();
                return;
            case 9:
                ((x4.d) this.f426b).invalidateSelf();
                return;
            case 10:
                ((m3) this.f426b).f47764f.invalidate();
                return;
            default:
                n0 n0Var = (n0) this.f426b;
                View view = n0Var.W;
                if (view != null) {
                    view.invalidate();
                    if (n0Var.R && n0Var.W.getParent() != null && (n0Var.W.getParent().getParent() instanceof View)) {
                        ((View) n0Var.W.getParent().getParent()).invalidate();
                        return;
                    }
                    return;
                }
                return;
        }
    }

    @Override
    public final void scheduleDrawable(Drawable drawable, Runnable runnable, long j3) {
        switch (this.f425a) {
            case 0:
                ((e) this.f426b).scheduleSelf(runnable, j3);
                return;
            case 1:
                Drawable.Callback callback = (Drawable.Callback) this.f426b;
                if (callback != null) {
                    callback.scheduleDrawable(drawable, runnable, j3);
                    return;
                }
                return;
            case 2:
                return;
            case 3:
                ((eq) this.f426b).scheduleSelf(runnable, j3);
                return;
            case 4:
                return;
            case 5:
                ((ed) this.f426b).scheduleSelf(runnable, j3);
                return;
            case 6:
                return;
            case 7:
                ((wg.a) this.f426b).f45342c.invalidate();
                return;
            case 8:
                ((wg.c) this.f426b).f45367c.invalidate();
                return;
            case 9:
                ((x4.d) this.f426b).scheduleSelf(runnable, j3);
                return;
            case 10:
                return;
            default:
                View view = ((n0) this.f426b).W;
                if (view != null) {
                    view.scheduleDrawable(drawable, runnable, j3);
                    return;
                }
                return;
        }
    }

    @Override
    public final void unscheduleDrawable(Drawable drawable, Runnable runnable) {
        switch (this.f425a) {
            case 0:
                ((e) this.f426b).unscheduleSelf(runnable);
                return;
            case 1:
                Drawable.Callback callback = (Drawable.Callback) this.f426b;
                if (callback != null) {
                    callback.unscheduleDrawable(drawable, runnable);
                    return;
                }
                return;
            case 2:
                return;
            case 3:
                ((eq) this.f426b).unscheduleSelf(runnable);
                return;
            case 4:
                return;
            case 5:
                ((ed) this.f426b).unscheduleSelf(runnable);
                return;
            case 6:
                return;
            case 7:
                ((wg.a) this.f426b).f45342c.invalidate();
                return;
            case 8:
                ((wg.c) this.f426b).f45367c.invalidate();
                return;
            case 9:
                ((x4.d) this.f426b).unscheduleSelf(runnable);
                return;
            case 10:
                return;
            default:
                View view = ((n0) this.f426b).W;
                if (view != null) {
                    view.unscheduleDrawable(drawable, runnable);
                    return;
                }
                return;
        }
    }

    public d(Object obj, int i10) {
        this.f425a = i10;
        this.f426b = obj;
    }

    private final void a(Drawable drawable) {
    }

    private final void f(Drawable drawable, Runnable runnable) {
    }

    private final void g(Drawable drawable, Runnable runnable) {
    }

    private final void h(Drawable drawable, Runnable runnable) {
    }

    private final void i(Drawable drawable, Runnable runnable) {
    }

    private final void b(Drawable drawable, Runnable runnable, long j3) {
    }

    private final void c(Drawable drawable, Runnable runnable, long j3) {
    }

    private final void d(Drawable drawable, Runnable runnable, long j3) {
    }

    private final void e(Drawable drawable, Runnable runnable, long j3) {
    }
}
