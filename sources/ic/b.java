...
public final class b {
    ...
    public final a b(int i10, int i11, int[] iArr) {
        ...
        if (!Float.isNaN(f10)) {
            ...
            ArrayList arrayList = this.f11059b;
            int size = arrayList.size();
            while (i13 < size) {
                Object obj = arrayList.get(i13);
                i13++;
                a aVar = (a) obj;
                float f12 = aVar.f11057c;
                float f13 = aVar.f4207a;
                float f14 = aVar.f4208b;
                if (Math.abs(f10 - f14) <= f11 && Math.abs(f7 - f13) <= f11) {
                    float abs = Math.abs(f11 - f12);
                    if (abs <= 1.0f || abs <= f12) {
                        return new a((f13 + f7) / 2.0f, (f14 + f10) / 2.0f, (aVar.f11057c + f11) / 2.0f);
                    }
                }
            }
