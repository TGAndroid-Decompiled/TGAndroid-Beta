package n7;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.ColorFilter;
import android.graphics.Paint;
import android.graphics.Point;
import android.graphics.SurfaceTexture;
import android.graphics.drawable.Drawable;
import android.os.Build;
import android.os.Parcel;
import android.text.Editable;
import android.text.TextUtils;
import android.text.style.CharacterStyle;
import android.util.SparseIntArray;
import android.view.View;
import android.widget.EditText;
import com.google.android.gms.common.api.Status;
import com.google.android.gms.tasks.OnCompleteListener;
import com.google.android.gms.tasks.Task;
import com.google.android.gms.tasks.TaskCompletionSource;
import java.io.File;
import java.util.ArrayList;
import java.util.HashSet;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.BotInlineKeyboard;
import org.telegram.messenger.FileLog;
import org.telegram.messenger.MessageObject;
import org.telegram.tgnet.TLObject;
import org.telegram.tgnet.TLRPC;
import org.telegram.tgnet.tl.TL_iv;
import org.telegram.tgnet.tl.TL_keyboard;
import org.telegram.ui.ActionBar.ActionBarPopupWindow$ActionBarPopupWindowLayout;
import org.telegram.ui.Components.EditTextBoldCursor;
import org.telegram.ui.Components.cc0;
import org.telegram.ui.Components.ey0;
import org.telegram.ui.Components.f01;
import org.telegram.ui.Components.f71;
import org.telegram.ui.Components.i71;
import org.telegram.ui.Components.ll0;
import org.telegram.ui.Components.of0;
import org.telegram.ui.Components.t61;
import org.telegram.ui.Components.tt;
import org.telegram.ui.Components.vt;
import org.telegram.ui.PhotoViewer;
import org.telegram.ui.PremiumPreviewFragment;
import org.telegram.ui.SecretMediaViewer;
import org.telegram.ui.b41;
import org.telegram.ui.dn0;
import org.telegram.ui.el0;
import org.telegram.ui.fv;
import org.telegram.ui.gv;
import org.telegram.ui.l01;
import org.telegram.ui.lm0;
import org.telegram.ui.m21;
import org.telegram.ui.nw;
import org.telegram.ui.ny;
import org.telegram.ui.oy;
import org.telegram.ui.rm0;
import org.telegram.ui.vu0;
public class qa implements OnCompleteListener, od.b, org.telegram.ui.ActionBar.f6, tt, ph.ia, org.telegram.ui.Cells.k1, org.telegram.ui.k7, ll0, rm0, sg.a, f71, p2.a0, com.google.android.gms.common.api.internal.s {
    public final int f14701a;
    public Object f14702b;
    public Object f14703c;

    public qa(int i10) {
        this.f14701a = i10;
    }

    public static qa N0(View view) {
        return new qa(view);
    }

    @Override
    public boolean A0(MessageObject messageObject) {
        return true;
    }

    @Override
    public org.telegram.ui.Cells.n9 C2() {
        return null;
    }

    @Override
    public void D(String str, String str2) {
        dn0 dn0Var = ((lm0) this.f14703c).f36108a;
        if ("PHONE_VERIFICATION_NEEDED".equals(str)) {
            dn0Var.O1(true, str2, (el0) this.f14702b, this, dn0Var.f33739y1);
        } else {
            dn0Var.N1(true, false);
        }
    }

    @Override
    public boolean D1() {
        return false;
    }

    @Override
    public Paint G(String str) {
        switch (this.f14701a) {
            case 8:
                return org.telegram.ui.ActionBar.j6.S0(str);
            case 20:
                return org.telegram.ui.ActionBar.j6.S0(str);
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.S0(str);
                }
                return f6Var.G(str);
        }
    }

    @Override
    public boolean I1(org.telegram.ui.Cells.t1 t1Var, TLRPC.Chat chat) {
        return false;
    }

    @Override
    public void K0(int i10, int i11) {
        switch (this.f14701a) {
            case 8:
            case 20:
                return;
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var != null) {
                    f6Var.K0(i10, i11);
                    return;
                }
                return;
        }
    }

    @Override
    public boolean K1() {
        return false;
    }

    @Override
    public boolean O0(long j10) {
        return ((f01) this.f14703c).v;
    }

    @Override
    public boolean P(org.telegram.ui.Cells.t1 t1Var, TLRPC.TodoItem todoItem, boolean z4) {
        return false;
    }

    @Override
    public void P1(org.telegram.ui.Cells.t1 t1Var, TLRPC.WebPage webPage, String str, boolean z4) {
        af.g.s(t1Var.getContext(), str);
    }

    @Override
    public boolean Q() {
        return false;
    }

    public p2.d Q0() {
        if (((p2.n) this.f14702b) != null) {
            return new p2.d(this);
        }
        throw new NullPointerException("ProductDetails is required for constructing ProductDetailsParams.");
    }

    @Override
    public boolean R(org.telegram.ui.Cells.t1 t1Var) {
        return false;
    }

    @Override
    public CharacterStyle R1(org.telegram.ui.Cells.t1 t1Var) {
        return null;
    }

    @Override
    public boolean S() {
        return false;
    }

    @Override
    public boolean T1(org.telegram.ui.Cells.t1 t1Var, MessageObject messageObject) {
        return false;
    }

    public pb.b V0() {
        if (((pb.b) this.f14703c) == null) {
            pb.f fVar = (pb.f) this.f14702b;
            int[] iArr = fVar.f41112c;
            ob.d dVar = fVar.f41110a;
            int i10 = dVar.f16523a;
            int i11 = dVar.f16524b;
            pb.b bVar = new pb.b(i10, i11);
            if (fVar.f41111b.length < i10) {
                fVar.f41111b = new byte[i10];
            }
            for (int i12 = 0; i12 < 32; i12++) {
                iArr[i12] = 0;
            }
            for (int i13 = 1; i13 < 5; i13++) {
                byte[] b10 = dVar.b((i11 * i13) / 5, fVar.f41111b);
                int i14 = (i10 * 4) / 5;
                for (int i15 = i10 / 5; i15 < i14; i15++) {
                    int i16 = (b10[i15] & 255) >> 3;
                    iArr[i16] = iArr[i16] + 1;
                }
            }
            int length = iArr.length;
            int i17 = 0;
            int i18 = 0;
            int i19 = 0;
            for (int i20 = 0; i20 < length; i20++) {
                int i21 = iArr[i20];
                if (i21 > i17) {
                    i19 = i20;
                    i17 = i21;
                }
                if (i21 > i18) {
                    i18 = i21;
                }
            }
            int i22 = 0;
            int i23 = 0;
            for (int i24 = 0; i24 < length; i24++) {
                int i25 = i24 - i19;
                int i26 = iArr[i24] * i25 * i25;
                if (i26 > i23) {
                    i22 = i24;
                    i23 = i26;
                }
            }
            if (i19 <= i22) {
                int i27 = i19;
                i19 = i22;
                i22 = i27;
            }
            if (i19 - i22 > length / 16) {
                int i28 = i19 - 1;
                int i29 = i28;
                int i30 = -1;
                while (i28 > i22) {
                    int i31 = i28 - i22;
                    int i32 = (i18 - iArr[i28]) * (i19 - i28) * i31 * i31;
                    if (i32 > i30) {
                        i29 = i28;
                        i30 = i32;
                    }
                    i28--;
                }
                int i33 = i29 << 3;
                byte[] a2 = dVar.a();
                for (int i34 = 0; i34 < i11; i34++) {
                    int i35 = i34 * i10;
                    for (int i36 = 0; i36 < i10; i36++) {
                        if ((a2[i35 + i36] & 255) < i33) {
                            int i37 = (i36 / 32) + (bVar.f41098c * i34);
                            int[] iArr2 = bVar.d;
                            iArr2[i37] = iArr2[i37] | (1 << (i36 & 31));
                        }
                    }
                }
                this.f14703c = bVar;
            } else {
                throw ob.e.a();
            }
        }
        return (pb.b) this.f14703c;
    }

    @Override
    public int W() {
        return 0;
    }

    @Override
    public void X(Canvas canvas, float f10, float f11, float f12, float f13) {
        switch (this.f14701a) {
            case 18:
                canvas.save();
                canvas.clipRect(f10, f11, f12, f13);
                ((PhotoViewer) this.f14703c).T0(canvas, (org.telegram.ui.Components.fa) this.f14702b, -14277082, 855638016, false, true, true);
                canvas.drawColor(637534208);
                canvas.restore();
                return;
            default:
                Paint paint = (Paint) this.f14702b;
                PremiumPreviewFragment premiumPreviewFragment = (PremiumPreviewFragment) this.f14703c;
                eg.q1 q1Var = premiumPreviewFragment.f31964j0;
                if (premiumPreviewFragment.f31958e0) {
                    paint.setColor(premiumPreviewFragment.getThemedColor(org.telegram.ui.ActionBar.j6.f19852a7));
                    canvas.drawRect(f10, f11, f12, f13, paint);
                    return;
                }
                q1Var.d(0, (-premiumPreviewFragment.f31952a0.getMeasuredWidth()) * 0.1f * premiumPreviewFragment.Y, 0, premiumPreviewFragment.f31952a0.getMeasuredWidth(), 0.0f, premiumPreviewFragment.f31952a0.getMeasuredHeight());
                canvas.drawRect(f10, f11, f12, f13, q1Var.f5457f);
                return;
        }
    }

    public void X0(Exception exc, boolean z4) {
        int i10;
        this.f14703c = null;
        HashSet hashSet = (HashSet) this.f14702b;
        s8.v t6 = s8.v.t(hashSet);
        hashSet.clear();
        s8.t listIterator = t6.listIterator(0);
        while (listIterator.hasNext()) {
            o3.b bVar = (o3.b) listIterator.next();
            bVar.getClass();
            if (z4) {
                i10 = 1;
            } else {
                i10 = 3;
            }
            bVar.k(exc, i10);
        }
    }

    @Override
    public boolean Y0(org.telegram.ui.Cells.t1 t1Var, boolean z4) {
        return false;
    }

    @Override
    public ug.a Z() {
        return null;
    }

    @Override
    public boolean Z1(org.telegram.ui.Cells.t1 t1Var, TLRPC.PollAnswer pollAnswer) {
        return false;
    }

    @Override
    public boolean a() {
        switch (this.f14701a) {
            case 8:
            case 20:
            default:
                return org.telegram.ui.ActionBar.j6.I.q();
        }
    }

    @Override
    public boolean a0(org.telegram.ui.Cells.t1 t1Var) {
        return false;
    }

    @Override
    public vu0 a2() {
        return null;
    }

    @Override
    public void accept(Object obj, Object obj2) {
        boolean z4;
        q5.e0 e0Var = (q5.e0) this.f14702b;
        String str = (String) this.f14703c;
        u5.x xVar = (u5.x) obj;
        TaskCompletionSource taskCompletionSource = (TaskCompletionSource) obj2;
        if (e0Var.F == 2) {
            z4 = true;
        } else {
            z4 = false;
        }
        b6.m.j("Not connected to device", z4);
        u5.f fVar = (u5.f) xVar.u();
        Parcel M0 = fVar.M0();
        M0.writeString(str);
        fVar.R0(M0, 5);
        synchronized (e0Var.f42797s) {
            try {
                if (e0Var.f42794p != null) {
                    taskCompletionSource.setException(b6.m.m(new Status(2001, null, null, null)));
                } else {
                    e0Var.f42794p = taskCompletionSource;
                }
            } catch (Throwable th2) {
                throw th2;
            }
        }
    }

    @Override
    public void b0(float f10, boolean z4) {
        cc0 cc0Var = (cc0) this.f14702b;
        of0 of0Var = (of0) this.f14703c;
        i71 i71Var = of0Var.d;
        if (i71Var != null) {
            long p10 = i71Var.p();
            float max = 2.8f / ((float) Math.max(60L, p10));
            long j10 = (((f10 / (1.0f - max)) * max) + f10) * ((float) p10);
            of0Var.e = j10;
            of0Var.d.L(j10, !z4);
            if (!z4) {
                AndroidUtilities.cancelRunOnUIThread(cc0Var);
                AndroidUtilities.runOnUIThread(cc0Var, 120L);
            }
        }
    }

    public void b1(boolean z4, boolean z10, float f10) {
        ActionBarPopupWindow$ActionBarPopupWindowLayout actionBarPopupWindow$ActionBarPopupWindowLayout = (ActionBarPopupWindow$ActionBarPopupWindowLayout) this.f14702b;
        nh.w3 w3Var = (nh.w3) this.f14703c;
        nh.i9 i9Var = w3Var.e;
        nh.i9.f15446y1 = f10;
        nh.h9 h9Var = i9Var.f15496w0;
        if (h9Var != null) {
            h9Var.setSpeed(f10);
        }
        nh.d4.a0(w3Var.f16027l, z4);
        if (z10 && actionBarPopupWindow$ActionBarPopupWindowLayout.getSwipeBack() != null) {
            actionBarPopupWindow$ActionBarPopupWindowLayout.getSwipeBack().b(true);
        }
    }

    @Override
    public boolean mo18c(float f10, float f11, int i10, View view) {
        oy oyVar = (oy) this.f14703c;
        if (view instanceof org.telegram.ui.Cells.r2) {
            org.telegram.ui.Cells.r2 r2Var = (org.telegram.ui.Cells.r2) view;
            if (r2Var.f21671k2) {
                oyVar.N4(r2Var.getDialogId(), view);
                return true;
            }
        }
        nw nwVar = oyVar.f37115w0;
        if (nwVar != null && nwVar.getVisibility() == 0 && oyVar.f37115w0.f25830n) {
            return false;
        }
        return oyVar.o4(view, i10, f10, ((ny) this.f14702b).d);
    }

    @Override
    public int c0(int i10) {
        switch (this.f14701a) {
            case 8:
                return x0(i10);
            case 20:
                return x0(i10);
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.w0(null, i10, false);
                }
                return f6Var.c0(i10);
        }
    }

    public void c1(o0.g gVar) {
        androidx.biometric.p pVar = (androidx.biometric.p) this.f14703c;
        o5.i iVar = (o5.i) this.f14702b;
        int i10 = gVar.f16162b;
        if (i10 == 0) {
            pVar.execute(new androidx.biometric.k(iVar, gVar.f16161a, false, 26));
        } else {
            pVar.execute(new m2.b(iVar, i10));
        }
    }

    @Override
    public boolean d() {
        return false;
    }

    @Override
    public boolean d0(org.telegram.ui.Cells.t1 t1Var, TLRPC.User user) {
        return false;
    }

    @Override
    public boolean d1(int i10, org.telegram.ui.Cells.t1 t1Var) {
        return false;
    }

    @Override
    public boolean d2(long j10) {
        return ((f01) this.f14703c).f24742s;
    }

    @Override
    public void dismiss() {
        ((gv) this.f14703c).dismiss();
    }

    @Override
    public boolean e() {
        return true;
    }

    @Override
    public void f() {
        int i10 = this.f14701a;
    }

    @Override
    public boolean f2(org.telegram.ui.Cells.t1 t1Var, TLRPC.TodoItem todoItem) {
        return false;
    }

    @Override
    public String g(org.telegram.ui.Cells.t1 t1Var) {
        return null;
    }

    @Override
    public int g1(int i10) {
        switch (this.f14701a) {
            case 8:
                return ((SparseIntArray) this.f14702b).get(i10);
            case 20:
                return x0(i10);
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.w0(null, i10, false);
                }
                return f6Var.g1(i10);
        }
    }

    @Override
    public Drawable getDrawable(String str) {
        switch (this.f14701a) {
            case 8:
                return null;
            case 20:
                return null;
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.O0(str);
                }
                return f6Var.getDrawable(str);
        }
    }

    @Override
    public void h() {
        Point point = AndroidUtilities.displaySize;
        if (point.x > point.y) {
            ((oy) this.f14703c).finishPreviewFragment();
        }
    }

    @Override
    public boolean h0() {
        return false;
    }

    public void i1(p2.n nVar) {
        this.f14702b = nVar;
        if (nVar.a() != null) {
            nVar.a().getClass();
            String str = nVar.a().d;
            if (str != null) {
                this.f14703c = str;
            }
        }
    }

    @Override
    public void j0() {
        ((vt) this.f14702b).getText();
        ((org.telegram.ui.Cells.f3) this.f14703c).b();
    }

    @Override
    public boolean j1(MessageObject messageObject) {
        return org.telegram.ui.b.a(messageObject);
    }

    @Override
    public pg.b k() {
        switch (this.f14701a) {
            case 18:
                if (Build.VERSION.SDK_INT >= 29) {
                    pg.c cVar = new pg.c(this);
                    ((PhotoViewer) this.f14703c).W.add(cVar);
                    return cVar;
                }
                return new pg.d(this);
            default:
                return new pg.d(this);
        }
    }

    @Override
    public int k0(org.telegram.ui.Cells.t1 t1Var) {
        return 0;
    }

    public r3.v k1(int i10) {
        int i11 = 0;
        while (true) {
            int[] iArr = (int[]) this.f14702b;
            if (i11 < iArr.length) {
                if (i10 == iArr[i11]) {
                    return ((o4.k0[]) this.f14703c)[i11];
                }
                i11++;
            } else {
                h5.a.o("BaseMediaChunkOutput", "Unmatched track of type: " + i10);
                return new r3.j();
            }
        }
    }

    @Override
    public void l(float f10, float f11, int i10, int i11) {
        switch (this.f14701a) {
            case 8:
                org.telegram.ui.ActionBar.j6.q(f10, f11, i10, i11);
                return;
            case 20:
                org.telegram.ui.ActionBar.j6.q(f10, f11, i10, i11);
                return;
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    org.telegram.ui.ActionBar.j6.q(f10, f11, i10, i11);
                    return;
                } else {
                    f6Var.l(f10, f11, i10, i11);
                    return;
                }
        }
    }

    @Override
    public boolean m0() {
        switch (this.f14701a) {
            case 8:
                return false;
            case 20:
                return false;
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.a1();
                }
                return f6Var.m0();
        }
    }

    public void m1(com.google.android.gms.internal.play_billing.g3 g3Var) {
        try {
            v1(g3Var, (com.google.android.gms.internal.play_billing.p3) this.f14702b);
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public boolean n0() {
        return false;
    }

    public void n1(com.google.android.gms.internal.play_billing.g3 g3Var, int i10, long j10) {
        try {
            com.google.android.gms.internal.play_billing.o3 o3Var = (com.google.android.gms.internal.play_billing.o3) ((com.google.android.gms.internal.play_billing.p3) this.f14702b).g();
            o3Var.c();
            com.google.android.gms.internal.play_billing.p3.p((com.google.android.gms.internal.play_billing.p3) o3Var.f3595b, i10);
            com.google.android.gms.internal.play_billing.p3 p3Var = (com.google.android.gms.internal.play_billing.p3) o3Var.a();
            this.f14702b = p3Var;
            if (j10 != 0) {
                com.google.android.gms.internal.play_billing.o3 o3Var2 = (com.google.android.gms.internal.play_billing.o3) p3Var.g();
                o3Var2.c();
                com.google.android.gms.internal.play_billing.p3.r((com.google.android.gms.internal.play_billing.p3) o3Var2.f3595b, j10);
                p3Var = (com.google.android.gms.internal.play_billing.p3) o3Var2.a();
            }
            v1(g3Var, p3Var);
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public void onComplete(Task task) {
        o8.c cVar = (o8.c) this.f14702b;
        TaskCompletionSource taskCompletionSource = (TaskCompletionSource) this.f14703c;
        synchronized (cVar.f16471f) {
            cVar.e.remove(taskCompletionSource);
        }
    }

    @Override
    public void onError(i71 i71Var, Exception exc) {
        SecretMediaViewer secretMediaViewer = (SecretMediaViewer) this.f14703c;
        int i10 = secretMediaViewer.Y;
        if (i10 > 0) {
            secretMediaViewer.Y = i10 - 1;
            AndroidUtilities.runOnUIThread(new l01(10, this, (File) this.f14702b), 100L);
            return;
        }
        FileLog.e(exc);
    }

    @Override
    public void onRenderedFirstFrame(k3.a aVar) {
    }

    @Override
    public void onStateChanged(boolean z4, int i10) {
        SecretMediaViewer secretMediaViewer = (SecretMediaViewer) this.f14703c;
        b41 b41Var = secretMediaViewer.f32239f1;
        if (secretMediaViewer.f32275y != null && secretMediaViewer.f32235e0 != null) {
            AndroidUtilities.cancelRunOnUIThread(b41Var);
            AndroidUtilities.runOnUIThread(b41Var);
            if (i10 != 4 && i10 != 1) {
                try {
                    secretMediaViewer.f32227b.getWindow().addFlags(128);
                } catch (Exception e) {
                    FileLog.e(e);
                }
            } else {
                try {
                    secretMediaViewer.f32227b.getWindow().clearFlags(128);
                } catch (Exception e6) {
                    FileLog.e(e6);
                }
            }
            if (i10 == 3 && secretMediaViewer.f32271w.getVisibility() != 0) {
                secretMediaViewer.f32271w.setVisibility(0);
            }
            if (secretMediaViewer.f32275y.y() && i10 != 4) {
                if (!secretMediaViewer.B) {
                    secretMediaViewer.B = true;
                }
            } else if (secretMediaViewer.B) {
                secretMediaViewer.B = false;
                if (i10 == 4) {
                    secretMediaViewer.E = true;
                    if (secretMediaViewer.F) {
                        secretMediaViewer.e(true, !secretMediaViewer.f32254n1);
                        return;
                    }
                    secretMediaViewer.f32275y.L(0L, false);
                    secretMediaViewer.f32275y.C();
                }
            }
        }
    }

    @Override
    public boolean onSurfaceDestroyed(SurfaceTexture surfaceTexture) {
        return false;
    }

    @Override
    public void onVideoSizeChanged(int i10, int i11, int i12, float f10) {
        float f11;
        org.telegram.ui.o4 o4Var = ((SecretMediaViewer) this.f14703c).f32271w;
        if (o4Var != null) {
            if (i12 != 90 && i12 != 270) {
                i11 = i10;
                i10 = i11;
            }
            if (i10 == 0) {
                f11 = 1.0f;
            } else {
                f11 = (i11 * f10) / i10;
            }
            o4Var.a(f11, i12);
        }
    }

    @Override
    public void p(float f10) {
        Point point = AndroidUtilities.displaySize;
        if (point.x > point.y) {
            ((oy) this.f14703c).movePreviewFragment(f10);
        }
    }

    public void p1(com.google.android.gms.internal.play_billing.g3 g3Var, long j10, boolean z4) {
        com.google.android.gms.internal.play_billing.p3 p3Var;
        try {
            com.google.android.gms.internal.play_billing.f3 f3Var = (com.google.android.gms.internal.play_billing.f3) g3Var.g();
            com.google.android.gms.internal.play_billing.t3 t3Var = (com.google.android.gms.internal.play_billing.t3) g3Var.o().g();
            t3Var.c();
            com.google.android.gms.internal.play_billing.v3.n((com.google.android.gms.internal.play_billing.v3) t3Var.f3595b, z4);
            f3Var.c();
            com.google.android.gms.internal.play_billing.g3.r((com.google.android.gms.internal.play_billing.g3) f3Var.f3595b, (com.google.android.gms.internal.play_billing.v3) t3Var.a());
            com.google.android.gms.internal.play_billing.g3 g3Var2 = (com.google.android.gms.internal.play_billing.g3) f3Var.a();
            if (j10 == 0) {
                p3Var = (com.google.android.gms.internal.play_billing.p3) this.f14702b;
            } else {
                com.google.android.gms.internal.play_billing.o3 o3Var = (com.google.android.gms.internal.play_billing.o3) ((com.google.android.gms.internal.play_billing.p3) this.f14702b).g();
                o3Var.c();
                com.google.android.gms.internal.play_billing.p3.r((com.google.android.gms.internal.play_billing.p3) o3Var.f3595b, j10);
                p3Var = (com.google.android.gms.internal.play_billing.p3) o3Var.a();
            }
            v1(g3Var2, p3Var);
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public boolean p2(org.telegram.ui.Cells.t1 t1Var, TL_iv.PageBlock pageBlock) {
        return false;
    }

    @Override
    public boolean q0(org.telegram.ui.Components.u5 u5Var) {
        return false;
    }

    public void q1(com.google.android.gms.internal.play_billing.g3 g3Var, int i10, long j10, boolean z4) {
        com.google.android.gms.internal.play_billing.p3 p3Var;
        try {
            com.google.android.gms.internal.play_billing.o3 o3Var = (com.google.android.gms.internal.play_billing.o3) ((com.google.android.gms.internal.play_billing.p3) this.f14702b).g();
            o3Var.c();
            com.google.android.gms.internal.play_billing.p3.p((com.google.android.gms.internal.play_billing.p3) o3Var.f3595b, i10);
            this.f14702b = (com.google.android.gms.internal.play_billing.p3) o3Var.a();
            com.google.android.gms.internal.play_billing.f3 f3Var = (com.google.android.gms.internal.play_billing.f3) g3Var.g();
            com.google.android.gms.internal.play_billing.t3 t3Var = (com.google.android.gms.internal.play_billing.t3) g3Var.o().g();
            t3Var.c();
            com.google.android.gms.internal.play_billing.v3.n((com.google.android.gms.internal.play_billing.v3) t3Var.f3595b, z4);
            f3Var.c();
            com.google.android.gms.internal.play_billing.g3.r((com.google.android.gms.internal.play_billing.g3) f3Var.f3595b, (com.google.android.gms.internal.play_billing.v3) t3Var.a());
            com.google.android.gms.internal.play_billing.g3 g3Var2 = (com.google.android.gms.internal.play_billing.g3) f3Var.a();
            if (j10 == 0) {
                p3Var = (com.google.android.gms.internal.play_billing.p3) this.f14702b;
            } else {
                com.google.android.gms.internal.play_billing.o3 o3Var2 = (com.google.android.gms.internal.play_billing.o3) ((com.google.android.gms.internal.play_billing.p3) this.f14702b).g();
                o3Var2.c();
                com.google.android.gms.internal.play_billing.p3.r((com.google.android.gms.internal.play_billing.p3) o3Var2.f3595b, j10);
                p3Var = (com.google.android.gms.internal.play_billing.p3) o3Var2.a();
            }
            v1(g3Var2, p3Var);
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public void r(org.telegram.ui.u6 u6Var, mh.a aVar, boolean z4) {
        gv gvVar = (gv) this.f14703c;
        fv fvVar = gvVar.U;
        if (aVar != null) {
            ((mh.b) this.f14702b).i(aVar);
            gvVar.f34686b0.d();
            mh.b bVar = gvVar.f34688d0;
            ey0[] ey0VarArr = gvVar.Y;
            org.telegram.ui.Cells.z1[] z1VarArr = gvVar.Z;
            org.telegram.ui.Cells.z1 z1Var = z1VarArr[0];
            if (z1Var != null) {
                ey0 ey0Var = ey0VarArr[0];
                boolean z10 = bVar.f14183m;
                ey0Var.f24703c = z10;
                z1Var.c(z10, true);
            }
            org.telegram.ui.Cells.z1 z1Var2 = z1VarArr[1];
            if (z1Var2 != null) {
                ey0 ey0Var2 = ey0VarArr[1];
                boolean z11 = bVar.f14184n;
                ey0Var2.f24703c = z11;
                z1Var2.c(z11, true);
            }
            org.telegram.ui.Cells.z1 z1Var3 = z1VarArr[2];
            if (z1Var3 != null) {
                ey0 ey0Var3 = ey0VarArr[2];
                boolean z12 = bVar.f14185o;
                ey0Var3.f24703c = z12;
                z1Var3.c(z12, true);
            }
            org.telegram.ui.Cells.z1 z1Var4 = z1VarArr[3];
            if (z1Var4 != null) {
                ey0 ey0Var4 = ey0VarArr[3];
                boolean z13 = bVar.f14186p;
                ey0Var4.f24703c = z13;
                z1Var4.c(z13, true);
            }
            org.telegram.ui.Cells.z1 z1Var5 = z1VarArr[4];
            if (z1Var5 != null) {
                ey0 ey0Var5 = ey0VarArr[4];
                boolean z14 = bVar.f14187q;
                ey0Var5.f24703c = z14;
                z1Var5.c(z14, true);
            }
            gvVar.X.a(fvVar.d(), true);
            fvVar.c(true);
        }
    }

    public void s1(com.google.android.gms.internal.play_billing.l3 l3Var) {
        try {
            com.google.android.gms.internal.play_billing.w3 t6 = com.google.android.gms.internal.play_billing.x3.t();
            t6.d((com.google.android.gms.internal.play_billing.p3) this.f14702b);
            t6.c();
            com.google.android.gms.internal.play_billing.x3.p((com.google.android.gms.internal.play_billing.x3) t6.f3595b, l3Var);
            ((c2.u) this.f14703c).h((com.google.android.gms.internal.play_billing.x3) t6.a());
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    public void t1(com.google.android.gms.internal.play_billing.a4 a4Var) {
        try {
            com.google.android.gms.internal.play_billing.w3 t6 = com.google.android.gms.internal.play_billing.x3.t();
            t6.d((com.google.android.gms.internal.play_billing.p3) this.f14702b);
            t6.c();
            com.google.android.gms.internal.play_billing.x3.r((com.google.android.gms.internal.play_billing.x3) t6.f3595b, a4Var);
            ((c2.u) this.f14703c).h((com.google.android.gms.internal.play_billing.x3) t6.a());
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    public String toString() {
        switch (this.f14701a) {
            case 6:
                try {
                    return V0().toString();
                } catch (ob.e unused) {
                    return "";
                }
            default:
                return super.toString();
        }
    }

    public void u1(com.google.android.gms.internal.play_billing.b4 b4Var) {
        if (b4Var == null) {
            return;
        }
        try {
            com.google.android.gms.internal.play_billing.w3 t6 = com.google.android.gms.internal.play_billing.x3.t();
            t6.d((com.google.android.gms.internal.play_billing.p3) this.f14702b);
            t6.c();
            com.google.android.gms.internal.play_billing.x3.s((com.google.android.gms.internal.play_billing.x3) t6.f3595b, b4Var);
            ((c2.u) this.f14703c).h((com.google.android.gms.internal.play_billing.x3) t6.a());
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    public void v1(com.google.android.gms.internal.play_billing.g3 g3Var, com.google.android.gms.internal.play_billing.p3 p3Var) {
        if (g3Var == null) {
            return;
        }
        try {
            com.google.android.gms.internal.play_billing.w3 t6 = com.google.android.gms.internal.play_billing.x3.t();
            t6.d(p3Var);
            t6.c();
            com.google.android.gms.internal.play_billing.x3.n((com.google.android.gms.internal.play_billing.x3) t6.f3595b, g3Var);
            ((c2.u) this.f14703c).h((com.google.android.gms.internal.play_billing.x3) t6.a());
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public String w(long j10) {
        String trim = ((EditTextBoldCursor) this.f14702b).getText().toString().trim();
        if (trim.length() > 16) {
            trim = trim.substring(0, 16);
        }
        if (!((f01) this.f14703c).f24742s && TextUtils.isEmpty(trim)) {
            return null;
        }
        return trim;
    }

    public void w1(com.google.android.gms.internal.play_billing.i3 i3Var, com.google.android.gms.internal.play_billing.p3 p3Var) {
        try {
            com.google.android.gms.internal.play_billing.w3 t6 = com.google.android.gms.internal.play_billing.x3.t();
            t6.d(p3Var);
            t6.c();
            com.google.android.gms.internal.play_billing.x3.o((com.google.android.gms.internal.play_billing.x3) t6.f3595b, i3Var);
            ((c2.u) this.f14703c).h((com.google.android.gms.internal.play_billing.x3) t6.a());
        } catch (Throwable th2) {
            com.google.android.gms.internal.play_billing.u.i("BillingLogger", "Unable to log.", th2);
        }
    }

    @Override
    public ColorFilter x() {
        switch (this.f14701a) {
            case 8:
                return org.telegram.ui.ActionBar.j6.f20222v3;
            case 20:
                return org.telegram.ui.ActionBar.j6.f20222v3;
            default:
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.f20222v3;
                }
                return f6Var.x();
        }
    }

    @Override
    public int x0(int i10) {
        switch (this.f14701a) {
            case 8:
                SparseIntArray sparseIntArray = (SparseIntArray) this.f14702b;
                int indexOfKey = sparseIntArray.indexOfKey(i10);
                if (indexOfKey >= 0) {
                    return sparseIntArray.valueAt(indexOfKey);
                }
                return org.telegram.ui.ActionBar.j6.w0(null, i10, false);
            case 20:
                SparseIntArray sparseIntArray2 = (SparseIntArray) this.f14702b;
                if (sparseIntArray2 != null) {
                    return sparseIntArray2.get(i10);
                }
                return org.telegram.ui.ActionBar.j6.w0(null, i10, false);
            default:
                SparseIntArray sparseIntArray3 = (SparseIntArray) this.f14702b;
                int indexOfKey2 = sparseIntArray3.indexOfKey(i10);
                if (indexOfKey2 >= 0) {
                    return sparseIntArray3.valueAt(indexOfKey2);
                }
                org.telegram.ui.ActionBar.f6 f6Var = (org.telegram.ui.ActionBar.f6) this.f14703c;
                if (f6Var == null) {
                    return org.telegram.ui.ActionBar.j6.w0(null, i10, false);
                }
                return f6Var.x0(i10);
        }
    }

    @Override
    public boolean y2(int i10) {
        return false;
    }

    @Override
    public java.lang.Object z(od.c r6, wc.c r7) {
        throw new UnsupportedOperationException("Method not decompiled: n7.qa.z(od.c, wc.c):java.lang.Object");
    }

    public qa(int i10, Object obj, Object obj2) {
        this.f14701a = i10;
        this.f14703c = obj;
        this.f14702b = obj2;
    }

    @Override
    public void onRenderedFirstFrame() {
        SecretMediaViewer secretMediaViewer = (SecretMediaViewer) this.f14703c;
        if (secretMediaViewer.Z) {
            return;
        }
        secretMediaViewer.Z = true;
        secretMediaViewer.e.invalidate();
    }

    public qa(Object obj, Object obj2, boolean z4, int i10) {
        this.f14701a = i10;
        this.f14702b = obj;
        this.f14703c = obj2;
    }

    public qa(Context context, com.google.android.gms.internal.play_billing.p3 p3Var) {
        this.f14701a = 24;
        c2.u uVar = new c2.u(5);
        try {
            y2.q.b(context);
            uVar.f2007c = y2.q.a().c(w2.a.e).a("PLAY_BILLING_LIBRARY", new v2.c("proto"), new db.a(18));
        } catch (Throwable unused) {
            uVar.f2006b = true;
        }
        this.f14703c = uVar;
        this.f14702b = p3Var;
    }

    public qa(org.telegram.ui.ActionBar.f6 f6Var) {
        this.f14701a = 22;
        this.f14702b = new SparseIntArray();
        this.f14703c = f6Var;
        M0();
    }

    public qa(pb.f fVar) {
        this.f14701a = 6;
        this.f14702b = fVar;
    }

    public qa(View view) {
        this.f14701a = 14;
        t61 t61Var = new t61(this, view);
        this.f14702b = t61Var;
        view.addOnLayoutChangeListener(t61Var);
    }

    public qa(of0 of0Var) {
        this.f14701a = 12;
        this.f14703c = of0Var;
        this.f14702b = new cc0(this, 9);
    }

    public qa(EditText editText) {
        this.f14701a = 26;
        this.f14702b = editText;
        q1.i iVar = new q1.i(editText);
        this.f14703c = iVar;
        editText.addTextChangedListener(iVar);
        if (q1.a.f42679b == null) {
            synchronized (q1.a.f42678a) {
                try {
                    if (q1.a.f42679b == null) {
                        ?? factory = new Editable.Factory();
                        try {
                            q1.a.f42680c = Class.forName("android.text.DynamicLayout$ChangeWatcher", false, q1.a.class.getClassLoader());
                        } catch (Throwable unused) {
                        }
                        q1.a.f42679b = factory;
                    }
                } finally {
                }
            }
        }
        editText.setEditableFactory(q1.a.f42679b);
    }

    private final void S0() {
    }

    private final void U0() {
    }

    @Override
    public void A2() {
    }

    @Override
    public void C0() {
    }

    @Override
    public void H() {
    }

    public void M0() {
    }

    @Override
    public void V1() {
    }

    @Override
    public void clear() {
    }

    @Override
    public void h1() {
    }

    @Override
    public void j() {
    }

    @Override
    public void l1() {
    }

    @Override
    public void o() {
    }

    @Override
    public void s() {
    }

    @Override
    public void t2() {
    }

    @Override
    public void w0() {
    }

    @Override
    public void A(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void B(float f10) {
    }

    @Override
    public void B0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void D0(float f10) {
    }

    @Override
    public void E1(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void F(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void H0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void I(MessageObject.TextLayoutBlock textLayoutBlock) {
    }

    @Override
    public void I0(long j10) {
    }

    @Override
    public void J(float f10) {
    }

    @Override
    public void J0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void K(boolean z4) {
    }

    @Override
    public void L(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void L0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void M1(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void O(MessageObject messageObject) {
    }

    @Override
    public void O1(MessageObject messageObject) {
    }

    @Override
    public void P0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void U(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void Y(float f10) {
    }

    @Override
    public void Z0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void a1(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void b(int i10) {
    }

    @Override
    public void e0(long j10) {
    }

    @Override
    public void f0(boolean z4) {
    }

    @Override
    public void g0(int i10) {
    }

    @Override
    public void i2(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void l0(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void n(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void onSeekFinished(k3.a aVar) {
    }

    @Override
    public void onSeekStarted(k3.a aVar) {
    }

    @Override
    public void onSurfaceTextureUpdated(SurfaceTexture surfaceTexture) {
    }

    @Override
    public void p0(String str) {
    }

    @Override
    public void q(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void q2(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void r0(float f10) {
    }

    @Override
    public void t(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void t0(float f10) {
    }

    @Override
    public void u(org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void v(float f10) {
    }

    @Override
    public void v0(float f10) {
    }

    @Override
    public void y(org.telegram.ui.Cells.t1 t1Var) {
    }

    public qa(PremiumPreviewFragment premiumPreviewFragment) {
        this.f14701a = 19;
        this.f14703c = premiumPreviewFragment;
        this.f14702b = new Paint();
    }

    private final void e1(int i10, int i11) {
    }

    private final void f1(int i10, int i11) {
    }

    @Override
    public void C(long j10, boolean z4) {
    }

    @Override
    public void E(org.telegram.ui.Cells.t1 t1Var, BotInlineKeyboard.ButtonCustom buttonCustom) {
    }

    @Override
    public void G0(int i10, long j10) {
    }

    @Override
    public void G1(org.telegram.ui.Cells.t1 t1Var, boolean z4) {
    }

    @Override
    public void J1(org.telegram.ui.Cells.t1 t1Var, TL_keyboard.KeyboardButtonProto keyboardButtonProto) {
    }

    @Override
    public void M(org.telegram.ui.Cells.t1 t1Var, jh.f fVar) {
    }

    @Override
    public void N(int i10, org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void R0(int i10, org.telegram.ui.Cells.t1 t1Var) {
    }

    @Override
    public void T0(org.telegram.ui.Cells.t1 t1Var, TL_keyboard.KeyboardInlineButton keyboardInlineButton) {
    }

    @Override
    public void V(float f10, int i10) {
    }

    @Override
    public void X1(org.telegram.ui.Cells.t1 t1Var, TLRPC.MessageExtendedMedia messageExtendedMedia) {
    }

    @Override
    public void k2(org.telegram.ui.Cells.t1 t1Var, long j10) {
    }

    @Override
    public void o0(float f10, int i10) {
    }

    @Override
    public void o1(org.telegram.ui.Cells.t1 t1Var, TL_keyboard.KeyboardButtonProto keyboardButtonProto) {
    }

    @Override
    public void r1(org.telegram.ui.Cells.t1 t1Var, TLRPC.Document document) {
    }

    @Override
    public void u0(float f10, int i10) {
    }

    public qa() {
        this.f14701a = 4;
        this.f14702b = new HashSet();
    }

    public qa(m21 m21Var) {
        this.f14701a = 20;
        this.f14703c = m21Var;
    }

    public qa(bf.b bVar) {
        this.f14701a = 27;
        this.f14702b = bVar;
        this.f14703c = "";
    }

    public qa(PhotoViewer photoViewer) {
        this.f14701a = 18;
        this.f14703c = photoViewer;
        this.f14702b = new org.telegram.ui.Components.fa(photoViewer.Y, photoViewer.f31705b0, 0, false);
    }

    @Override
    public void B2(org.telegram.ui.Cells.t1 t1Var, int i10, int i11) {
    }

    @Override
    public void E0(org.telegram.ui.Cells.t1 t1Var, TLObject tLObject, boolean z4) {
    }

    @Override
    public void F0(org.telegram.ui.Cells.t1 t1Var, float f10, float f11) {
    }

    @Override
    public void W0(org.telegram.ui.Cells.t1 t1Var, CharacterStyle characterStyle, boolean z4) {
    }

    @Override
    public void i0(org.telegram.ui.Cells.t1 t1Var, float f10, float f11) {
    }

    @Override
    public void s0(org.telegram.ui.Cells.t1 t1Var, float f10, float f11) {
    }

    @Override
    public void x1(org.telegram.ui.Cells.t1 t1Var, float f10, float f11) {
    }

    @Override
    public void Y1(org.telegram.ui.Cells.t1 t1Var, TLRPC.User user, TLRPC.Document document, String str) {
    }

    @Override
    public void m(org.telegram.ui.Cells.t1 t1Var, TLRPC.PollAnswer pollAnswer, TLRPC.MessageMedia messageMedia, int i10) {
    }

    @Override
    public void y0(org.telegram.ui.Cells.t1 t1Var, TLRPC.User user, float f10, float f11) {
    }

    @Override
    public void z0(org.telegram.ui.Cells.t1 t1Var, float f10, float f11, boolean z4) {
    }

    @Override
    public void e2(org.telegram.ui.Cells.t1 t1Var, int i10, float f10, float f11, boolean z4) {
    }

    @Override
    public void i(org.telegram.ui.Cells.t1 t1Var, ArrayList arrayList, int i10, int i11, int i12) {
    }

    @Override
    public void v2(org.telegram.ui.Cells.t1 t1Var, TLRPC.ReactionCount reactionCount, boolean z4, float f10, float f11) {
    }

    @Override
    public void T(org.telegram.ui.Cells.t1 t1Var, TLRPC.Chat chat, int i10, float f10, float f11, boolean z4) {
    }

    @Override
    public void S1(MessageObject messageObject, String str, String str2, String str3, String str4, int i10, int i11) {
    }
}
